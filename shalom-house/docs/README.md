...
- 기준 문서는 현재 유효한 목표를 기술한다. `1차`, `후속 단계`처럼 과거 작업 시점에 의존하는 설명은 새 요구사항을 덧붙이기 전에 갱신한다.
- 실제 구현은 대상 커밋과 호출 관계로 확인한다. 배포 화면은 URL과 확인 시점을 구분하고 로컬 구현과 동일하다고 가정하지 않는다.
- 디자인 시스템의 토큰 이름은 CSS와 연결하되 현재 값의 목록을 목표 디자인의 근거로 삼지 않는다.
- 새 기준 화면은 홈·소식 목록·찾아오시는 길의 실제 렌더링으로 확인한다. 기준 화면이 아직 없으면 미확정이라고 표시한다.
- 캡처와 일회성 검증 결과는 작업 응답 또는 사용자가 요청한 산출물에 남긴다. 기준 문서에 감사 기록을 누적하지 않는다.

### 특정 작업용 문서

- [`technical/content-platform-architecture.md`](./technical/content-platform-architecture.md): 뉴스·CMS 기술 구조
- [`operations/official-content-intake.md`](./operations/official-content-intake.md): 첫 콘텐츠 전달 항목, 관리자 입력 위치와 항목별 공개 승인
...
- 중복되거나 충돌하는 규칙은 소유 문서에서 해결하고 다른 문서는 링크로 참조한다. 모호한 금지사항을 추가하는 것으로 대체하지 않는다.
