...
export type StaffProfileFieldErrors = Partial<
  Record<keyof StaffProfileInput | "saveConfirmed" | "expectedUpdatedAt", string>
>;
...
export function validateStaffProfileInput(
  input: unknown,
): { ok: true; value: StaffProfileInput } | { ok: false; fieldErrors: StaffProfileFieldErrors } {
  ...
  if (!role || role.length > 80 || forbiddenMarkup.test(role))
    errors.role = "직책·역할은 서식 없이 1~80자로 입력해 주세요.";
  if (responsibility.length < 10 || responsibility.length > 600 || forbiddenMarkup.test(responsibility))
    errors.responsibility = "담당 업무는 서식 없이 10~600자로 입력해 주세요.";
  ...
  if (typeof value.nameDisclosureConfirmed !== "boolean")
    errors.nameDisclosureConfirmed = "이름 공개 확인 여부를 확인해 주세요.";
  if (reference.length > 120 || forbiddenMarkup.test(reference))
    errors.nameDisclosureReference = "확인 근거는 서식 없이 120자 이하로 입력해 주세요.";
  if (value.showName === true && (!name || value.nameDisclosureConfirmed !== true || !reference))
    errors.nameDisclosureConfirmed = "이름 공개에는 이름, 공개 확인, 확인 근거가 모두 필요합니다.";
  ...
  return {
    ...
    value: {
      ...
      responsibility,
      name,
      showName: value.showName as boolean,
      nameDisclosureConfirmed: value.showName ? (value.nameDisclosureConfirmed as boolean) : false,
      nameDisclosureReference: value.showName ? reference : "",
      publicationStatus: value.publicationStatus as StaffProfileInput["publicationStatus"],
      displayOrder: value.displayOrder as number,
    },
  };
}
export function validateStaffProfileSaveInput(
  input: unknown,
): { ok: true; value: StaffProfileSaveInput } | { ok: false; fieldErrors: StaffProfileFieldErrors } {
  const value = typeof input === "object" && input !== null ? (input as Record<string, unknown>) : {};
  const errors: StaffProfileFieldErrors = {};
  if (!exactKeys(value, ["expectedUpdatedAt", "saveConfirmed", "profile"]))
  ...
  if (!profile.ok || Object.keys(errors).length)
    return {
      ok: false,
      fieldErrors: { ...(!profile.ok ? profile.fieldErrors : {}), ...errors },
    };
