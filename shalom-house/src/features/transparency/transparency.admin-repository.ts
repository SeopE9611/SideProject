...
async function changeAdminTransparencyTrashState(input: { id: string; expectedUpdatedAt: Date; actor: AdminPrincipal; restore: boolean }): Promise<AdminTransparencyTrashResult> {
  ...
  try { return await transaction(async (database, session) => {
    ...
    const after = await collection.findOneAndUpdate({ _id: id, updatedAt: input.expectedUpdatedAt, deletedAt: input.restore ? { $type: "date" } : null }, { $set: { deletedAt: input.restore ? null : transitionAt, archivedAt: input.restore ? null : transitionAt, publicationStatus: input.restore ? "draft" : "archived", approvalStatus: "pending", publishedAt: null, updatedAt: transitionAt } }, { session, returnDocument: "after" });
    if (!after) return { ok: false, reason: "edit_conflict" } as const;
    await insertTransparencyAuditEvent({ database, session, documentId: id, action: input.restore ? "restored" : "soft_deleted", actor: input.actor, occurredAt: transitionAt, fromVersionAt: current.updatedAt, toVersionAt: transitionAt, before: createTransparencyAuditSnapshot(current), after: createTransparencyAuditSnapshot(after), changedFields: ["deletedAt", "archivedAt", "publicationStatus", "approvalStatus", "publishedAt"] });
