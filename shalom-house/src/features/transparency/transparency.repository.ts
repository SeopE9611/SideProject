...
export type PublicTransparencyDocument = {
  slug: string; title: string; category: TransparencyCategory; periodLabel: string;
  summary: string; documentDate: string; publishedAt: string; fileType: "PDF"; byteSize: number;
};
export type PublicTransparencyDocumentMedia = { bucket: string; objectPath: string; mimeType: "application/pdf"; byteSize: number };
...
const empty: TransparencyRepository = { async listPublished() { return []; }, async findMediaBySlug() { return null; } };
function getTransparencyRepository(): TransparencyRepository {
  const configured = process.env.SHALOM_CONTENT_SOURCE;
  const source = configured || (process.env.NODE_ENV === "development" || process.env.VERCEL_ENV === "preview" ? "fixture" : "empty");
...
export const findPublicTransparencyDocumentMediaBySlug = (slug: string) => getTransparencyRepository().findMediaBySlug(slug);
