...
export function normalizeAdminTransparencyOriginalFileName(value: unknown): string | null {
  ...
  const name = value.replaceAll("\\", "/").split("/").at(-1)?.replace(/[\u0000-\u001F\u007F]/g, "").trim().slice(0, 120).trim();
  return name || null;
}
export function validateAdminTransparencyDraftInput(input: unknown) {
  const value = typeof input === "object" && input !== null ? input as Record<string, unknown> : {};
  const draft = {
    slug: text(value.slug).toLowerCase(), title: text(value.title), category: value.category,
    periodLabel: text(value.periodLabel), summary: text(value.summary), documentDate: text(value.documentDate),
    privacyReviewStatus: value.privacyReviewStatus, finalDocumentStatus: value.finalDocumentStatus,
  };
  const errors: AdminTransparencyFieldErrors = {};
  if (!draft.slug || draft.slug.length > 80 || !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(draft.slug)) errors.slug = "영문 소문자, 숫자와 하이픈만 사용해 80자 이하로 입력해 주세요.";
  ...
  if (!draft.periodLabel || draft.periodLabel.length > 80) errors.periodLabel = "기준 기간을 1자 이상 80자 이하로 입력해 주세요.";
  ...
  if (!isTransparencyPrivacyReviewStatus(draft.privacyReviewStatus)) errors.privacyReviewStatus = "개인정보 검토 상태를 선택해 주세요.";
  if (!isTransparencyFinalDocumentStatus(draft.finalDocumentStatus)) errors.finalDocumentStatus = "최종본 상태를 선택해 주세요.";
...
export function validateAdminTransparencyDraftUpdateInput(input: unknown) {
  ...
  return result.ok && valid ? { ok: true as const, value: { draft: result.value, expectedUpdatedAt: expected } } : { ok: false as const, fieldErrors: result.ok ? {} : result.fieldErrors, formError: valid ? undefined : "invalid_version" };
}
...
export async function validateAdminTransparencyPdf(file: File) {
  ...
  if (file.type !== "application/pdf") return { ok: false as const, error: "application/pdf 형식만 업로드할 수 있습니다." };
  const buffer = Buffer.from(await file.arrayBuffer());
  if (buffer.subarray(0, 5).toString("ascii") !== "%PDF-") return { ok: false as const, error: "올바른 PDF 파일이 아닙니다." };
  return { ok: true as const, value: { buffer, byteSize: buffer.byteLength, sha256: createHash("sha256").update(buffer).digest("hex") } };
}
