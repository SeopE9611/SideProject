...
import {
  ...
  type TransparencyAuditAction,
  type TransparencyAuditSnapshot,
  ...
  await input.database.collection(TRANSPARENCY_AUDIT_COLLECTION_NAME).insertOne(
    {
      _id: new ObjectId(),
      schemaVersion: 1,
      transparencyDocumentId: input.documentId,
      action: input.action,
      actor: {
...
const adminAuditFieldLabels: Record<string, string> = {
  ...
  summary: "요약",
  documentDate: "문서일",
  privacyReviewStatus: "개인정보 검토 상태",
  finalDocumentStatus: "최종본 상태",
  file: "파일",
...
type AdminAuditProjection = {
  ...
  transparencyDocumentId?: unknown;
};

export async function listAdminTransparencyAuditHistory(input: {
  contentId: string;
  limit?: number;
  ...
  const documents = await (
    ...
    .find(
      { transparencyDocumentId: contentId },
      {
        projection: {
          _id: 1,
          action: 1,
  ...
  return documents.flatMap((event) => {
    const valid =
      ...
      transparencyAuditActions.includes(event.action as never) &&
