...
async function getContent<T extends FacilityOverviewContent | GreetingContent>(key: SiteContentKey, fallback: T): Promise<T> {
  if (source() !== "mongodb") return fallback;
  const document = await (await getMongoDatabase()).collection(SITE_CONTENT_COLLECTION_NAME).findOne({ key }, { projection: { content: 1 } });
  ...
  if (!result.ok) { console.error("공식 콘텐츠 문서 검증 실패", { siteContentKey: key, errorName: "InvalidSiteContentDocument" }); return fallback; }
