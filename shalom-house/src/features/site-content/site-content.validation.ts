...
function text(value: unknown, path: string, errors: Record<string, string>, min: number, max: number, allowEmpty = false): string {
  if (typeof value !== "string") { errors[path] = "문자열로 입력해 주세요."; return ""; }
  const trimmed = value.trim();
  if ((!allowEmpty && trimmed.length < min) || trimmed.length > max || controlCharacters.test(trimmed)) {
    errors[path] = allowEmpty ? `${max}자 이하의 제어문자 없는 텍스트를 입력해 주세요.` : `${min}~${max}자의 제어문자 없는 텍스트를 입력해 주세요.`;
  }
  return trimmed;
}

function objectArray<T>(value: unknown, path: string, count: number, keys: readonly string[], parse: (item: Record<string, unknown>, path: string) => T, errors: Record<string, string>): T[] {
  if (!Array.isArray(value) || value.length !== count) { errors[path] = `${count}개 항목이 필요합니다.`; return []; }
  return value.map((item, index) => {
    if (!exactObject(item, keys)) { errors[`${path}.${index}`] = "허용되지 않은 항목 구조입니다."; return parse({}, `${path}.${index}`); }
...
export function validateFacilityOverviewInput(value: unknown): SiteContentValidationResult<FacilityOverviewContent> {
  ...
  const keys = ["pageDescription", "facts", "principlesEyebrow", "principlesTitle", "principlesDescription", "principles", "scenesEyebrow", "scenesTitle", "scenesDescription", "scenes", "policyEyebrow", "policyTitle", "policyItems"];
  if (!exactObject(value, keys)) return { ok: false, fieldErrors: { content: "허용되지 않은 콘텐츠 구조입니다." } };
  const fact = (item: Record<string, unknown>, path: string) => ({ label: text(item.label, `${path}.label`, errors, 1, 40), value: text(item.value, `${path}.value`, errors, 1, 200) });
  const principle = (item: Record<string, unknown>, path: string) => ({ title: text(item.title, `${path}.title`, errors, 1, 100), description: text(item.description, `${path}.description`, errors, 1, 400) });
...
export function validateGreetingInput(value: unknown): SiteContentValidationResult<GreetingContent> {
  const errors: Record<string, string> = {};
  const keys = ["pageDescription", "notice", "statusLabel", "title", "paragraphs", "signerRole", "signerName", "showSignerName"];
  if (!exactObject(value, keys)) return { ok: false, fieldErrors: { content: "허용되지 않은 콘텐츠 구조입니다." } };
  const paragraphs = Array.isArray(value.paragraphs) && value.paragraphs.length >= 1 && value.paragraphs.length <= 8
  ...
  if (value.showSignerName === true && signerName.length === 0) errors.signerName = "이름을 공개하려면 서명 이름을 입력해 주세요.";
  const content: GreetingContent = { pageDescription: text(value.pageDescription, "pageDescription", errors, 10, 400), notice: text(value.notice, "notice", errors, 1, 400), statusLabel: text(value.statusLabel, "statusLabel", errors, 1, 60), title: text(value.title, "title", errors, 1, 160), paragraphs, signerRole: text(value.signerRole, "signerRole", errors, 0, 80, true), signerName, showSignerName: value.showSignerName === true };
  return Object.keys(errors).length ? { ok: false, fieldErrors: errors } : { ok: true, value: content };
}

export function validateSiteContentSaveInput(key: SiteContentKey, value: unknown): SiteContentValidationResult<{ expectedUpdatedAt: Date | null; content: FacilityOverviewContent | GreetingContent }> {
  if (!exactObject(value, ["expectedUpdatedAt", "saveConfirmed", "content"])) return { ok: false, fieldErrors: { form: "허용되지 않은 요청 구조입니다." } };
  if (value.saveConfirmed !== true) return { ok: false, fieldErrors: { saveConfirmed: "공개 즉시 반영을 확인해 주세요." } };
  let expectedUpdatedAt: Date | null = null;
  if (value.expectedUpdatedAt !== null) {
    if (typeof value.expectedUpdatedAt !== "string") return { ok: false, fieldErrors: {}, formError: "invalid_version" };
    expectedUpdatedAt = new Date(value.expectedUpdatedAt);
    if (Number.isNaN(expectedUpdatedAt.getTime()) || expectedUpdatedAt.toISOString() !== value.expectedUpdatedAt) return { ok: false, fieldErrors: {}, formError: "invalid_version" };
  }
  const validated = key === "facility-overview" ? validateFacilityOverviewInput(value.content) : validateGreetingInput(value.content);
