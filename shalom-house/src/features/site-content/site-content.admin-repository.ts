...
export type AdminSiteContentDetail =
  | { key: "facility-overview"; persisted: boolean; content: FacilityOverviewContent; updatedAt: string | null }
  | { key: "greeting"; persisted: boolean; content: GreetingContent; updatedAt: string | null };
export type SaveAdminSiteContentResult = { ok: true; key: SiteContentKey; created: boolean; updatedAt: string } | { ok: false; reason: "not_found" | "edit_conflict" | "invalid_document" };
...
export async function getAdminSiteContent(key: SiteContentKey): Promise<AdminSiteContentDetail> {
  ...
  if (!document) return { key, persisted: false, content: fallback, updatedAt: null } as AdminSiteContentDetail;
  if (!validDate(document.updatedAt)) throw new Error("공식 콘텐츠 수정 시각이 유효하지 않습니다.");
  const result = key === "facility-overview" ? validateFacilityOverviewInput(document.content) : validateGreetingInput(document.content);
  if (!result.ok) throw new Error("공식 콘텐츠 문서가 유효하지 않습니다.");
  return { key, persisted: true, content: result.value, updatedAt: document.updatedAt.toISOString() } as AdminSiteContentDetail;
}
...
export async function saveAdminSiteContent(input: { key: SiteContentKey; content: FacilityOverviewContent | GreetingContent; expectedUpdatedAt: Date | null; actor: AdminPrincipal; now?: Date }): Promise<SaveAdminSiteContentResult> {
  ...
  try {
    return await transaction(async (database, session) => {
      ...
      if (existing) {
        ...
        const validation = input.key === "facility-overview" ? validateFacilityOverviewInput(existing.content) : validateGreetingInput(existing.content);
      ...
      return { ok: true, key: input.key, created: !existing, updatedAt: transitionAt.toISOString() };
    });
  } catch (error) {
