...
const adminAuditFieldLabels: Record<string, string> = { slug: "주소 식별자", category: "분류", title: "제목", summary: "요약", body: "본문", deletedAt: "삭제일", publicationStatus: "게시 상태", approvalStatus: "승인 상태", publishedAt: "게시일" };

type AdminAuditProjection = {
  _id?: unknown; action?: unknown; actor?: { displayName?: unknown };
  occurredAt?: unknown; changedFields?: unknown; newsPostId?: unknown;
};

export async function listAdminNewsAuditHistory(input: { contentId: string; limit?: number }): Promise<AdminAuditHistoryItem[]> {
  ...
  const documents = await (await getMongoDatabase())
    ...
    .find({ newsPostId: contentId }, { projection: { _id: 1, action: 1, "actor.displayName": 1, occurredAt: 1, changedFields: 1, newsPostId: 1 } })
  ...
  return documents.flatMap((event) => {
    const valid = event._id instanceof ObjectId && event.occurredAt instanceof Date && !Number.isNaN(event.occurredAt.getTime())
      && typeof event.action === "string" && newsAuditActions.includes(event.action as never)
    ...
    if (!valid) {
      console.error("관리자 감사 이벤트 검증에 실패했습니다.", { auditEventId: event._id instanceof ObjectId ? event._id.toHexString() : "unknown", domain: "news", errorName: "InvalidAuditEvent" });
      return [];
    }
