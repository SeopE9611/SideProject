...
export type CreatePublicInquiryResult =
  { ok: true; reference: string } | { ok: false; reason: "rate_limited" | "unavailable" };
const alphabet = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";
export function createInquiryReference(): string {
  const bytes = randomBytes(12);
  return `INQ-${Array.from(bytes, (b) => alphabet[b % alphabet.length]).join("")}`;
}
async function incrementInquirySubmissionLimit(
  db: Db,
  input: { keyHash: string; windowStartedAt: Date; expiresAt: Date },
): Promise<number> {
  const filter = { keyHash: input.keyHash, windowStartedAt: input.windowStartedAt };
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      const limit = await db
        .collection("inquiry_submission_limits")
        .findOneAndUpdate(
          filter,
          { $inc: { count: 1 }, $setOnInsert: { expiresAt: input.expiresAt } },
          { upsert: attempt === 0, returnDocument: "after" },
        );
...
export async function createPublicInquiry(input: {
  ...
  try {
    ...
    const count = await incrementInquirySubmissionLimit(db, {
      keyHash,
      windowStartedAt,
      expiresAt: new Date(windowStartedAt.getTime() + 45 * 60 * 1000),
    });
      ...
      try {
        await db
          ...
          .insertOne({
            ...
            privacyConsentVersion: inquiryPrivacyConsentVersion,
