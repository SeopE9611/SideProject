...
export const inquiryKindLabels = {
  ...
  volunteer: "자원봉사 문의",
  donation: "후원 문의",
  donation_receipt: "후원금 영수증·내역 문의",
} satisfies Record<InquiryKind, string>;
...
export type InquiryDocument = {
  ...
  email: string;
  message: string;
  privacyConsentVersion: typeof inquiryPrivacyConsentVersion;
...
export function isInquiryKind(value: unknown): value is InquiryKind {
  return typeof value === "string" && inquiryKinds.some((candidate) => candidate === value);
}
export function isInquiryStatus(value: unknown): value is InquiryStatus {
  return typeof value === "string" && inquiryStatuses.some((candidate) => candidate === value);
}
export function isValidInquiryDate(value: unknown): value is Date {
  return value instanceof Date && !Number.isNaN(value.getTime());
}
export function isInquiryReference(value: unknown): value is string {
  return typeof value === "string" && /^INQ-[A-Z0-9]{12}$/.test(value);
}
