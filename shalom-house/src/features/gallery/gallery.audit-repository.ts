...
export async function insertGalleryAuditEvent(input: {
  ...
  await input.database.collection<Event>(GALLERY_AUDIT_COLLECTION_NAME).insertOne(
    {
      _id: new ObjectId(),
      schemaVersion: 1,
...
const adminAuditFieldLabels: Record<string, string> = {
  ...
  altText: "대체 텍스트",
  activityDate: "활동일",
  subjectPresence: "인물 포함 여부",
  consentStatus: "공개 동의 상태",
  consentCheckedOn: "동의 확인일",
  displayStartOn: "표시 시작일",
  displayEndOn: "표시 종료일",
  media: "이미지",
  publicationStatus: "게시 상태",
  approvalStatus: "승인 상태",
  publishedAt: "게시일",
  consentWithdrawnAt: "동의 철회일",
  archivedAt: "보관일",
  deletedAt: "삭제일",
};
...
export async function listAdminGalleryAuditHistory(input: {
  contentId: string;
  limit?: number;
  ...
  return documents.flatMap((event) => {
    const valid =
      ...
      galleryAuditActions.includes(event.action as never) &&
