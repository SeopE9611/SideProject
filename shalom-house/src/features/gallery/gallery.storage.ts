import "server-only";
import {
  getGalleryPrivateBucketName,
  getSupabaseAdminClient,
} from "@/lib/supabase-admin";

const privateGalleryObjectPathPattern =
  /^shalom-house\/gallery\/[a-f0-9]{24}\/image\.webp$/;

function assertPrivateGalleryStorageLocation(
  bucket: string,
  objectPath: string,
): void {
  if (
    bucket !== getGalleryPrivateBucketName() ||
    !privateGalleryObjectPathPattern.test(objectPath)
  ) {
...
export async function downloadPrivateGalleryImage(
  bucket: string,
  objectPath: string,
): Promise<Blob> {
  assertPrivateGalleryStorageLocation(bucket, objectPath);
...
export async function removePrivateGalleryImage(
  bucket: string,
  objectPath: string,
): Promise<void> {
  assertPrivateGalleryStorageLocation(bucket, objectPath);
