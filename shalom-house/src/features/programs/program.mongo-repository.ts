...
function summary(d: WithId<Document>): PublicProgramSummary | null {
  if (
    !d._id ||
    !isValidProgramSlug(d.slug) ||
    !text(d.category) ||
    !text(d.title) ||
    !text(d.summary) ||
    !text(d.purpose) ||
    !(d.operationStatusLabel === null || text(d.operationStatusLabel)) ||
    !Number.isInteger(d.sortOrder) ||
    d.sortOrder < 0 ||
    d.sortOrder > 9999 ||
    !validDate(d.publishedAt) ||
  ...
  return {
    ...
    summary: d.summary,
    purpose: d.purpose,
    operationStatusLabel: d.operationStatusLabel,
    sortOrder: d.sortOrder,
    publishedAt: d.publishedAt.toISOString(),
    updatedAt: d.updatedAt.toISOString(),
  };
}
function detail(d: WithId<Document>): PublicProgram | null {
  const value = summary(d);
  return value && Array.isArray(d.body) && d.body.length > 0 && d.body.every(text) ? { ...value, body: d.body } : null;
}
function publicFilter(now: Date): Filter<MongoProgramDocument> {
  return {
...
export class MongoProgramRepository implements ProgramRepository {
  async listPublished(options?: { limit?: number }): Promise<readonly PublicProgramSummary[]> {
    const docs = await (
      ...
      .find(publicFilter(new Date()), { projection })
      .sort({ sortOrder: 1, publishedAt: -1, _id: -1 })
      .limit(normalizePublicProgramLimit(options?.limit))
      .toArray();
    return docs.flatMap((d) => {
      const p = summary(d);
      if (!p)
        console.error("공개 프로그램 문서 검증에 실패했습니다.", {
  ...
  async findPublishedBySlug(slug: string): Promise<PublicProgram | null> {
    ...
    const d = await (
      ...
      .collection<MongoProgramDocument>(PROGRAM_COLLECTION_NAME)
      .findOne({ ...publicFilter(new Date()), slug }, { projection: { ...projection, body: 1 } });
