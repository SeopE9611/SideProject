...
export const programAuditActions = [
  "draft_created",
  "draft_updated",
  "review_requested",
  "review_approved",
...
export const programAuditChangedFields = [
  "slug",
  "category",
  "title",
  "summary",
...
export function createProgramAuditSnapshot(document: MongoProgramDocument): ProgramAuditSnapshot {
  return {
    slug: document.slug,
...
export function getDraftChangedFields(
  ...
  after: ValidatedAdminProgramDraft,
): ProgramAuditChangedField[] {
  const fields: ProgramAuditChangedField[] = [];
  for (const key of ["slug", "category", "title", "summary", "purpose", "operationStatusLabel", "sortOrder"] as const)
    if (before[key] !== after[key]) fields.push(key);
  if (before.body.length !== after.body.length || before.body.some((p, i) => p !== after.body[i])) fields.push("body");
  return fields;
}
