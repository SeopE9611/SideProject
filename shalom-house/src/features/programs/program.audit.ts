...
export const programAuditActions = ["draft_created", "draft_updated", "review_requested", "review_approved", "review_rejected", "published", "unpublished", "archived"] as const;
export type ProgramAuditAction = (typeof programAuditActions)[number];
export const programAuditChangedFields = ["slug", "category", "title", "summary", "purpose", "body", "operationStatusLabel", "sortOrder", "publicationStatus", "approvalStatus", "publishedAt"] as const;
export type ProgramAuditChangedField = (typeof programAuditChangedFields)[number];
export type ProgramAuditSnapshot = { slug: string; category: string; title: string; summary: string; purpose: string; body: string[]; operationStatusLabel: string | null; sortOrder: number; publicationStatus: ProgramPublicationStatus; approvalStatus: ProgramApprovalStatus; publishedAt: Date | null };
