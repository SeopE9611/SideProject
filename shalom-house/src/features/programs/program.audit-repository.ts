...
import {
  programAuditActions,
  programAuditChangedFields,
  type ProgramAuditAction,
  type ProgramAuditActor,
...
export type MongoProgramAuditEventDocument = {
  ...
  actor: ProgramAuditActor;
  occurredAt: Date;
  fromVersionAt: Date | null;
  toVersionAt: Date;
  before: ProgramAuditSnapshot | null;
  after: ProgramAuditSnapshot;
...
function actor(value: AdminPrincipal): ProgramAuditActor {
  if (
    !ObjectId.isValid(value.id) ||
    new ObjectId(value.id).toHexString() !== value.id.toLowerCase() ||
    !value.displayName.trim() ||
    !isAdminRole(value.role)
  )
    throw new Error("감사 이벤트 관리자 정보가 유효하지 않습니다.");
  return {
    adminId: new ObjectId(value.id),
    displayName: value.displayName,
    role: value.role,
  };
}
export async function insertProgramAuditEvent(input: {
  database: Db;
  ...
  before: ProgramAuditSnapshot | null;
  after: ProgramAuditSnapshot;
  changedFields: readonly ProgramAuditChangedField[];
}): Promise<void> {
  ...
  await input.database.collection<MongoProgramAuditEventDocument>(PROGRAM_AUDIT_COLLECTION_NAME).insertOne(
    {
      _id: input.eventId,
      schemaVersion: 1,
