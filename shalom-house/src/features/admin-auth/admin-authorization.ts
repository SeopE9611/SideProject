...
const permissionsByRole: Record<AdminRole, readonly AdminPermission[]> = {
  ...
  editor: ["admin.read", "content.create", "content.update", "content.archive", "content.request_review"],
...
export function hasAdminRolePermission(role: AdminRole, permission: AdminPermission): boolean {
  return permissionsByRole[role].some((candidate) => candidate === permission);
}

export function hasAdminPermission(admin: Pick<AdminPrincipal, "role">, permission: AdminPermission): boolean {
  return hasAdminRolePermission(admin.role, permission);
}

export type AdminAuthorizationResult =
  { ok: true; admin: AdminPrincipal } | { ok: false; reason: "unauthorized" | "forbidden" };

export async function authorizeCurrentAdmin(permission: AdminPermission): Promise<AdminAuthorizationResult> {
