...
import {
  facilitySpaceAuditActions,
  facilitySpaceAuditChangedFields,
  type FacilitySpaceAuditAction,
  type FacilitySpaceAuditActor,
  type FacilitySpaceAuditChangedField,
  type FacilitySpaceAuditSnapshot,
...
export type FacilitySpaceAuditEventDocument = {
  ...
  action: FacilitySpaceAuditAction;
  actor: FacilitySpaceAuditActor;
  occurredAt: Date;
  fromVersionAt: Date | null;
  toVersionAt: Date;
  before: FacilitySpaceAuditSnapshot | null;
  after: FacilitySpaceAuditSnapshot;
  changedFields: FacilitySpaceAuditChangedField[];
};
export async function insertFacilitySpaceAuditEvent(input: {
  database: Db;
  ...
  before: FacilitySpaceAuditSnapshot | null;
  after: FacilitySpaceAuditSnapshot;
  changedFields: readonly FacilitySpaceAuditChangedField[];
}) {
  ...
  await input.database
    .collection<FacilitySpaceAuditEventDocument>(FACILITY_SPACE_AUDIT_COLLECTION_NAME)
    .insertOne(document, { session: input.session });
}
const actionLabels = { created: "생활공간 생성", updated: "생활공간 수정" };
const fieldLabels: Record<FacilitySpaceAuditChangedField, string> = {
  title: "공간명",
  description: "공간 설명",
...
export async function listAdminFacilitySpaceAuditHistory(facilitySpaceId: string): Promise<AdminAuditHistoryItem[]> {
  ...
  const documents = await (
    ...
    .collection<FacilitySpaceAuditEventDocument>(FACILITY_SPACE_AUDIT_COLLECTION_NAME)
    .find(
      { facilitySpaceId: new ObjectId(facilitySpaceId) },
      { projection: { action: 1, "actor.displayName": 1, occurredAt: 1, changedFields: 1 } },
    )
  ...
  return documents.flatMap((event) =>
    ...
    isValidFacilitySpaceDate(event.occurredAt) &&
    facilitySpaceAuditActions.includes(event.action) &&
    typeof event.actor?.displayName === "string" &&
    event.actor.displayName.trim() !== "" &&
    Array.isArray(event.changedFields) &&
    event.changedFields.every((field) => facilitySpaceAuditChangedFields.includes(field))
      ? [
          {
