...
function HeroAction({ action, primary = false }: { action: PageHeroAction; primary?: boolean }) {
  ...
  if (action.external || action.href.startsWith("tel:") || action.href.startsWith("#")) {
    return (
      <a
        className={className}
        href={action.href}
    ...
    <section className="overflow-hidden border-b border-border bg-home-cream">
      <div className="animate-page-enter mx-auto grid w-full max-w-site px-page py-14 sm:px-page-wide sm:py-20 lg:grid-cols-[minmax(0,1.25fr)_minmax(20rem,0.75fr)] lg:items-stretch lg:gap-14 lg:py-24">
        <div className="flex flex-col justify-between border-t-4 border-primary pt-7 sm:pt-9">
          <div>
            <p className="text-small font-bold tracking-[0.08em] text-accent">{eyebrow}</p>
        ...
        <aside className="mt-12 bg-home-ink px-7 py-8 text-hero-on-dark sm:px-9 sm:py-10 lg:mt-0">
          <p className="text-small font-bold tracking-[0.08em] text-sun-soft">{asideTitle}</p>
          <dl className="mt-8 border-t border-hero-on-dark/25">
            {items.map((item) => (
              ...
              >
                <dt className="text-small font-bold text-sun-soft">{item.label}</dt>
                <dd className="text-safe-wrap text-pretty text-body font-semibold text-hero-on-dark">{item.value}</dd>
              </div>
            ))}
          </dl>
        </aside>
