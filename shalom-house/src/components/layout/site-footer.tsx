        ...
        <div className="grid gap-8 border-b border-white/20 pb-8 md:grid-cols-[1.1fr_0.8fr_0.9fr] md:gap-10">
          <section aria-labelledby="footer-about-heading">
            <h2 id="footer-about-heading">
              <Link
                className="text-safe-wrap inline-flex min-h-11 items-center text-heading font-bold text-white underline decoration-white/60 underline-offset-4 transition-colors hover:text-white/80 focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-white"
                href="/"
              >
                {siteConfig.name}
              </Link>
          ...
          <nav aria-label="푸터 이용 안내">
            <h2 className="text-small font-bold text-white">이용 안내</h2>
            <ul className="mt-4 grid grid-cols-2 gap-x-6">
              {siteConfig.footerNavigation.map((item) => (
                <li key={item.href}>
                  <Link
                    className="text-safe-wrap inline-flex min-h-10 items-center text-small font-semibold text-white/75 underline decoration-white/40 underline-offset-4 transition-colors hover:text-white focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-white"
                    href={item.href}
                  >
                    {item.label}
                  </Link>
          ...
          <section aria-labelledby="footer-contact-heading">
            <h2 id="footer-contact-heading" className="text-small font-bold text-white">
              연락처
            </h2>
            <address className="mt-4 space-y-2 text-small not-italic text-white/75">
              <p>
