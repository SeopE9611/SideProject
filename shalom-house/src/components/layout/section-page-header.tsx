...
export function SectionPageHeader({ eyebrow, title, description, breadcrumbs, notice }: SectionPageHeaderProps) {
  return (
    <header className="border-b border-border bg-surface">
      <div className="mx-auto max-w-site px-page py-10 sm:px-page-wide sm:py-14">
          ...
          <ol className="flex flex-wrap gap-x-3 gap-y-1 text-small">
            {breadcrumbs.map((item, index) => (
              <li key={`${item.label}-${index}`} className="text-safe-wrap">
                {item.href ? <Link className="text-primary underline underline-offset-4" href={item.href}>{item.label}</Link> : <span aria-current="page">{item.label}</span>}
              </li>
            ))}
          </ol>
        </nav>
        ...
        {notice ? <p className="text-safe-wrap mt-6 border-l-4 border-primary bg-primary-soft px-4 py-3 text-small">{notice}</p> : null}
      </div>
    </header>
  );
}
