...
function describedBy(
  ...
  return [helpId, errors[field] ? fieldErrorIds[field] : undefined]
    .filter(Boolean)
    .join(" ") || undefined;
}
  ...
  return (
    ...
    >
      ...
      <div className="grid gap-2">
        <label htmlFor="admin-news-category" className="font-semibold">분류</label>
        <select
      ...
      <div className="grid gap-2">
        <div className="flex items-start gap-3 rounded-control border border-border bg-surface p-4">
          <input id="admin-news-content-safety" name="contentSafetyConfirmed" type="checkbox" required aria-invalid={fieldErrors.contentSafetyConfirmed ? true : undefined} aria-describedby={describedBy("contentSafetyConfirmed", fieldErrors)} className="mt-1 size-5 shrink-0 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-focus-ring" />
          <label htmlFor="admin-news-content-safety" className="text-small leading-relaxed">거주인·직원·보호자 등의 개인정보, 민감정보와 공개 금지 정보가 포함되지 않았으며 홈페이지 검토용으로 저장해도 되는 내용임을 확인했습니다.</label>
        </div>
