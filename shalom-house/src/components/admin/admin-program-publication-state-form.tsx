...
import {
  isAdminProgramPublicationAction,
  validateAdminProgramPublicationStateInput,
  type AdminProgramPublicationStateFieldErrors,
} from "@/features/programs/program.admin-validation";

export type AdminProgramPublicationStateFormProps = {
  postId: string;
...
export function AdminProgramPublicationStateForm(
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const validation = validateAdminProgramPublicationStateInput(input);
    setFormError(null);
    if (!validation.ok) {
    ...
    try {
      ...
      if (
        ...
        isAdminProgramPublicationAction(body.action) &&
        typeof body.redirectTo === "string"
      ) {
        const fallback = `/admin/programs/${props.postId}?publication=${body.action === "unpublish" ? "unpublished" : "archived"}`;
        router.push(body.redirectTo.startsWith("/admin/programs/") ? body.redirectTo : fallback);
        router.refresh();
        return;
      }
      ...
      if (response.status === 400 && error === "validation" && isRecord(body) && isRecord(body.fieldErrors)) {
        ...
        setFormError("상태를 변경할 프로그램을 찾을 수 없습니다.");
