...
export function AdminNewsPublicationStateForm(props: AdminNewsPublicationStateFormProps) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch(`/api/admin/news/${encodeURIComponent(props.postId)}/publication`, {
        method: "POST",
        headers: {
  ...
  return (
    <form onSubmit={handleSubmit} aria-busy={isSubmitting ? true : undefined} className="mt-5 max-w-3xl space-y-5">
      ...
      <fieldset
        aria-describedby={fieldErrors.action ? "admin-news-publication-action-error" : undefined}
        className="space-y-4"
      >
        <legend className="font-bold">변경할 상태 선택</legend>
        <div className="flex items-start gap-3">
          <input
            id="admin-news-publication-action-unpublish"
            name="action"
            value="unpublish"
            type="radio"
          ...
          />
          <label htmlFor="admin-news-publication-action-unpublish">
        ...
        <div className="flex items-start gap-3">
          <input
            id="admin-news-publication-action-archive"
            name="action"
            value="archive"
            type="radio"
            className="mt-1 size-5 shrink-0 accent-primary focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-focus-ring"
          />
          <label htmlFor="admin-news-publication-action-archive">
        ...
        {fieldErrors.action ? (
          <p id="admin-news-publication-action-error" className="text-small font-semibold text-danger">
            {fieldErrors.action}
          </p>
        ) : null}
      </fieldset>
      <div className="space-y-2">
        <div className="flex items-start gap-3">
          <input
            id="admin-news-publication-transition-confirmed"
            name="transitionConfirmed"
            type="checkbox"
            aria-invalid={fieldErrors.transitionConfirmed ? true : undefined}
            aria-describedby={
              fieldErrors.transitionConfirmed ? "admin-news-publication-transition-confirmed-error" : undefined
            }
            className="mt-1 size-5 shrink-0 accent-primary focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-focus-ring"
          />
          <label htmlFor="admin-news-publication-transition-confirmed" className="font-semibold">
            선택한 상태 변경과 공개 종료 결과를 확인했습니다.
          </label>
        </div>
        {fieldErrors.transitionConfirmed ? (
          <p id="admin-news-publication-transition-confirmed-error" className="text-small font-semibold text-danger">
            {fieldErrors.transitionConfirmed}
          </p>
        ) : null}
      </div>
