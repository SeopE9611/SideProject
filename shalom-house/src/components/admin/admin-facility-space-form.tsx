  ...
  return (
    <form className="space-y-6 rounded-card border p-5" onSubmit={submit} aria-busy={busy}>
      ...
      <div>
        ...
        <p id="description-help" className="mt-1 text-small text-muted-foreground">
      ...
      <div>
        <label className="flex gap-3">
          <input
            ...
            aria-describedby={errors.saveConfirmed ? "saveConfirmed-error" : undefined}
          />
          <span>입력한 공간 설명이 개인정보와 시설 보안상 공개 가능한 내용임을 확인했습니다.</span>
        </label>
        {errors.saveConfirmed ? (
          <p id="saveConfirmed-error" role="alert" className="mt-1 text-small text-destructive">
            {errors.saveConfirmed}
          </p>
        ) : null}
      </div>
