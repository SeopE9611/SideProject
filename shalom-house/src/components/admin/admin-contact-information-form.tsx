...
export function AdminContactInformationForm({ initialContent, expectedUpdatedAt }: AdminContactInformationFormProps) {
  ...
  return <form onSubmit={submit} aria-busy={busy} className="space-y-6">
    ...
    {fields.map(({ key, label, multiline }) => {
      ...
      const props = { id, value: content[key], onChange: (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => updateTextField(key, e.target.value), "aria-describedby": described, "aria-invalid": error ? true as const : undefined, className: "mt-2 w-full rounded-control border px-3 py-2" };
    ...
    <label className="flex gap-3"><input id={instagramDisclosureId} type="checkbox" checked={content.showInstagram} onChange={(e) => { setContent((v) => ({ ...v, showInstagram: e.target.checked })); setConfirmed(false); }} aria-invalid={errors.showInstagram ? true : undefined} aria-describedby={errors.showInstagram ? instagramDisclosureErrorId : undefined} /><span>인스타그램 공개</span></label>
    {errors.showInstagram ? <p id={instagramDisclosureErrorId} role="alert" className="text-small text-danger">{errors.showInstagram}</p> : null}
