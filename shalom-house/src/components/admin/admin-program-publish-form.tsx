...
import {
  validateAdminProgramPublishInput,
  type AdminProgramPublishFieldErrors,
} from "@/features/programs/program.admin-validation";

export type AdminProgramPublishFormProps = {
  postId: string;
  expectedUpdatedAt: string;
};

const checkboxId = "admin-program-publication-confirmed";
const checkboxErrorId = "admin-program-publication-confirmed-error";
...
export function AdminProgramPublishForm(props: AdminProgramPublishFormProps) {
  ...
  const [fieldErrors, setFieldErrors] = useState<AdminProgramPublishFieldErrors>({});
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (response.status === 400 && error === "validation") {
        ...
        setFormError("현재 프로그램을 공개할 수 없습니다. 잠시 후 다시 시도해 주세요.");
      }
    } catch {
