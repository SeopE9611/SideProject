...
export function AdminTransparencyArchiveForm({ id, expectedUpdatedAt }: { id: string; expectedUpdatedAt: string }) {
  ...
  return (
    <form onSubmit={submit} aria-busy={busy} className="space-y-3 rounded-card border border-danger p-5">
      <h2 className="font-bold">초안 보관</h2>
      <p>보관하면 수정할 수 없습니다. Supabase Storage 원본은 유지됩니다.</p>
      ...
      <p id="archive-help">영구 삭제가 아니며 PDF는 삭제되지 않습니다.</p>
      {error ? (
        <p id="archive-error" role="alert">
          {error}
        </p>
      ) : null}
      <button disabled={busy} className="min-h-11 border px-4">
        {busy ? "보관 중…" : "초안 보관"}
      </button>
    </form>
  );
