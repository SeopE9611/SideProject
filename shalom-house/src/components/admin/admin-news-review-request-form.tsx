...
import {
  validateAdminNewsReviewRequestInput,
  type AdminNewsReviewRequestFieldErrors,
} from "@/features/news/news.admin-validation";

export type AdminNewsReviewRequestFormProps = {
  postId: string;
  expectedUpdatedAt: string;
};

const checkboxId = "admin-news-review-readiness";
const checkboxErrorId = "admin-news-review-readiness-error";
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (response.status === 400 && error === "validation") {
        ...
        setFormError("검토를 요청할 게시물을 찾을 수 없습니다.");
      } else if (response.status === 409 && error === "edit_conflict") {
        setFormError("다른 관리자가 이 게시물을 먼저 수정했습니다. 상세 화면을 새로 확인해 주세요.");
