            ...
            <li key={item.id} className="min-w-0 border-t border-border pt-4 first:border-t-0 first:pt-0">
              ...
              <dl className="mt-2 grid min-w-0 gap-2 sm:grid-cols-2">
                <div className="min-w-0">
                  <dt className="text-small font-semibold text-muted-foreground">작업 관리자</dt>
                  <dd className="text-safe-wrap break-words">{item.actorDisplayName}</dd>
                </div>
                <div>
                  <dt className="text-small font-semibold text-muted-foreground">변경 시각</dt>
                  <dd>
              ...
              <div className="mt-3">
                <p className="text-small font-semibold text-muted-foreground">변경 필드</p>
                <ul className="mt-1 flex min-w-0 flex-wrap gap-2">
                  {(item.changedFieldLabels.length ? item.changedFieldLabels : ["상태 기록"]).map((label) => (
                    <li
                      key={label}
                      className="text-safe-wrap rounded-control border border-border px-2 py-1 text-small"
                    >
                      {label}
                    </li>
                  ))}
