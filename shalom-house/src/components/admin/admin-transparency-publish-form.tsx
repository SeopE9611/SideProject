...
export function AdminTransparencyPublishForm({ id, expectedUpdatedAt }: { id: string; expectedUpdatedAt: string }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/transparency/${encodeURIComponent(id)}/publish`, { method: "POST", headers: { "Content-Type": "application/json", Accept: "application/json" }, credentials: "same-origin", body: JSON.stringify({ expectedUpdatedAt,  publishConfirmed: confirmed }) });
  ...
  return <form onSubmit={submit} aria-busy={isBusy} className="space-y-3 rounded-card border p-5"><h2 className="font-bold">게시</h2><p>게시하면 자료공개 페이지에서 PDF를 열 수 있습니다.</p><label htmlFor="transparency-publish-confirmed" className="flex gap-2"><input id="transparency-publish-confirmed" name="publishConfirmed" type="checkbox" required />게시 확인</label>{error ? <p role="alert" className="text-danger">{error}</p> : null}<button type="submit" disabled={isBusy} className="min-h-11 border px-4 disabled:opacity-60">{isBusy ? "게시 중…" : "게시"}</button></form>;
}
