    ...
    <div className="space-y-8">
      ...
      {wasPublished ? (
        <div role="status" className="rounded-control border border-border-strong bg-surface p-4">
          ...
          <p className="mt-2 text-small text-muted-foreground">공개 프로그램 목록과 상세 페이지에서 확인할 수 있습니다.</p>
        </div>
      ) : null}
      ...
      {publication === "unpublished" ? (
        <div role="status" className="rounded-control border border-border-strong bg-surface p-4">
          ...
          <p className="mt-2 text-small text-muted-foreground">공개 프로그램 목록과 상세 페이지에서 더 이상 표시되지 않습니다.</p>
        </div>
      ) : null}
      ...
      <section aria-labelledby="admin-program-status-heading" className="rounded-card border border-border bg-surface p-5">
        <h2 id="admin-program-status-heading" className="text-heading font-bold">상태 정보</h2>
      ...
      <section aria-labelledby="admin-program-content-heading" className="space-y-6">
        <h2 id="admin-program-content-heading" className="text-heading font-bold">내용</h2>
        <div><h3 className="font-bold">목적</h3><p className="mt-2 whitespace-pre-wrap break-words">{post.purpose}</p></div>
        <div><h3 className="font-bold">운영 상태</h3><p className="mt-2">{post.operationStatusLabel ?? "미입력"}</p></div>
        <div><h3 className="font-bold">정렬 순서</h3><p className="mt-2">{post.sortOrder}</p></div>
      ...
      {post.canDirectPublish && canDirectPublish ? (
        <section aria-labelledby="admin-program-direct-publish-heading" className="rounded-card border border-border-strong bg-surface p-5">
          <h2 id="admin-program-direct-publish-heading" className="text-heading font-bold">바로 게시</h2>
          <p className="mt-3 text-small text-muted-foreground">
            시스템 관리자는 별도의 검토·승인 단계를 거치지 않고 현재 프로그램을 즉시 공개할 수 있습니다.<br />
            역할을 나눠 검토하려면 아래의 검토 요청 절차를 사용해 주세요.
          </p>
          <AdminDirectPublishForm id={post.id} endpoint={`/api/admin/programs/${post.id}/direct-publish`} expectedUpdatedAt={post.updatedAt} contentLabel="프로그램" />
        </section>
      ) : null}

      {post.canRequestReview && canRequestReview ? (
        <section aria-labelledby="admin-program-review-heading" className="rounded-card border border-border-strong bg-surface p-5">
          <h2 id="admin-program-review-heading" className="text-heading font-bold">{isRejectedDraft ? "재검토 요청" : "검토 요청"}</h2>
          {isRejectedDraft ? (
            <p className="mt-3 text-small text-muted-foreground">
            ...
            <p className="mt-3 text-small text-muted-foreground">
              ...
              이 작업만으로 프로그램이 승인되거나 공개되지는 않습니다.
            </p>
          )}
          <AdminProgramReviewRequestForm
            postId={post.id}
      ...
      {post.canDecideReview && canDecideReview ? (
        <section aria-labelledby="admin-program-decision-heading" className="rounded-card border border-border-strong bg-surface p-5">
          <h2 id="admin-program-decision-heading" className="text-heading font-bold">검토 결과 처리</h2>
          <p className="mt-3 text-small text-muted-foreground">
            승인은 검토 완료 상태만 기록하며 프로그램을 공개하지 않습니다.<br />
      ...
      {post.canPublish && canPublish ? (
        <section aria-labelledby="admin-program-publish-heading" className="rounded-card border border-border-strong bg-surface p-5">
          <h2 id="admin-program-publish-heading" className="text-heading font-bold">게시</h2>
          <p className="mt-3 text-small text-muted-foreground">
            게시하면 현재 승인된 내용이 즉시 공개 프로그램 목록과 상세 페이지에 표시됩니다.<br />
      ...
      {post.canManagePublicationState && canPublish ? (
        <section aria-labelledby="admin-program-publication-state-heading" className="rounded-card border border-border-strong bg-surface p-5">
          <h2 id="admin-program-publication-state-heading" className="text-heading font-bold">게시 상태 변경</h2>
          <p className="mt-3 text-small text-muted-foreground">
