  ...
  return (
    <div className="space-y-6">
      <header>
        ...
        <h1 className="mt-4 text-title font-bold">함께하는 사람들 관리</h1>
        <div className="mt-4 flex gap-5">
          <Link className="min-h-11 font-bold underline" href="/admin/site-content/people/new">
            직원 소개 등록
          </Link>
      ...
      {profiles.length ? (
            ...
            <tbody>
              {profiles.map((p) => (
                <tr className="border-t" key={p.id}>
                  ...
                  <td>
                    <Link className="underline" href={`/admin/site-content/people/${p.id}`}>
                      상세
                    </Link>
                  </td>
                  <td>
                    <Link className="underline" href={`/admin/site-content/people/${p.id}/edit`}>
                      편집
                    </Link>
                  </td>
                </tr>
        ...
        <p className="border-y py-6">등록된 직원 소개가 없습니다.</p>
      )}
    </div>
  );
}
