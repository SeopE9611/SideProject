...
export default async function Page({
  ...
  const rows = [
    ["공간명", d.title],
    ["공간 설명", d.description],
    ["공개 상태", getFacilitySpacePublicationStatusLabel(d.publicationStatus)],
  ...
  return (
    <div className="space-y-8">
      <header>
        ...
        <h1 className="mt-4 text-title font-bold">생활공간 상세</h1>
        {(await searchParams).saved === "1" ? <p role="status">저장했습니다.</p> : null}
        <Link className="mt-4 inline-block min-h-11 underline" href={`/admin/site-content/spaces/${id}/edit`}>
          편집
        </Link>
      </header>
