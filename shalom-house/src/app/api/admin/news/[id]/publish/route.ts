...
import { isValidAdminNewsId, publishAdminNews } from "@/features/news/news.admin-repository";
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    return jsonResponse({ ...result, redirectTo: `/admin/news/${result.id}?published=1` }, 200);
  } catch (error) {
    ...
    return jsonResponse(
      {
        ok: false,
        error: "unavailable",
        message: "현재 게시물을 공개할 수 없습니다. 잠시 후 다시 시도해 주세요.",
      },
      503,
    );
  }
