...
import { changeAdminNewsPublicationState, isValidAdminNewsId } from "@/features/news/news.admin-repository";
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    return jsonResponse(
      {
        ...result,
        redirectTo: `/admin/news/${result.id}?publication=${publication}`,
      },
      200,
    );
  } catch (error) {
