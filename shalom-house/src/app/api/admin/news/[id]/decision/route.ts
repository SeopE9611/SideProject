...
import { decideAdminNewsReview, isValidAdminNewsId } from "@/features/news/news.admin-repository";
import { validateAdminNewsReviewDecisionInput } from "@/features/news/news.admin-validation";
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    const authorization = await authorizeCurrentAdmin("content.decide_review");
    if (!authorization.ok) {
      return jsonResponse(
    ...
    const result = await decideAdminNewsReview({
      id,
      ...
      return jsonResponse(
        {
          ok: false,
          error: "edit_conflict",
          message: "다른 관리자가 이 게시물을 먼저 변경했습니다.",
        },
        409,
      );
    }

    const decisionQuery = result.decision === "approve" ? "approved" : "rejected";
    return jsonResponse(
      {
        ok: true,
        id: result.id,
        ...
        redirectTo: `/admin/news/${result.id}?decision=${decisionQuery}`,
      },
      200,
    );
  } catch (error) {
    console.error("관리자 뉴스 검토 결과 처리 실패", {
    ...
    return jsonResponse(
      {
        ok: false,
        error: "unavailable",
        message: "현재 검토 결과를 처리할 수 없습니다. 잠시 후 다시 시도해 주세요.",
      },
      503,
    );
  }
