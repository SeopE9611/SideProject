...
async function handle(request: Request, id: string, remove: boolean) {
  ...
  let result;
  try {
    result = removeValue ? await removeAdminNewsCoverImage({ id, expectedUpdatedAt: removeValue.expectedUpdatedAt, actor: auth.admin })
    ...
    console.error("소식 대표 이미지 저장 실패", { newsId: id, errorName: error instanceof Error ? error.name : "UnknownError" });
