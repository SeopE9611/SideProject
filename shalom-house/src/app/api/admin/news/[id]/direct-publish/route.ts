...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    const result = await directPublishAdminNewsPost({ id, expectedUpdatedAt: validation.value.expectedUpdatedAt, actor: authorization.admin });
    if (!result.ok) {
    ...
    console.error("관리자 게시물 바로 게시 처리 실패", { contentId, errorName: error instanceof Error ? error.name : "UnknownError" });
