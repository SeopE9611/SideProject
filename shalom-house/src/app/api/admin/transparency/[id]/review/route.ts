...
import { isValidAdminTransparencyDocumentId, requestAdminTransparencyReview } from "@/features/transparency/transparency.admin-repository";
import { validateAdminTransparencyReviewInput } from "@/features/transparency/transparency.admin-validation";
...
export async function POST(request: Request, { params }: Context) {
  ...
  const validation = validateAdminTransparencyReviewInput(body);
  if (!validation.ok) return json({ ok: false, error: "validation" }, 400);
  try {
    const result = await requestAdminTransparencyReview({ id, ...validation.value, actor: admin });
    if (!result.ok) return json({ ok: false, error: result.reason }, result.reason === "not_found" ? 404 : 409);
    return json({ ok: true, redirectTo: `/admin/transparency/${id}?reviewed=1` }, 200);
  } catch (error) {
