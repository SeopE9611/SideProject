...
import { isValidAdminTransparencyDocumentId, decideAdminTransparencyReview } from "@/features/transparency/transparency.admin-repository";
import { validateAdminTransparencyDecisionInput } from "@/features/transparency/transparency.admin-validation";
...
export async function POST(request: Request, { params }: Context) {
  ...
  const validation = validateAdminTransparencyDecisionInput(body);
  if (!validation.ok) return json({ ok: false, error: "validation" }, 400);
  try {
    const result = await decideAdminTransparencyReview({ id, ...validation.value, actor: admin });
    if (!result.ok) return json({ ok: false, error: result.reason }, result.reason === "not_found" ? 404 : 409);
    return json({ ok: true, redirectTo: `/admin/transparency/${id}?decided=1` }, 200);
  } catch (error) {
