...
import {
  ADMIN_TRANSPARENCY_REQUEST_MAX_BYTES,
  normalizeAdminTransparencyOriginalFileName,
  validateAdminTransparencyDraftInput,
  validateAdminTransparencyPdf,
} from "@/features/transparency/transparency.admin-validation";
...
export async function POST(request: Request) {
  ...
  if (Number.isFinite(declaredLength) && declaredLength > ADMIN_TRANSPARENCY_REQUEST_MAX_BYTES)
    return json({ ok: false, error: "payload_too_large" }, 413);
  const rawBody = await request.arrayBuffer();
  if (rawBody.byteLength > ADMIN_TRANSPARENCY_REQUEST_MAX_BYTES)
    return json({ ok: false, error: "payload_too_large" }, 413);
  let form: FormData;
  try {
      ...
      return json(
        {
          ok: false,
          error: "validation",
          fieldErrors: { document: "PDF 파일을 선택해 주세요." },
        },
        400,
      );
      ...
      return json(
        {
          ok: false,
          error: "validation",
          fieldErrors: { document: pdf.error },
        },
        400,
      );
      ...
      return json(
        {
          ok: false,
          error: "validation",
          fieldErrors: { document: "원본 파일명을 확인할 수 없습니다." },
        },
        400,
      );
    const result = await createAdminTransparencyDraft({
      draft: draft.value,
      pdf: pdf.value,
      originalFileName,
      actor: admin,
    ...
    return json(
      {
        ok: true,
        id: result.id,
        redirectTo: `/admin/transparency/${result.id}?created=1`,
      },
      201,
    );
  } catch (error) {
    console.error("관리자 자료공개 초안 저장 실패", {
