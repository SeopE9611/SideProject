...
import { createAdminStaffProfile } from "@/features/staff/staff.admin-repository";
import { validateStaffProfileSaveInput } from "@/features/staff/staff.validation";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const validation = validateStaffProfileSaveInput(json);
    ...
    const result = await createAdminStaffProfile(validation.value.profile, authorization.admin);
    if (!result.ok) return response({ ok: false, error: result.reason }, 503);
    return response(
      {
        ok: true,
        redirectTo: `/admin/site-content/people/${result.id}?saved=1`,
      },
      201,
    );
  } catch (error) {
    console.error("직원 소개 생성 실패", {
