...
import { updateAdminStaffProfile } from "@/features/staff/staff.admin-repository";
...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const result = await updateAdminStaffProfile(
      id,
    ...
    return response({ ok: true, redirectTo: `/admin/site-content/people/${id}?saved=1` }, 200);
  } catch (error) {
    console.error("직원 소개 수정 실패", {
      staffProfileId: id,
