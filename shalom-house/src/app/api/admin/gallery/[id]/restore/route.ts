...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (length !== null && Number(length) > MAX_BYTES) return json({ ok: false, error: "payload_too_large" }, 413);
    ...
    console.error("관리자 활동사진 restore 실패", {
