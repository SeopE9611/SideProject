...
import { restoreAdminGalleryItem, isValidAdminGalleryItemId } from "@/features/gallery/gallery.admin-repository";
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const result = await restoreAdminGalleryItem({ id, expectedUpdatedAt: validation.value.expectedUpdatedAt, actor: authorization.admin });
    if (!result.ok) return json({ ok: false, error: result.reason }, result.reason === "not_found" ? 404 : 409);
    return json(result, 200);
  } catch (error) { console.error("관리자 활동사진 restore 실패", { name: error instanceof Error ? error.name : "UnknownError" }); return json({ ok: false, error: "unavailable" }, 503); }
}
