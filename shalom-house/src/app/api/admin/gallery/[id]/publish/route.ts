...
import { isValidAdminGalleryItemId, publishAdminGalleryItem } from "@/features/gallery/gallery.admin-repository";
import { validateAdminGalleryPublishInput } from "@/features/gallery/gallery.admin-validation";
...
export async function POST(request: Request, { params }: Context) {
  ...
  const checked = validateAdminGalleryPublishInput(body);
  ...
  try {
    const result = await publishAdminGalleryItem({
      id,
      ...checked.value,
      actor: admin,
    });
    ...
    return json({ ...result, redirectTo: `/admin/gallery/${id}?transition=publish` }, 200);
  } catch (error) {
