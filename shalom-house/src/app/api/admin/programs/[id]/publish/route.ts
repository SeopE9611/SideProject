...
import { isValidAdminProgramId, publishAdminProgram } from "@/features/programs/program.admin-repository";
import { validateAdminProgramPublishInput } from "@/features/programs/program.admin-validation";
  ...
  return jsonResponse(
    {
      ...
      message: "게시 요청 내용이 허용된 크기를 초과했습니다.",
    },
    413,
  );
}
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
      ...
      return jsonResponse(
        {
          ...
          message: "JSON 형식으로 게시 요청을 보내 주세요.",
        },
        415,
      );
    }
    ...
    const validation = validateAdminProgramPublishInput(input);
    if (!validation.ok) {
    ...
    const result = await publishAdminProgram({
      id,
        ...
        return jsonResponse(
          {
            ...
            message: "현재 게시 상태에서는 공개할 수 없습니다.",
          },
          409,
        );
      }
    ...
    return jsonResponse({ ...result, redirectTo: `/admin/programs/${result.id}?published=1` }, 200);
  } catch (error) {
    console.error("관리자 프로그램 게시 처리 실패", {
    ...
    return jsonResponse(
      {
        ok: false,
        error: "unavailable",
        message: "현재 프로그램을 공개할 수 없습니다. 잠시 후 다시 시도해 주세요.",
      },
      503,
    );
  }
