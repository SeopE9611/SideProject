...
import { directPublishAdminProgramPost, isValidAdminProgramId } from "@/features/programs/program.admin-repository";
import { validateAdminProgramDirectPublishInput } from "@/features/programs/program.admin-validation";
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    if (length !== null) {
      ...
      if (Number.isFinite(bytes) && bytes > MAX_REQUEST_BYTES) return json({ ok: false, error: "payload_too_large" }, 413);
    }
    const text = await request.text();
    if (new TextEncoder().encode(text).byteLength > MAX_REQUEST_BYTES) return json({ ok: false, error: "payload_too_large" }, 413);
    let input: unknown;
    try { input = JSON.parse(text) as unknown; } catch { return json({ ok: false, error: "invalid_json" }, 400); }
    const validation = validateAdminProgramDirectPublishInput(input);
    if (!validation.ok) return json({ ok: false, error: "validation", fieldErrors: validation.fieldErrors }, 400);
    const result = await directPublishAdminProgramPost({ id, expectedUpdatedAt: validation.value.expectedUpdatedAt, actor: authorization.admin });
    if (!result.ok) {
      if (result.reason === "not_found") return json({ ok: false, error: result.reason }, 404);
      return json({ ok: false, error: result.reason }, 409);
    }
    return json({ ok: true, redirectTo: `/admin/programs/${result.id}?directPublished=1` }, 200);
  } catch (error) {
    console.error("관리자 프로그램 바로 게시 처리 실패", { contentId, errorName: error instanceof Error ? error.name : "UnknownError" });
