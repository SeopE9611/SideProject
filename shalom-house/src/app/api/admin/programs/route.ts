...
import { createAdminProgramDraft } from "@/features/programs/program.admin-repository";
import { validateAdminProgramDraftInput } from "@/features/programs/program.admin-validation";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const validation = validateAdminProgramDraftInput(input);
    if (!validation.ok) {
      return jsonResponse({ ok: false, error: "validation", fieldErrors: validation.fieldErrors }, 400);
    }

    const result = await createAdminProgramDraft(validation.value, admin);
    if (!result.ok) {
      return jsonResponse(
        {
          ok: false,
    ...
    return jsonResponse(
      {
        ...
        id: result.id,
        slug: result.slug,
        redirectTo: "/admin/programs?created=1",
      },
      201,
    );
  } catch (error) {
    console.error("관리자 프로그램 초안 저장 실패", {
