...
export default async function ContactPage({ searchParams }: { searchParams: Promise<{ kind?: string }> }) {
  ...
  const initialKind: InquiryKind = inquiryKinds.includes(requestedKind as InquiryKind) ? requestedKind as InquiryKind : "general";
  return <>
    ...
    <section className="mx-auto max-w-site px-page pb-16 sm:px-page-wide"><h2 className="text-heading font-bold">온라인 문의 접수</h2><p className="mt-3 text-muted-foreground">일반 문의, 방문, 자원봉사, 후원과 후원금 영수증·내역 문의를 접수할 수 있습니다.</p><PublicInquiryForm initialKind={initialKind} phoneFallback={contact.phone} /></section>
  </>;
}
