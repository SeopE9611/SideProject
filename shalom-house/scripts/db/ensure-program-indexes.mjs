...
if (!uri) {
  ...
  try {
    ...
    const programs = database.collection(programCollectionName);
    const audits = database.collection(auditCollectionName);
    const programIndexNames = await Promise.all([
      programs.createIndex({ slug: 1 }, { unique: true, name: "program_posts_slug_unique" }),
      programs.createIndex(
        {
          publicationStatus: 1,
          approvalStatus: 1,
          sortOrder: 1,
        ...
        { name: "program_posts_public_list" },
      ),
      programs.createIndex({ deletedAt: 1, updatedAt: -1, _id: -1 }, { name: "program_posts_admin_updated" }),
      programs.createIndex({ deletedAt: -1, _id: -1 }, { name: "program_posts_deleted_timeline" }),
    ]);
    const auditIndexNames = await Promise.all([
      audits.createIndex(
        { programId: 1, toVersionAt: 1 },
        { unique: true, name: "program_audit_events_program_version_unique" },
      ),
      ...
      audits.createIndex(
        ...
        { name: "program_audit_events_actor_timeline" },
      ),
    ]);
    console.log("프로그램 인덱스를 확인했습니다.", {
      databaseName,
      programCollectionName,
      programIndexNames,
      auditCollectionName,
      auditIndexNames,
    });
