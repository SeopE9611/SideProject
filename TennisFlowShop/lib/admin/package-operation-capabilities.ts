...
export function getAdminPackageOperationCapabilities({
  ...
  if (!hasIssuedPass) blockReasons.push("연결된 이용권이 없어 운영 작업을 할 수 있습니다.");
  if (!normalizedPassStatus) blockReasons.push("이용권 상태를 확인할 수 없어 운영 작업을 할 수 있습니다.");
    ...
    blockReasons.push("이용권 상태를 확인할 수 없어 운영 작업을 할 수 있습니다.");
  ...
  if (canExtend && isExpired(expiresAt, now))
    ...
    blockReasons.push("현재 잔여 횟수를 확인할 수 없어 횟수를 조절할 수 있습니다.");
