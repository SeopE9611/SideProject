...
export function buildDangerousActionToken(
  actionKey: AdminDangerousActionKey,
  adminId: string,
  previewHash: string,
): string {
...
export function isDangerousActionConfirmationValid(params: {
  ...
  const expectedToken = buildDangerousActionToken(
    params.actionKey,
    params.adminId,
    params.previewHash,
  );
  ...
  const tokenValid = Boolean(
    params.confirmationToken && params.confirmationToken === expectedToken,
  );
  const textValid = Boolean(
    params.confirmationText && params.confirmationText.trim() === expectedText,
  );
