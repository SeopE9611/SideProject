        ...
        $cond: [
          {
            $and: [
              { $eq: [{ $type: path }, "string"] },
              { $regexMatch: { input: path, regex: /^[0-9a-fA-F]{24}$/ } },
            ],
          },
          { $toObjectId: path },
          null,
...
export function buildAdminReviewRelationStages(): Record<string, unknown>[] {
  return [
    ...
    },
    {
      $lookup: {
        from: "used_rackets",
        localField: "racketIdObj",
        foreignField: "_id",
        as: "_racketByRacketId",
      },
    },
    {
      $lookup: {
    ...
    },
    {
      $lookup: {
        ...
        localField: "serviceApplicationIdObj",
        foreignField: "_id",
        as: "_application",
      },
    },
    ...
    },
    {
      $addFields: {
        ...
        rentalBrand: {
          $ifNull: [
            { $arrayElemAt: ["$_rentalRacket.brand", 0] },
            { $arrayElemAt: ["$_rental.brand", 0] },
          ],
        },
        rentalModel: {
          $ifNull: [
            { $arrayElemAt: ["$_rentalRacket.model", 0] },
            { $arrayElemAt: ["$_rental.model", 0] },
          ],
        },
