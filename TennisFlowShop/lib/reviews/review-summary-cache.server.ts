...
async function existsById(db: Db, collection: string, value: unknown) {
  ...
  return Boolean(await db.collection(collection).findOne({ _id: { $in: candidates } }, { projection: { _id: 1 } }));
}

async function findById(db: Db, collection: string, value: unknown, projection: Record<string, 1>) {
  const candidates = idCandidates(value);
  if (!candidates.length) return null;
  return db.collection(collection).findOne({ _id: { $in: candidates } }, { projection });
}
...
async function updateProductCache(db: Db, productId: string) {
  const summary = await getPublicReviewSummary(db, { type: "product", id: productId });
  const average = Number(Number(summary.average || 0).toFixed(2));
  const result = await db.collection("products").updateOne(
    { _id: { $in: idCandidates(productId) } },
...
async function updateRacketCache(db: Db, racketId: string) {
  const summary = await getPublicReviewSummary(db, { type: "racket", id: racketId });
  const average = Number(Number(summary.average || 0).toFixed(2));
  const count = Math.max(0, Number(summary.count) || 0);
  const result = await db.collection("used_rackets").updateOne(
    { _id: { $in: idCandidates(racketId) } },
    { $set: { ratingAvg: average, ratingAverage: average, ratingCount: count, reviewCount: count, reviewSummaryUpdatedAt: new Date() } },
  );
  return result.modifiedCount;
}

export async function refreshReviewSummaryCachesForTargets(db: Db, targets: AffectedReviewTargets) {
  let productsUpdated = 0;
  let racketsUpdated = 0;
  for (const productId of dedupeStringIds(targets.productIds)) productsUpdated += await updateProductCache(db, productId);
  for (const racketId of dedupeStringIds(targets.racketIds)) racketsUpdated += await updateRacketCache(db, racketId);
  return { productsUpdated, racketsUpdated };
}

export async function refreshReviewSummaryCachesForReview(db: Db, review: ReviewLike) {
  return refreshReviewSummaryCachesForTargets(db, await resolveAffectedReviewTargets(db, review));
}
...
export async function refreshReviewSummaryCachesForReviewSafely(
  ...
  try {
    await refreshReviewSummaryCachesForReview(db, review);
  } catch (error) {
