import { getReviewContextLabel, getReviewManagementCategory, inferReviewContext, type ReviewContext } from "./review-target";
...
export function buildAdminReviewSubject(row: Record<string, unknown>, context: ReviewContext) {
  const productName = pickString(row.productName, row.productTitle, row.productNameKo, row.productNameResolved);
  const racketName = pickString(row.racketName, [row.racketBrand, row.racketModel].map(asString).filter(Boolean).join(" "));
  const rentalName = pickString(row.rentalName, [row.rentalBrand, row.rentalModel].map(asString).filter(Boolean).join(" "));
  const stringName = pickString(row.stringName, row.serviceTargetName);
  if (context === "standalone_stringing") return stringName ? `교체서비스 · ${stringName}` : "교체서비스";
...
export function shapeAdminReview(row: Record<string, unknown>, contentLimit?: number): AdminReviewListItemDto {
