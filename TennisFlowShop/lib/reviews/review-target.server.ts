...
import {
  getStandaloneStringingIneligibleReason,
  hasStringingApplicationLink,
...
export function makeBundle(
  ...
  return {
    ...
    counts: {
      ...
      reviewed: reviewedEligibleTargets.length,
...
function imageOf(doc: any) {
  return (
    ...
    doc?.thumbnail ??
    doc?.image ??
    (Array.isArray(doc?.images) ? doc.images[0] : null) ??
    null
  );
}
...
function optionFromOrderItem(item: any) {
  return (
    [item?.optionLabel, item?.optionName, item?.gauge, item?.color].filter(Boolean).join(" / ") ||
    null
  );
}
function serviceOption(app: any) {
  ...
  return (
    [lines.length ? `라켓 ${lines.length}개` : null, tension].filter(Boolean).join(" / ") || null
  );
}
...
function relatedItem(params: {
  ...
  return {
    type: params.type,
    id: params.id ?? (params.doc?._id ? String(params.doc._id) : null),
    name: nameOf(params.doc, params.fallbackName),
    imageUrl: imageOf(params.doc),
...
function makeTarget(
  params: Omit<CanonicalReviewTarget, "targetKey" | "contextLabel"> & {
    targetKey?: string;
    contextLabel?: string;
  },
) {
...
export function collectOrderStringProductIds(order: any) {
  ...
  return dedupeStringIds(
    items
      .filter((it: any) =>
        [it?.kind, it?.category, it?.productType, it?.type].some(
          (v) =>
            String(v ?? "")
              .toLowerCase()
              .includes("string") || String(v ?? "").includes("스트링"),
        ),
      )
      .map((it: any) => it?.productId)
      .filter(Boolean),
...
export function isRacketOrderItem(item: any): boolean {
  return [item?.kind, item?.category, item?.productType, item?.type].some((v) => {
    const value = String(v ?? "").toLowerCase();
    return ["racket", "used_racket"].includes(value) || String(v ?? "").includes("라켓");
  });
}
export function collectOrderRacketIds(order: any) {
  ...
  return dedupeStringIds(
    items
      .flatMap((it: any) => [it?.racketId, isRacketOrderItem(it) ? it?.productId : null])
      .filter(Boolean),
  );
}
export function collectStringProductIdsFromApplication(app: any) {
  const directStringTypeIds = Array.isArray(app?.stringDetails?.stringTypes)
...
export function collectRacketIdsFromApplication(app: any) {
  ...
  return dedupeStringIds(
    [
      app?.racketId,
      app?.racket?._id,
      ...lines.flatMap((x: any) => [x?.racketId, x?.racket?._id]),
    ].filter(Boolean),
  );
}
...
export function matchReviewToCanonicalTarget(review: any, target: CanonicalReviewTarget) {
  const orderIds = target.orderId
    ? new Set(idCandidates(target.orderId).map(String))
    : new Set<string>();
  const rentalIds = target.rentalId
    ? new Set(idCandidates(target.rentalId).map(String))
    : new Set<string>();
  const appIds = new Set(target.applicationIds.flatMap(idCandidates).map(String));
  const productIds = target.primaryProductId
    ? new Set(idCandidates(target.primaryProductId).map(String))
    : new Set<string>();
  if (target.reviewContext === "product")
    return orderIds.has(String(review?.orderId)) && productIds.has(String(review?.productId));
  if (target.reviewContext === "product_stringing")
    return (
      (orderIds.has(String(review?.orderId)) && review?.reviewContext === "product_stringing") ||
      appIds.has(String(review?.serviceApplicationId)) ||
      appIds.has(String(review?.applicationId))
    );
  if (target.reviewContext === "standalone_stringing")
    return (
      appIds.has(String(review?.serviceApplicationId)) || appIds.has(String(review?.applicationId))
    );
  if (target.reviewContext === "rental" || target.reviewContext === "rental_stringing")
    return rentalIds.has(String(review?.rentalId));
  return false;
}
export function resolveReviewedFromLoadedReviews(
...
function withStatus(ctx: ReviewResolutionLoadContext, target: CanonicalReviewTarget) {
  return {
    ...target,
    ...resolveReviewedFromLoadedReviews(target, allReviewsForTarget(ctx, target)),
  };
}

function buildProductItem(
  ctx: ReviewResolutionLoadContext,
  id: string,
  order?: any,
  options?: { forceType?: "product" | "string" },
) {
  const item = (Array.isArray(order?.items) ? order.items : []).find(
    (it: any) => String(it?.productId) === id,
  );
  const doc = ctx.productsById.get(id) ?? item;
  const inferredType = String(item?.kind ?? item?.type ?? item?.category ?? "")
    .toLowerCase()
    .includes("string")
    ? "string"
    : "product";
  return relatedItem({
    type: options?.forceType ?? (inferredType as "product" | "string"),
    id,
    doc,
    optionLabel: optionFromOrderItem(item),
  });
}
function buildRacketItem(ctx: ReviewResolutionLoadContext, id: string, fallback?: any) {
  return relatedItem({
    type: "racket",
    id,
    doc: ctx.racketsById.get(id) ?? fallback,
    optionLabel: null,
    fallbackName: "라켓",
  });
}

export function buildOrderReviewTargetBundleFromLoadedData(
  ctx: ReviewResolutionLoadContext,
  order: any,
): ReviewTargetBundle {
  ...
  const isService =
    ...
    (hasLegacyDirect &&
      toId(order?.stringingApplicationId) &&
      !isStringingReviewBlockedStatus(
        ctx.applicationById.get(String(order.stringingApplicationId))?.status,
      ));
  ...
  if (isService) {
    ...
    const racketIds = dedupeStringIds([
      ...collectOrderRacketIds(order),
      ...applications.flatMap(collectRacketIdsFromApplication),
    ]);
    ...
    targets.push(
      withStatus(
        ...
        makeTarget({
          ...
          eligible: orderReviewEligible,
          reviewed: false,
          orderId: subjectId,
          rentalId: null,
          applicationIds,
          relatedProductIds: [...orderProductIds, ...appProductIds],
          relatedRacketIds: racketIds,
          primaryProductId,
          primaryApplicationId: applicationIds[0] ?? null,
          primaryRacketId: racketIds[0] ?? null,
          relatedItems,
          targetKey: buildReviewTargetKey({
            subjectType: "order",
            subjectId,
            reviewContext: "product_stringing",
          }),
...
export function buildRentalReviewTargetBundleFromLoadedData(
  ctx: ReviewResolutionLoadContext,
  rental: any,
): ReviewTargetBundle {
  const subjectId = String(rental._id);
  const directApplication = toId(rental?.stringingApplicationId)
    ? ctx.applicationById.get(String(rental.stringingApplicationId))
    : null;
  const linkedApps = ctx.applicationsByRentalId.get(subjectId) ?? [];
  const applications = validApps([
    ...linkedApps,
    ...(directApplication ? [directApplication] : []),
  ]).filter(
    (app) =>
      !app?.rentalId ||
      String(app.rentalId) === subjectId ||
      String(app._id) === String(rental?.stringingApplicationId),
  );
  const rentalReviewEligible = isRentalReviewEligible(rental);
  const racketIds = dedupeStringIds([
    ...collectRentalRacketIds(rental),
    ...applications.flatMap(collectRacketIdsFromApplication),
  ]);
  const applicationIds = applications.map((app) => String(app._id));
  ...
  const relatedItems = dedupeRelatedItems([
    relatedItem({
      ...
      id: subjectId,
      doc: rental,
      optionLabel: rentalPeriod(rental),
      fallbackName: "대여",
    }),
...
export function buildApplicationReviewTargetBundleFromLoadedData(
  ctx: ReviewResolutionLoadContext,
  app: any,
): ReviewTargetBundle {
  const subjectId = String(app._id);
  const parentOrder =
    (app.orderId ? ctx.parentOrderByApplicationId.get(subjectId) : null) ??
    ctx.parentOrderByApplicationId.get(subjectId);
  const parentRental =
    (app.rentalId ? ctx.parentRentalByApplicationId.get(subjectId) : null) ??
    ctx.parentRentalByApplicationId.get(subjectId);
  if (parentOrder) {
    const parent = buildOrderReviewTargetBundleFromLoadedData(ctx, parentOrder);
    return makeBundle(
      "application",
      subjectId,
      parent.targets.map((t) => ({
        ...t,
        subjectType: "application" as const,
        subjectId,
        eligible: false,
        ineligibleReason: "coveredByIntegratedReview",
        coveredBySubjectType: "order" as const,
        coveredBySubjectId: String(parentOrder._id),
        redirectTarget: t,
      })),
    );
  }
  if (parentRental) {
    ...
    return makeBundle(
      ...
      parent.targets.map((t) => ({
        ...
        eligible: false,
        ineligibleReason: "coveredByIntegratedReview",
        coveredBySubjectType: "rental" as const,
        coveredBySubjectId: String(parentRental._id),
        redirectTarget: t,
      })),
    );
  }
  ...
  const base = makeTarget({
    ...
    eligible: isStandaloneStringingReviewEligible(app),
    reviewed: false,
    ineligibleReason: getStandaloneStringingIneligibleReason(app),
    orderId: null,
    rentalId: null,
    applicationIds: [subjectId],
    relatedProductIds: productIds,
    relatedRacketIds: racketIds,
    primaryProductId: productIds[0] ?? null,
    primaryApplicationId: subjectId,
    primaryRacketId: racketIds[0] ?? null,
    relatedItems,
  });
...
export async function loadReviewResolutionContext(
  db: any,
  userId: ObjectId,
  params: { orders?: any[]; rentals?: any[]; applications?: any[] },
): Promise<ReviewResolutionLoadContext> {
  ...
  const directAppIds = dedupeStringIds(
    [
      ...orders.map((o) => o?.stringingApplicationId),
      ...rentals.map((r) => r?.stringingApplicationId),
      ...seedApps.map((a) => a?._id),
    ].filter(Boolean),
  );
  const appOr: any[] = [];
  ...
  const loadedApps = appOr.length
    ? await db
        .collection("stringing_applications")
        .find({ userId, status: { $ne: "draft" }, $or: appOr })
  ...
  const missingRentalIds = parentRentalIds.filter(
    (id) => !rentals.some((r) => String(r._id) === id),
  );
  const [parentOrders, parentRentals, directOrders, directRentals] = await Promise.all([
    missingOrderIds.length
      ? db
          .collection("orders")
          .find({ userId, _id: { $in: missingOrderIds.map(oid).filter(Boolean) } })
          .toArray()
      : [],
    missingRentalIds.length
      ? db
          .collection("rental_orders")
          .find({ userId, _id: { $in: missingRentalIds.map(oid).filter(Boolean) } })
          .toArray()
      : [],
    directAppIds.length
      ? db
          .collection("orders")
          .find({ userId, stringingApplicationId: { $in: directAppIds.flatMap(idCandidates) } })
          .toArray()
      : [],
    directAppIds.length
      ? db
  ...
  for (const app of allApps) {
    ...
    const directOrder = allOrders.find(
      (o) => String(o?.stringingApplicationId) === String(app._id),
    );
    const directRental = allRentals.find(
      (r) => String(r?.stringingApplicationId) === String(app._id),
    );
    const parentOrder = (app.orderId ? orderById.get(String(app.orderId)) : null) ?? directOrder;
    const parentRental =
      (app.rentalId ? rentalById.get(String(app.rentalId)) : null) ?? directRental;
    if (parentOrder) {
      parentOrderByApplicationId.set(String(app._id), parentOrder);
      pushMap(applicationsByOrderId, parentOrder._id, app);
    }
    if (parentRental) {
      parentRentalByApplicationId.set(String(app._id), parentRental);
      pushMap(applicationsByRentalId, parentRental._id, app);
    }
  }
  for (const map of [applicationsByOrderId, applicationsByRentalId])
    for (const [k, v] of map) map.set(k, Array.from(mapById(v).values()).sort(byCreatedAsc));
  ...
  const reviewOr: any[] = [];
  if (subjectOrderIds.length)
    reviewOr.push({ orderId: { $in: subjectOrderIds.flatMap(idCandidates) } });
  if (subjectRentalIds.length)
    reviewOr.push({ rentalId: { $in: subjectRentalIds.flatMap(idCandidates) } });
  if (subjectAppIds.length)
    reviewOr.push(
      { serviceApplicationId: { $in: subjectAppIds.flatMap(idCandidates) } },
      { applicationId: { $in: subjectAppIds.flatMap(idCandidates) } },
    );
  const reviews = reviewOr.length
    ? await db
        .collection("reviews")
        .find({ userId, isDeleted: { $ne: true }, $or: reviewOr })
        .toArray()
    : [];
  ...
  const productIds = sortStringIdsDeterministically([
    ...allOrders.flatMap(collectOrderProductIds),
    ...allApps.flatMap(collectStringProductIdsFromApplication),
  ]);
  const racketIds = sortStringIdsDeterministically([
    ...allOrders.flatMap(collectOrderRacketIds),
    ...allRentals.flatMap(collectRentalRacketIds),
    ...allApps.flatMap(collectRacketIdsFromApplication),
  ]);
  const [products, usedRackets, rackets] = await Promise.all([
    productIds.length
      ? db
          .collection("products")
          .find({ _id: { $in: productIds.map(oid).filter(Boolean) } })
          .toArray()
      : [],
    racketIds.length
      ? db
          .collection("used_rackets")
          .find({ _id: { $in: racketIds.map(oid).filter(Boolean) } })
          .toArray()
      : [],
    racketIds.length
      ? db
  ...
  return {
    applicationsByOrderId,
    applicationsByRentalId,
    applicationById,
    reviewsByOrderId,
    reviewsByRentalId,
    reviewsByApplicationId,
    productsById: mapById(products),
    racketsById: mapById([...rackets, ...usedRackets]),
    parentOrderByApplicationId,
    parentRentalByApplicationId,
  };
}
...
export async function resolveOrderReviewTargetBundle(
  db: any,
  userId: ObjectId,
  orderId: string,
): Promise<ReviewTargetBundle | null> {
...
export async function resolveRentalReviewTargetBundle(
  db: any,
  userId: ObjectId,
  rentalId: string,
): Promise<ReviewTargetBundle | null> {
  const rental = await db
    .collection("rental_orders")
    .findOne({ _id: new ObjectId(rentalId), userId });
...
export async function resolveApplicationReviewTargetBundle(
  db: any,
  userId: ObjectId,
  ...
  const app = await db
    .collection("stringing_applications")
    .findOne({ _id: new ObjectId(applicationId), userId });
  if (!app) return null;
...
export async function resolveOrderReviewTargetBundlesBatch(
  db: any,
  userId: ObjectId,
  orders: any[],
) {
  ...
  return new Map(
    orders.map((order) => [
      String(order._id),
      buildOrderReviewTargetBundleFromLoadedData(ctx, order),
    ]),
  );
}
export async function resolveRentalReviewTargetBundlesBatch(
  db: any,
  userId: ObjectId,
  rentals: any[],
) {
  ...
  return new Map(
    rentals.map((rental) => [
      String(rental._id),
      buildRentalReviewTargetBundleFromLoadedData(ctx, rental),
    ]),
  );
}
export async function resolveApplicationReviewTargetBundlesBatch(
  db: any,
  userId: ObjectId,
  applications: any[],
) {
  ...
  return new Map(
    applications.map((app) => [
      String(app._id),
      buildApplicationReviewTargetBundleFromLoadedData(ctx, app),
    ]),
  );
}

export async function resolveOrderReviewTarget(
  db: any,
  userId: ObjectId,
  orderId: string,
  optionalProductId?: string | null,
) {
  const bundle = await resolveOrderReviewTargetBundle(db, userId, orderId);
  if (!bundle) return null;
  const target = optionalProductId
    ? (bundle.targets.find((t) => t.primaryProductId === optionalProductId) ??
      bundle.nextTarget ??
      bundle.targets[0])
    : (bundle.nextTarget ?? bundle.targets[0]);
  return target
    ? {
        order: await db.collection("orders").findOne({ _id: new ObjectId(orderId), userId }),
        application: target.primaryApplicationId
          ? { _id: new ObjectId(target.primaryApplicationId) }
          : null,
...
export async function resolveStringingApplicationReviewTarget(
  db: any,
  userId: ObjectId,
  applicationId: string,
) {
  ...
  return {
    application: { _id: new ObjectId(applicationId) },
    reviewContext: target.reviewContext,
    contextLabel: target.contextLabel,
    relatedProductIds: objectIds(target.relatedProductIds),
    relatedRacketIds: target.relatedRacketIds,
    orderId: target.orderId ?? target.coveredBySubjectId,
    rentalId:
      target.rentalId ??
      (target.coveredBySubjectType === "rental" ? target.coveredBySubjectId : null),
    ...
    ineligibleReason:
      ...
      (target.subjectType !== "application" ? "coveredByIntegratedReview" : null),
  };
}
export async function resolveRentalReviewTarget(db: any, userId: ObjectId, rentalId: string) {
  ...
  if (!target) return null;
  return {
    rental: await db.collection("rental_orders").findOne({ _id: new ObjectId(rentalId), userId }),
    application: target.primaryApplicationId
      ? { _id: new ObjectId(target.primaryApplicationId) }
      : null,
    reviewContext: target.reviewContext,
