...
export function getShippingBadge(order: Order) {
  ...
  const displayLabel =
    label === "미입력"
      ? "선택 없음"
      : code === "courier" && (label === "등록됨" || label === "미등록")
        ? `택배 · ${label}`
        : label;
...
export function getShippingMethodBadge(order: Order) {
  ...
  return {
    ...
    tone,
    variant: badgeToneVariant(tone),
    color: shippingMethodColors[label]!,
  };
}
...
export function getTrackingBadge(order: Order) {
  ...
  if (code !== "courier") {
    return {
      ...
      color: trackingStatusColors["해당없음"],
    };
  }
  ...
  return {
    ...
    tone,
    variant: badgeToneVariant(tone),
    color: trackingStatusColors[label],
  };
}
...
export function getApplicationStatusTone(status?: string | null): BadgeSemanticTone {
  ...
  if (normalized === "접수완료" || lower === "received") return "warning";
  if (normalized.includes("취소") || lower === "canceled" || lower === "cancelled") return "danger";
  return "neutral";
}
...
export function getBoardCategoryTone(
  kind: BoardBadgeKind,
