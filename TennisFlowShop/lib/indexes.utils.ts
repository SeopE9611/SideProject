 ...
 */
export function hasMatchingIndex(
  existing: readonly IndexDocument[],
  spec: DesiredIndexSpec,
) {
  return existing.some((idx) => {
    if (idx?.name === spec.name) return true;

    const keysMatch = isSameKeys(
      (idx?.key ?? {}) as Record<string, unknown>,
      spec.keys,
    );
    ...
    const desiredExpireAfterSeconds = normalizeExpireAfterSeconds(
      spec.options?.expireAfterSeconds,
    );
    const existingExpireAfterSeconds = normalizeExpireAfterSeconds(
