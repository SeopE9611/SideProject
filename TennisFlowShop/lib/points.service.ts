...
export async function getPointsBalance(
  ...
  const u = await users.findOne(
    { _id: userId as any },
    { projection: { pointsBalance: 1 } as any },
  );
...
export async function getPointsSummary(
  db: Db,
  userId: ObjectId,
): Promise<{ balance: number; debt: number; available: number }> {
  const users = db.collection("users");
  const u = await users.findOne(
    { _id: userId as any },
...
export async function getPointsState(
  db: Db,
  userId: ObjectId,
    ...
    [
      ...
      { $set: { __payDebt: { $min: ["$pointsDebt", amount] } } },
      {
        $set: {
          ...
          pointsBalance: {
            $add: ["$pointsBalance", { $subtract: [amount, "$__payDebt"] }],
          },
          updatedAt: now,
        },
      },
...
export async function deductPoints(
  ...
  if (params.allowNegativeBalance) {
    ...
    res = await users.updateOne(
      filter,
      { $inc: { pointsBalance: -amount }, $set: { updatedAt: now } },
      { session: opts.session },
    );
...
): Promise<{ total: number; docs: PointTransaction[] }> {
