...
export function validateBaseSettings(
  ...
  if (
    !Number.isInteger(settings.interval) ||
  ...
  if (
    ...
    settings.capacity < CAPACITY_MIN ||
    settings.capacity > CAPACITY_MAX
  ) {
  ...
  if (
    ...
    settings.bookingWindowDays < BOOKING_WINDOW_MIN ||
    settings.bookingWindowDays > BOOKING_WINDOW_MAX
  ) {
...
export function validateExceptionItem(exception: ExceptionItem): string | null {
  ...
  if (hasStart && hasEnd) {
    ...
    if (
      typeof exception.interval === "number" &&
      !hasAtLeastOneSlot(exception.start!, exception.end!, exception.interval)
    ) {
  ...
  if (typeof exception.interval === "number") {
    if (
      !Number.isInteger(exception.interval) ||
      exception.interval < INTERVAL_MIN ||
      exception.interval > INTERVAL_MAX
    ) {
  ...
  if (typeof exception.capacity === "number") {
    if (
      !Number.isInteger(exception.capacity) ||
      exception.capacity < CAPACITY_MIN ||
      exception.capacity > CAPACITY_MAX
    ) {
