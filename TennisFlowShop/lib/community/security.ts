...
export const COMMUNITY_RATE_LIMIT_POLICIES: Record<
  CommunityRateLimitPolicy["routeId"],
...
export async function enforceCommunityRateLimit(params: {
  ...
  if (userId) {
    const userResult = await consumeRateLimitWindow(
      `user:${policy.routeId}:${userId}`,
      policy,
      policy.perUserLimit,
    );
