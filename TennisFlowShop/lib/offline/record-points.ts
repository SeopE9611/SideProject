...
function sanitizePoints(points: any) {
  return {
    ...
    grantRevertTxId: points?.grantRevertTxId
      ? String(points.grantRevertTxId)
      : null,
    grantRevertedAt: serializeDate(points?.grantRevertedAt),
    grantRevertedBy: points?.grantRevertedBy
      ? String(points.grantRevertedBy)
      : null,
    grantRevertReason:
      typeof points?.grantRevertReason === "string"
        ? points.grantRevertReason
        : null,
    deductRevertTxId: points?.deductRevertTxId
      ? String(points.deductRevertTxId)
      : null,
    deductRevertedAt: serializeDate(points?.deductRevertedAt),
    deductRevertedBy: points?.deductRevertedBy
      ? String(points.deductRevertedBy)
      : null,
    deductRevertReason:
      typeof points?.deductRevertReason === "string"
        ? points.deductRevertReason
        : null,
  };
}
...
export async function handleOfflineRecordPoints(
  ...
  if (mode === "grant" && previousPointsState.grantTxId) {
    return NextResponse.json(
      { message: "points already granted for this record" },
      { status: 409 },
    );
  }
  if (mode === "deduct" && previousPointsState.deductTxId) {
    return NextResponse.json(
      { message: "points already deducted for this record" },
      { status: 409 },
    );
  }

  const offlineCustomerId =
    (record as any).offlineCustomerId instanceof ObjectId
      ? (record as any).offlineCustomerId
      : null;
  ...
  const customer = await db
    ...
    .findOne({ _id: offlineCustomerId }, { projection: { linkedUserId: 1 } });
  if (!customer)
    return NextResponse.json(
      { message: "offline customer not found" },
      { status: 404 },
  ...
  const linkedUserId =
    (customer as any).linkedUserId instanceof ObjectId
      ? (customer as any).linkedUserId
      : null;
  if (!linkedUserId)
    return NextResponse.json(
  ...
  const user = await db
    ...
    .findOne(
      { _id: linkedUserId },
      { projection: { _id: 1, pointsBalance: 1 } },
    );
  if (!user)
  ...
  try {
    if (mode === "grant") {
      const result = await grantPoints(db, {
        ...
        ref: {
          adminId: admin._id,
          offlineRecordId: recordObjectId,
          offlineCustomerId,
        },
      });
    ...
    return NextResponse.json(
      { message: "points transaction failed" },
      { status: 500 },
    );
  }
  ...
  const updateResult = await db.collection("offline_service_records").updateOne(
    {
      ...
      $or:
        mode === "grant"
          ? [
              { "points.grantTxId": { $exists: false } },
              { "points.grantTxId": null },
              { "points.grantTxId": "" },
            ]
          : [
              { "points.deductTxId": { $exists: false } },
              { "points.deductTxId": null },
              { "points.deductTxId": "" },
            ],
    },
...
async function findPointTransaction(db: Db, txId: string) {
  const filter = ObjectId.isValid(txId)
    ? { _id: new ObjectId(txId) }
    : { _id: txId as any };
  return db
    .collection("points_transactions")
    .findOne(filter as any, { projection: { _id: 1 } });
}
...
export async function handleOfflineRecordPointRevert(
  ...
  if (!reason)
    return NextResponse.json(
      { message: "revert reason required" },
      { status: 400 },
    );
  ...
  const originalTxId =
    mode === "grant"
      ? previousPointsState.grantTxId
      : previousPointsState.deductTxId;
  const amount =
    mode === "grant" ? previousPointsState.earn : previousPointsState.use;
  if (!originalTxId || !amount || amount < 1) {
    return NextResponse.json(
      {
        message:
          mode === "grant"
            ? "points grant not found"
            : "points deduction not found",
      },
      { status: 404 },
    );
  }
  if (
    mode === "grant" &&
    (previousPointsState.grantRevertedAt || previousPointsState.grantRevertTxId)
  ) {
    return NextResponse.json(
      { message: "points grant already reverted" },
      { status: 409 },
    );
  }
  if (
    mode === "deduct" &&
    (previousPointsState.deductRevertedAt ||
      previousPointsState.deductRevertTxId)
  ) {
    return NextResponse.json(
      { message: "points deduction already reverted" },
      { status: 409 },
    );
  }
  ...
  if (mode === "grant" && balanceBefore < amount) {
    return NextResponse.json(
      { message: "insufficient points to revert grant" },
      { status: 409 },
    );
  }
      ...
      return NextResponse.json(
        {
          message:
            mode === "grant"
              ? "points grant already reverted"
              : "points deduction already reverted",
        },
        { status: 409 },
      );
    }
    console.error(
      `[offline record points:${mode}:revert] transaction failed`,
      err,
    );
    return NextResponse.json(
      { message: "points revert failed" },
      { status: 500 },
    );
  }
  ...
  const updateResult = await db.collection("offline_service_records").updateOne(
    {
      ...
      ...(mode === "grant"
        ? {
            ...
            $or: [
              { "points.grantRevertTxId": { $exists: false } },
              { "points.grantRevertTxId": null },
        ...
        : {
            ...
            $or: [
              { "points.deductRevertTxId": { $exists: false } },
              { "points.deductRevertTxId": null },
