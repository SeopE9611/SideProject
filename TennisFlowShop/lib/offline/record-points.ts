...
function sanitizePoints(points: any) {
  return {
    ...
    grantRevertReason:
      typeof points?.grantRevertReason === "string" ? points.grantRevertReason : null,
    ...
    deductRevertReason:
      typeof points?.deductRevertReason === "string" ? points.deductRevertReason : null,
  };
}
...
export async function handleOfflineRecordPoints(
  ...
  const linkedUserId =
    (customer as any).linkedUserId instanceof ObjectId ? (customer as any).linkedUserId : null;
  if (!linkedUserId) return NextResponse.json({ message: "linked user required" }, { status: 400 });

  const user = await db
    .collection("users")
    .findOne({ _id: linkedUserId }, { projection: { _id: 1, pointsBalance: 1 } });
...
export async function handleOfflineRecordPointRevert(
  ...
  const previousPointsState = sanitizePoints((record as any).points);
  const originalTxId =
    mode === "grant" ? previousPointsState.grantTxId : previousPointsState.deductTxId;
  ...
  if (
    mode === "deduct" &&
    (previousPointsState.deductRevertedAt || previousPointsState.deductRevertTxId)
  ) {
  ...
  if (updateResult.matchedCount === 0) {
    return NextResponse.json(
      {
        message:
          mode === "grant" ? "points grant already reverted" : "points deduction already reverted",
      },
      { status: 409 },
    );
  }
