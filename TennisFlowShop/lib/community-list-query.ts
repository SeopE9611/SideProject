...
export function buildCommunityListMongoFilter(
  query: Pick<
    CommunityListQuery,
    | "typeParam"
    | "brand"
  ...
  if (query.typeParam === "market") {
    ...
    if (query.category === "racket") {
      if (f.modelKeyword)
        filter["marketMeta.racketSpec.modelName"] = {
          $regex: escapeRegex(f.modelKeyword),
          $options: "i",
        };
    ...
    if (query.category === "string") {
      if (f.modelKeyword)
        filter["marketMeta.stringSpec.modelName"] = {
          $regex: escapeRegex(f.modelKeyword),
          $options: "i",
        };
...
export function parseCommunityListQuery(
  ...
  const typeParam =
    rawType && (COMMUNITY_BOARD_TYPES as readonly string[]).includes(rawType)
      ? (rawType as CommunityBoardType)
      : null;
  ...
  [marketFilters.minPrice, marketFilters.maxPrice] = normalizeRange(
    marketFilters.minPrice,
    marketFilters.maxPrice,
  );
  [marketFilters.minWeight, marketFilters.maxWeight] = normalizeRange(
    marketFilters.minWeight,
    marketFilters.maxWeight,
  );
  [marketFilters.minBalance, marketFilters.maxBalance] = normalizeRange(
    marketFilters.minBalance,
    marketFilters.maxBalance,
  );
  [marketFilters.minHeadSize, marketFilters.maxHeadSize] = normalizeRange(
    marketFilters.minHeadSize,
    marketFilters.maxHeadSize,
  );
  [marketFilters.minSwingWeight, marketFilters.maxSwingWeight] = normalizeRange(
    marketFilters.minSwingWeight,
    marketFilters.maxSwingWeight,
  );
  [marketFilters.minStiffnessRa, marketFilters.maxStiffnessRa] = normalizeRange(
    marketFilters.minStiffnessRa,
    marketFilters.maxStiffnessRa,
  );
