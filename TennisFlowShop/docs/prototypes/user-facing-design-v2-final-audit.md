...
## 8. PR #2330 배포 후 사용자 상태·모바일 CTA 후속 보완

PR #2330 배포 화면을 직접 확인한 결과 전역 404, 라켓 비교 빈 상태, 커뮤니티 후기 CTA, 아카데미 단일 클래스 카드 중앙 정렬은 통과했다. 다만 후속 QA에서 `/board/qna`의 실제 빈 상태에 검은 36px `문의하기`가 남아 있었고, Hero·목록 헤더·빈 상태에 같은 CTA가 반복되는 문제를 확인했다.

정적 교차 확인에서는 전역 오류 복구 버튼, 일반 게시판 상세의 동일 목적 목록 이동 중복, 아카데미 클래스·신청 상태와 라켓 비교의 모바일 조작 높이가 남아 있었다. 이번 수정은 해당 상태 화면의 행동 위계를 명시하고 모바일 핵심 조작을 최소 44px로 보완했으며, 데스크톱 공개 화면에서 기존 compact 배치를 보존하는 범위다.

이 기록은 데스크톱 공개 화면 확인과 모바일 코드 정책 보완 결과이며, 인증 데이터가 필요한 모든 상태 또는 실제 모바일 기기에서의 QA까지 완료했다는 의미는 아니다.


## PR #2334 이후 활성 사용자 CTA·상태 복구 통합 보완

PR #2334 이후 `origin/main`의 활성 사용자 경로를 정적으로 다시 대조해, 모바일 action slot·필터·조건부 오류/종료 상태에 남은 36~40px 터치 영역과 과거 기본 검정 CTA를 보완했다. 공용 Hero/ResultState/EmptyState의 action 영역은 모바일에서 실제 링크·버튼에 최소 44px를 적용하고 `bp-sm` 이후에는 각 Button의 기존 size가 다시 적용되도록 제한했다. 관리자 tone과 action 영역 밖의 링크·버튼에는 이 규칙을 전파하지 않았다.

활성 공지·이벤트 및 Q&A 필터는 모바일 44px, 데스크톱 40px로 실제 컨트롤과 skeleton을 맞췄고, FAQ 카테고리와 검색어 삭제 터치 영역도 같은 기준으로 정리했다. 선택된 탭·필터·FAQ 카테고리의 검정 배경은 선택 상태이므로 유지했다. 가입·추천·오류 재시도·다음 단계는 `highlight`, 주문과 교체서비스의 반복 후속 행동은 `highlight_soft`, 목록·고객센터 같은 보조 이동은 `outline` 또는 `secondary`로 구분했다. 쪽지·알림 오류에는 기존 조회 함수를 재사용한 재시도를, 종료된 자가발송 신청에는 기존 return 경로를 재사용한 진행상태 복귀 행동을 추가했다.

이번 기록은 지정된 활성 사용자 코드의 정적 확인과 typecheck 범위이며 모든 조건·기기에서의 시각 검증 완료를 뜻하지 않는다. NicePay와 무통장입금이 현재 활성 결제 흐름이고 Toss 및 비회원 주문 조회는 현재 활성 사용자 결제 흐름이 아니므로 이번 변경에서 제외했다. `/board/brands`, `/board/hot`, 비활성 커뮤니티 작성·수정 화면은 기능 활성화 또는 별도 검증 환경이 마련될 때 확인할 후순위 조건부 잔여로 남긴다. 관리자 화면, 결제·주문 상태 전이, API·데이터 모델·SWR 키·mutation 로직은 변경하지 않았다.

## PR #2335 이후 인증 사용자 목록·패널 후속 검증

PR #2335 이후 활성 인증 사용자 화면을 정적으로 재검색한 결과, 마이페이지 거래·문의·아카데미·후기·위시리스트·포인트 목록과 공개 후기 카드 및 알림 패널에 모바일 32~40px action 잔여가 확인됐다. 실제 action은 모바일 최소 44px, 넓은 breakpoint에서는 기존 36px 또는 40px로 맞췄으며 목록 skeleton도 정상 화면과 같은 높이로 보완했다. 거래 카드 더보기 항목은 모바일 32px에서 44px로 변경하고 데스크톱 32px은 유지했다. 위시리스트와 알림 오류 화면에는 기존 조회 mutate를 재사용한 재시도를 제공했다.

이는 인증 데이터가 필요한 상태의 정적 코드 검증이며 실제 모바일 기기와 가능한 모든 사용자 데이터 조합의 시각 QA 완료를 뜻하지 않는다. 현재 활성 렌더 경로에 연결되지 않은 `ActivityFeed`, `OrdersScopeContextNav`, `UserSection`, 구형 `HomePageClient`는 이번 완료 판정에서 제외했다. 선택 탭의 검정 active 상태와 파괴적 action은 유지했으며 API·상태 전이·mutation 로직은 변경하지 않았다. 비활성 커뮤니티 게시판과 Toss·비회원 주문 조회는 운영 활성화 시 별도 검증할 후순위 조건부 범위로 계속 분리한다.
