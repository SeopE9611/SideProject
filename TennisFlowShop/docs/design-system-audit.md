...
- 공용 상세 Hero를 단일 surface 안에서 상세 종류, 제목, 상태·식별 번호, 요약, 다음 할 일, 보조 액션 순으로 읽히도록 정리했다.
- 주문·대여·교체서비스·아카데미 상세의 모바일 핵심 액션을 최소 44px 터치 영역과 1열 우선 배치로 맞췄다.
- 비회원 주문 결과의 외곽/주문별 이중 카드와 로컬 버튼 색상 조합을 정리하고, 비회원 상세의 반복 `shadow-lg`를 낮은 elevation으로 조정했다.
