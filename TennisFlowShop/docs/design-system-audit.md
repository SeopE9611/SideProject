...
## 11. 사용자 거래 상세 V2 표면·반응형 정리

### 완료

- 마이페이지 거래 목록은 `DashboardSectionPanel`을 유지하고 주문·대여·교체서비스 상세은 평면 상세 컨테이너로 분리했다.
- 주문·대여·교체서비스 상세 내부의 사용자용 `SiteContainer` 여백을 제거하고, 아카데미 상세 Shell의 중복 섹션 패널도 제거했다.
- 공용 상세 Hero는 모바일에서 상태·식별 번호, 요약, 다음 할 일, 보조 액션 순의 한 열을 유지하고, 데스크톱에서는 상태·요약을 왼쪽, 다음 행동을 오른쪽, 보조 액션을 하단 전체 폭에 배치했다.
- 주문·대여·교체서비스·아카데미 상세 Skeleton도 실제 Hero와 같은 평면 구분선, 모바일 정보 순서, 데스크톱 2열 및 하단 보조 액션 구조로 맞췄다.
- 이번 범위에서 확인된 주문·대여·교체서비스·아카데미 상세과 비회원 주문 화면의 사용자 액션을 최소 44px 터치 영역과 모바일 1열 우선 배치로 맞췄다.
- 교체서비스 상세의 일반 사용자 편집·운송장·라켓별 펼침 버튼과 수정 Dialog 닫기 버튼을 최소 44px 터치 영역으로 보정했다.
- 주문 상세의 라켓·스트링 상세 `summary` 클릭 영역을 최소 44px로 보정하고, 주문 Skeleton Hero의 radius를 실제 Hero와 같은 `rounded-2xl`로 맞췄다.
- 비회원 주문 결과와 상세의 버튼형 raw Link를 공용 `Button`으로 교체하고, 비회원 상세의 필드별 미니 카드와 결제 혜택 안내를 행·구분선 중심으로 완화했다.
- 상품·라켓 목록 Hero와 초기 loading은 공용 Commerce Catalog 문법으로 통일을 완료했다.

### 부분 적용

- 주요 정보 섹션 카드는 유지하면서 이번 범위에서 확인된 반복 필드와 안내 영역을 낮은 대비의 행·구분선 중심으로 완화했다. 기능 상태와 결합된 모든 내부 표시를 일괄 재구성하지는 않았다.
- 교체서비스 상세은 관리자와 공유되는 구조이므로 사용자 분기의 폭·Hero·CTA만 변경했고 관리자 `SiteContainer`, grid, surface는 유지했다.

### 다음 단계로 이월

- 상품·라켓 일반 상세은 아래 Commerce Detail 단계에서 정리했으며, 제외된 라켓 하위 구매·대여·스트링 선택 플로우는 다음 단계로 유지한다.
- 독립 후기 허브, 게시판, 장바구니, 체크아웃 및 결제 화면은 다음 단계로 유지한다.
- 공용 Button의 전역 size·variant 재정의와 사용하지 않는 구형 상세·목록 컴포넌트 정리는 별도 회귀 검토가 필요한 후속 작업으로 남긴다.

## 12. 일반 상품·라켓 상세 Commerce Detail 정리

### 완료

- 일반 `/products/[id]`, `/rackets/[id]` 상세의 갤러리·구매 패널·탭 외곽을 모바일 평면 섹션, 768px 이상 카드 표면이라는 공용 반응형 문법으로 맞췄다.
- 설명·스펙·후기·문의는 모바일에서 행과 구분선 중심으로 완화하고, 후기 작성·문의하기 empty state의 중복 CTA를 제거했다.
- 상품·라켓 후기의 정보 계층, 44px 모바일 관리·사진 조작 영역, 후기 수정 Dialog의 키보드 별점과 업로드 사진 표시 구조를 맞췄다.
- 두 후기 이미지 뷰어에 유효 index 방어, 현재 위치, 이전·다음 조작, 선택 가능한 썸네일과 접근성 상태를 적용했다.
- 공용 상세 loading에 도메인별 스크린리더 문구와 busy 상태를 추가하고, 상품·라켓 결과 상태를 같은 `PublicPageHero`·`ResultState` 표현으로 맞췄다.
- 관련 상품과 최근 본 상품은 상품 카드 목록을 다시 큰 Card로 감싸지 않는 일반 section 구조로 정리했다.
