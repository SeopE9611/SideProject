...
- 후기 관리와 게시판 관리는 Step 2-C 대상이다.
- 후기의 `SWRInfinite`·일괄 작업과 게시판의 게시글·신고 탭 구조를 각각 보존한 채
  공통 필터 구조를 전파해야 한다.
...
1. Step 2-C에서 후기·게시판의 복합 목록 구조를 보존하며 공통 필터 구조를 전파한다.
2. 대여 상세의 업무 우선순위를 기준으로 주문·교체서비스·회원·패키지 상세를 통일한다.
3. 상품 신규 등록 패턴을 상품 수정에 먼저 맞춘 뒤 라켓과 아카데미 폼으로 확장한다.
4. 감사 로그, 사설 결제, 매출 보고, 오프라인 운영 등 별도 도구·리포트는 분류 후 밀도와 표 규격을 정리한다.
