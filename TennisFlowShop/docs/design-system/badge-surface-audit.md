...
| 영역 | 활성 화면 수 | V2 이관 | raw 허용 | 실제 미이관 |
| --- | ---: | ---: | ---: | ---: |
| 메인·상품·라켓 핵심 화면 | 8 | 8 | 0 | 0 |
| 라켓 케어 | 2 | 2 | 0 | 0 |

표의 화면 수는 이 문서의 고객 화면 조합 추적에서 실제 경로로 확인한 핵심 화면과 이번에 직접
수정한 라켓 케어 화면만 집계한다. 서로 다른 의미를 한 화면에서 여러 번 렌더링하는 경우에도
화면은 한 번만 센다.
