...
## PR #2335 이후 인증 사용자 목록·패널 모바일 action 보완

PR #2335 병합 뒤 활성 인증 사용자 렌더 경로를 다시 정적 검색해 거래/이용 내역, 문의, 아카데미 신청, 후기 관리, 위시리스트, 포인트와 공개 후기·헤더 알림 패널에 남은 모바일 32~40px action을 확인했다. 대상 action과 대응 skeleton은 모바일 최소 44px로 맞추고 `bp-sm`, `sm` 또는 `md` 이상에서는 기존 36px·40px compact 밀도를 유지했다. 거래 action group은 범위가 제한된 descendant selector로 실제 링크·버튼에 같은 규칙을 적용했으며, 모바일 더보기 메뉴는 32px에서 44px로 보완했다.

위시리스트와 알림 오류 상태에는 각 훅이 이미 제공하는 `mutateAll`과 `mutate`를 재사용한 `highlight` 재시도를 추가했다. 선택된 마이페이지 탭의 검정 배경, 거래 후속 action의 `highlight_soft`, 후기 작성의 `secondary`, 삭제·취소의 파괴적 의미는 유지했다. API, 인증·권한, 결제, 거래 상태 판정·전이와 mutation 동작은 변경하지 않았다.

이번 완료 판정은 인증 데이터가 필요한 화면의 정적 코드 검증 결과이며 실제 모바일 기기 또는 모든 사용자 데이터 상태의 시각 QA 완료를 의미하지 않는다. 활성 렌더 경로에 연결되지 않은 `ActivityFeed`, `OrdersScopeContextNav`, `UserSection`, 구형 `HomePageClient`는 판정에서 제외했다. 비활성 커뮤니티 게시판과 Toss·비회원 주문 조회도 계속 후순위 조건부 범위로 분리한다.
