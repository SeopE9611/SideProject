...
export default function ReviewsClient() {
  ...
  const activeFilterCount =
    (sort !== "latest" ? 1 : 0) + (rating !== "all" ? 1 : 0) + (hasPhoto ? 1 : 0);
  const activeChips = [
    sort !== "latest"
      ? { key: "sort", label: sortLabels[sort], onRemove: () => setSort("latest") }
      : null,
    rating !== "all"
      ? { key: "rating", label: `${rating}점`, onRemove: () => setRating("all") }
      : null,
        ...
        <CardContent className="space-y-4 p-4 bp-md:p-5">
              ...
              {categories.map((category) => {
                ...
                return (
                  <button
                    ...
                    className="min-h-11 whitespace-nowrap rounded-control border border-border px-4 text-ui-body-sm font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring data-[selected=true]:border-brand-highlight/45 data-[selected=true]:bg-brand-highlight-muted data-[selected=true]:text-brand-highlight-foreground dark:data-[selected=true]:text-brand-highlight"
          ...
          <div className="hidden flex-wrap items-center gap-3 bp-md:flex">
            <Select value={sort} onValueChange={(v) => setSort(v as SortKey)}>
              <SelectTrigger className="h-10 w-[150px] rounded-control border-border bg-background">
            ...
            <Select value={rating} onValueChange={(v) => setRating(v as RatingFilter)}>
              <SelectTrigger className="h-10 w-[140px] rounded-control border-border bg-background">
            ...
            <label className="inline-flex min-h-10 items-center gap-2 rounded-control border border-border bg-card px-3 text-ui-body-sm font-medium text-foreground">
              <Checkbox
                ...
                className="rounded border-border data-[state=checked]:border-primary data-[state=checked]:bg-primary"
              />
              사진 포함
            </label>
            <Button
              ...
              className="ml-auto h-10 rounded-control"
            >
              <X className="h-4 w-4" />
              초기화
            </Button>
          ...
          <div className="grid grid-cols-[1fr_auto] gap-2 bp-md:hidden">
            <Button
              type="button"
              variant="outline"
              className="min-h-11 justify-start rounded-control"
              onClick={openMobileFilters}
              aria-label={`정렬 변경, 현재 ${sortLabels[sort]}`}
            >
              {sortLabels[sort]}
            </Button>
            <Button
              ...
              onClick={openMobileFilters}
              aria-label={`필터 열기, 활성 필터 ${activeFilterCount}개`}
            >
              ...
              >
                ...
                <div className="grid gap-5 py-5">
                  <div className="grid gap-2">
                    <label className="text-ui-label font-semibold text-foreground">정렬</label>
                    <Select value={draftSort} onValueChange={(v) => setDraftSort(v as SortKey)}>
                      <SelectTrigger className="min-h-11 rounded-control">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="latest">최신순</SelectItem>
                  ...
                  <div className="grid gap-2">
                    <label className="text-ui-label font-semibold text-foreground">별점</label>
                    <Select
                      value={draftRating}
                      onValueChange={(v) => setDraftRating(v as RatingFilter)}
                    >
                      <SelectTrigger className="min-h-11 rounded-control">
                        <SelectValue />
                      </SelectTrigger>
                  ...
                  <label className="flex min-h-11 items-center gap-3 rounded-control border border-border p-3 text-ui-body-sm font-medium">
                    <Checkbox
                      checked={draftHasPhoto}
                      onCheckedChange={(v) => setDraftHasPhoto(Boolean(v))}
                    />
                    사진 포함 후기만 보기
                  </label>
                </div>
        ...
        <>
          {isInitialError ? (
            <EmptyState
              ...
              action={
                <Button variant="outline" onClick={() => mutate()} className="rounded-control">
            ...
            <EmptyState
              ...
              description="필터를 초기화하거나 작성 가능한 후기를 확인해 보세요."
              action={
          ...
          <div className="flex justify-center pt-4">
            {!isInitialError && !isLoadMoreError && hasMore ? (
              <Button
                onClick={() => setSize(size + 1)}
                disabled={isValidating}
                variant="outline"
                className="rounded-control border-border bg-background px-6 py-2 hover:bg-card bp-md:px-8"
              >
                {isValidating ? (
                  <>
              ...
              <div className="py-6 text-center text-ui-body-sm text-muted-foreground">
                마지막 페이지입니다
              </div>
            ) : null}
          </div>
