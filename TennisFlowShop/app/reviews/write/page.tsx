...
export default function ReviewWritePage() {
  ...
  const badgeVariant = state === "loading" ? "secondary" : state === "ok" ? "success" : state === "already" ? "neutral" : state === "unauthorized" ? "warning" : state === "error" ? "danger" : state === "coveredByIntegratedReview" ? "info" : "warning";

  if (!authChecked) {
    return (
      <div className="min-h-screen bg-background pb-28 bp-lg:pb-0">
        <SiteContainer className="py-6 bp-md:py-8"><div className="mx-auto max-w-6xl space-y-5"><Card variant="feature"><CardContent className="space-y-4 p-5"><Skeleton className="h-4 w-36" /><Skeleton className="h-8 w-72 max-w-full" /><Skeleton className="h-20 w-full" /></CardContent></Card><Skeleton className="h-20 w-full rounded-panel" /><div className="grid gap-5 bp-lg:grid-cols-[minmax(0,1fr)_320px]"><Card><CardContent className="space-y-5 p-5"><Skeleton className="h-24 w-full" /><Skeleton className="h-44 w-full" /><Skeleton className="h-32 w-full" /></CardContent></Card><Skeleton className="hidden h-56 rounded-panel bp-lg:block" /></div></div></SiteContainer>
      </div>
    );
  }
        ...
        <div className="mx-auto max-w-6xl space-y-5">
          <Card variant="feature" className="rounded-panel">
            <CardContent className="space-y-4 p-5 bp-md:p-6">
              <Button type="button" variant="ghost" onClick={() => confirmLeaveIfDirty(() => { void photoSession.cleanupUncommittedPhotos(); photoSession.resetSession(); router.replace("/reviews"); })} className="h-auto w-fit px-0 text-ui-body-sm text-muted-foreground hover:bg-transparent hover:text-foreground">← 후기 목록</Button>
              <div className="space-y-2">
                <Badge variant="signal" className="w-fit">VERIFIED EXPERIENCE</Badge>
                <h1 className="break-keep font-brand-heading text-ui-section-title font-bold text-foreground">확인된 경험을 후기로 남겨주세요</h1>
                <p className="max-w-2xl text-ui-body-sm text-muted-foreground">구매·대여·교체서비스를 완료한 경험만 후기로 등록할 수 있습니다.</p>
              </div>
            </CardContent>
          </Card>

          <ReviewWriteProgress targetVerified={targetVerified} formStarted={formStarted} isSubmitting={isSubmitting} isLoading={state === "loading"} />
          ...
          <Card className="rounded-panel border-border bg-card shadow-soft">
            <CardContent className="space-y-4 p-4 bp-sm:p-5">
              ...
              {canonicalTarget ? <ReviewTargetSummary target={canonicalTarget} /> : <p className="rounded-panel border border-border bg-muted/30 p-4 text-ui-body-sm text-muted-foreground">후기 대상을 확인하는 중입니다.</p>}
            </CardContent>
          </Card>

          <div className="bp-lg:hidden"><ReviewWriteChecklist targetReady={targetVerified} ratingReady={ratingReady} contentReady={contentReady} photosCount={photos.length} isUploading={isUploading} /></div>
            ...
            <Card className="rounded-panel border-border bg-card shadow-soft">
              <CardContent className="p-4 bp-sm:p-6">
                <form id="review-write-form" onSubmit={onSubmit} className="space-y-7">
                  {state !== "ok" && (
                    <ResultState status={state === "error" || state === "unauthorized" ? "error" : "info"} title={state === "already" ? "이미 작성한 대상입니다." : "후기 작성 상태를 확인해 주세요"} description={stateMessage(state) ?? undefined} className="rounded-panel border border-border bg-muted/30 px-4 py-8">
                  ...
                  {state === "invalid" && !canonicalTarget && <EmptyState title="작성할 후기를 찾을 수 없어요" description="구매확정 또는 이용확정이 완료된 내역에서 후기를 작성할 수 있습니다." action={<Button type="button" variant="outline" onClick={() => confirmLeaveIfDirty(() => router.replace("/mypage?tab=orders"))} className="w-full bp-sm:w-auto">마이페이지에서 확인</Button>} />}

                  <section className="space-y-3"><div><Label className="text-ui-body-lg font-semibold text-foreground">별점</Label><p className="mt-1 text-ui-body-sm text-muted-foreground">이용 경험에 가까운 점수를 선택하세요.</p></div><div className="rounded-panel border border-border bg-background px-3 py-5"><ReviewRatingInput value={rating} onChange={setRating} disabled={locked} /></div></section>
                  <section className="space-y-3"><div className="flex items-end justify-between gap-3"><div><Label className="text-ui-body-lg font-semibold text-foreground">후기 내용</Label><p className="mt-1 text-ui-body-sm text-muted-foreground">직접 경험한 만족도와 도움이 될 정보를 적어주세요.</p></div><span className="text-ui-label text-muted-foreground tabular-nums">{content.length} / {REVIEW_CONTENT_MAX_LENGTH}자</span></div><Textarea value={content} onChange={(e) => setContent(e.target.value)} maxLength={REVIEW_CONTENT_MAX_LENGTH} placeholder={reviewPlaceholder} className="min-h-[180px] resize-y rounded-control border-border bg-background text-foreground focus-visible:ring-2 focus-visible:ring-ring" disabled={locked} />{content.length > 0 && content.trim().length < REVIEW_CONTENT_MIN_LENGTH && <p className="text-ui-label text-warning">후기 내용은 5자 이상 입력해 주세요.</p>}{content.length > REVIEW_CONTENT_MAX_LENGTH - 20 && <p className="text-ui-label text-warning">최대 글자 수에 가까워졌어요.</p>}</section>
                  <section className="space-y-3"><div><Label className="text-ui-body-lg font-semibold text-foreground">사진 첨부</Label><p className="mt-1 text-ui-body-sm text-muted-foreground">선택 사항이며 최대 5장까지 등록할 수 있습니다.</p></div><div className="rounded-panel border border-border bg-background p-4"><PhotosUploader variant="dropzone" value={photos} onChange={setPhotos} max={5} onUploadingChange={setIsUploading} onUploaded={photoSession.registerUploadedUrls} onRemove={photoSession.removeUploadedUrl} uploadSessionId={photoSession.uploadSessionId} previewMode="queue" disabled={locked || isUploading || !photoSession.uploadSessionId} /><PhotosReorderGrid value={photos} onChange={setPhotos} onRemove={(url) => { const sessionId = photoSession.uploadSessionId; if (sessionId) void photoSession.removeUploadedUrl(url, sessionId); }} disabled={locked || isUploading} mobileControls responsiveColumns />{isUploading && <div className="mt-2 text-ui-label text-muted-foreground" aria-live="polite">이미지 업로드 중...</div>}</div></section>
                  <div className="flex flex-col-reverse gap-2 border-t border-border pt-5 bp-sm:flex-row bp-sm:justify-end"><Button type="button" variant="outline" onClick={() => confirmLeaveIfDirty(goPrimary)} className="h-11 w-full overflow-hidden whitespace-nowrap rounded-control bg-transparent bp-sm:w-auto">{reviewDestination?.label ?? "후기 관리로 이동"}</Button><Button data-cy="submit-review" type="submit" variant="highlight" disabled={locked || invalidForm || isUploading} aria-disabled={locked || invalidForm || isUploading} className="hidden h-11 w-full overflow-hidden whitespace-nowrap rounded-control font-semibold bp-lg:inline-flex bp-lg:w-auto">{isSubmitting ? "등록 중" : isUploading ? "이미지 업로드 중..." : "후기 등록"}</Button></div>
                </form>
              </CardContent>
            </Card>
            <ReviewWriteChecklist targetReady={targetVerified} ratingReady={ratingReady} contentReady={contentReady} photosCount={photos.length} isUploading={isUploading} className="hidden bp-lg:sticky bp-lg:top-24 bp-lg:block" />
          </div>
        </div>
      </SiteContainer>
      <ReviewWriteMobileBar locked={locked} invalidForm={invalidForm} isUploading={isUploading} isSubmitting={isSubmitting} />
    </div>
  );
}
