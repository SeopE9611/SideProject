        ...
        <section className="space-y-6">
          ...
          <div className="grid gap-4 bp-lg:grid-cols-3">
            {Array.from({ length: 3 }).map((_, index) => (
              <div
                key={index}
                className="rounded-panel border border-border/80 bg-card p-5 shadow-soft sm:p-6"
              >
        ...
        <section className="space-y-6">
          ...
          <div className="grid overflow-hidden rounded-panel border border-border/80 bg-card shadow-soft bp-lg:grid-cols-2">
            {Array.from({ length: 2 }).map((_, index) => (
              <div
                key={index}
                className="flex gap-4 border-b border-border p-5 last:border-b-0 bp-lg:border-b-0 bp-lg:p-6"
              >
...
function ClassCardSkeleton() {
  return (
    <div className="rounded-panel border border-border/80 bg-card p-5 shadow-soft sm:p-6">
      ...
      <div className="mt-4 space-y-3 border-y border-border py-3">
        <Skeleton className="h-4 w-full" />
        <Skeleton className="h-4 w-5/6" />
        <Skeleton className="h-4 w-full" />
