...
export default function RacketCareRegistrationDialog(props: {
  ...
  const visibleSteps = editing
    ? [
        { actualStep: 2, label: "라켓 정보" },
        { actualStep: 3, label: "관리 기준" },
      ]
    : [
        { actualStep: 1, label: "등록 방식" },
        { actualStep: 2, label: "라켓 정보" },
        { actualStep: 3, label: "관리 기준" },
      ];
  const editImportCandidates = useMemo(
    () => importCandidates.filter((candidate) => Boolean(candidate.latestCompletedApplication?.id)),
    [importCandidates],
  );
  const clearHistoryLink = (nextForm: CareForm) => {
    setHistoryLinkIntent("clear");
    setSelectedImportCandidateId(null);
    setForm({ ...nextForm, latestCompletedApplicationId: "" });
  };
  const applyCreateCandidate = (candidate: RacketCareImportCandidate) => {
    setSelectedImportCandidateId(candidate.id);
    setForm(formFromCandidate(candidate));
  };
  const applyEditCandidate = (candidate: RacketCareImportCandidate) => {
    if (!candidate.latestCompletedApplication?.id || !candidate.lastStringingAt) return;
    setHistoryLinkIntent("replace");
    setSelectedImportCandidateId(candidate.id);
    setForm({
      ...form,
  ...
  return (
    ...
    >
      <DialogContent
        className="grid max-h-[min(720px,calc(100dvh-2rem))] grid-rows-[auto_minmax(0,1fr)_auto] overflow-hidden"
          ...
          <div className={`mt-3 grid gap-2 ${editing ? "bp-sm:grid-cols-2" : "bp-sm:grid-cols-3"}`}>
            {visibleSteps.map(({ actualStep, label }, index) => {
              ...
              return (
                <div
                  key={label}
                  className="rounded-control border border-border bg-card p-3 data-[active=true]:border-brand-highlight data-[active=true]:bg-brand-highlight-muted"
                  data-active={step === actualStep}
                >
                  <span className="flex items-center gap-2 text-ui-label text-muted-foreground">
                    <span
                      className="grid h-6 w-6 place-items-center rounded-full bg-muted text-ui-micro data-[active=true]:bg-brand-highlight data-[active=true]:text-brand-highlight-foreground"
                      data-active={step === actualStep}
                    >
        ...
        <div className="min-h-0 overflow-y-auto pr-1">
          {!editing && step === 1 ? (
            <div className="grid gap-3 bp-sm:grid-cols-2">
              <Button
                ...
                onClick={() => {
                  ...
                  if (importCandidates[0]) {
                    setSelectedImportCandidateId(importCandidates[0].id);
                    setForm(formFromCandidate(importCandidates[0]));
                  }
                  setStep(2);
                }}
              >
                <span>
                  <span className="block font-semibold">기존 정보에서 가져오기</span>
                  <span className="mt-1 block text-ui-label opacity-70">
              ...
              >
                <span>
                  <span className="block font-semibold">직접 입력하기</span>
                  <span className="mt-1 block text-ui-label opacity-70">
                    라켓과 관리 기준을 직접 작성합니다.
                  </span>
                </span>
          ...
          {step === 2 ? (
            <div className="space-y-4">
              {editing ? (
                <div
                  id="latestCompletedApplicationId"
                  tabIndex={-1}
                  className="space-y-3 rounded-panel border border-border bg-card p-4 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                >
                  <div>
                    <h3 className="font-semibold">완료된 교체 이력 불러오기</h3>
                    <p className="mt-1 text-ui-label text-muted-foreground">
                      이전에 완료한 교체서비스의 날짜와 스트링 정보를 적용할 수 있어요.
                    </p>
                    <p className="mt-1 text-ui-label text-muted-foreground">
                      교체일과 스트링 정보만 적용되며, 라켓 별칭과 기본 정보는 유지됩니다.
                    </p>
                  </div>
                  {editing.lastApplicationId && historyLinkIntent === "keep" ? (
                    <Badge variant="success">현재 완료된 교체 이력과 연결되어 있습니다.</Badge>
                  ) : null}
                  {historyLinkIntent === "replace" ? (
                    <Badge variant="info">
                      선택한 완료 이력으로 교체일과 스트링 정보가 변경됩니다.
                    </Badge>
                  ) : null}
                  {historyLinkIntent === "clear" ? (
                    editing.lastApplicationId ? (
                      <Badge variant="warning">
                        직접 입력한 값으로 저장하면 기존 완료 이력 연결이 해제됩니다.
                      </Badge>
                    ) : (
                      <Badge variant="secondary">직접 입력한 값으로 저장됩니다.</Badge>
                    )
                  ) : null}
                  {errors.latestCompletedApplicationId ? (
                        ...
                        return (
                          ...
                          >
                            <span className="font-medium">
                              {c.nickname || `${c.racket.brand} ${c.racket.model}`.trim()}
                            </span>
          ...
          {step === 3 ? (
            <div className="grid gap-3">
              ...
              <Card className="rounded-control border-brand-highlight/30 bg-brand-highlight-muted">
                <CardContent className="p-3 text-ui-body-sm">
                  {estimatedDate ? (
                    <>
