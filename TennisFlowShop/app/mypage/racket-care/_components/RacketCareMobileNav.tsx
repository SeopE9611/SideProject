...
export default function RacketCareMobileNav({ item }: { item: CareItem }) {
  const [active, setActive] = useState<(typeof anchors)[number]["id"]>("workspace");
  ...
  return <nav className="fixed inset-x-3 bottom-3 z-40 rounded-panel border border-border bg-card/95 p-2 shadow-float backdrop-blur bp-lg:hidden" style={{ paddingBottom: "calc(0.5rem + env(safe-area-inset-bottom))" }} aria-label="라켓 케어 바로가기"><div className="grid grid-cols-4 gap-1 text-ui-label">{anchors.slice(0, 2).map((anchor) => { const Icon = anchor.icon; return <a key={anchor.id} className="grid min-h-11 place-items-center rounded-control px-1 py-2 text-center data-[active=true]:bg-brand-highlight-muted data-[active=true]:text-brand-highlight-foreground" href={anchor.href} onClick={scrollTo(anchor.href, anchor.id)} data-active={active === anchor.id} aria-current={active === anchor.id ? "location" : undefined}><Icon className="h-4 w-4" /><span>{anchor.label}</span></a>; })}<Link className="grid min-h-11 place-items-center rounded-control px-1 py-2 text-center" href={recommendHref}><Sparkles className="h-4 w-4" /><span>맞춤 추천</span></Link>{anchors.slice(2).map((anchor) => { const Icon = anchor.icon; return <a key={anchor.id} className="grid min-h-11 place-items-center rounded-control px-1 py-2 text-center data-[active=true]:bg-brand-highlight-muted data-[active=true]:text-brand-highlight-foreground" href={anchor.href} onClick={scrollTo(anchor.href, anchor.id)} data-active={active === anchor.id} aria-current={active === anchor.id ? "location" : undefined}><Icon className="h-4 w-4" /><span>{anchor.label}</span></a>; })}</div></nav>;
}
