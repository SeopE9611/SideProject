...
function recommendHref(item: CareItem) {
  return `/products/recommend?from=racket-care&careItemId=${item.id}&freq=${item.playFrequency}`;
}
...
export default function RacketCareHero({
  selected,
  onCreate,
}: {
  selected: CareItem | null;
  onCreate: (options?: StartCreateOptions) => void;
}) {
  const meta = selected ? stateMeta[selected.careStatus.state] : null;
  return (
    <section
      id="racket-care-hero"
      className="relative overflow-hidden rounded-hero border border-border bg-card p-5 shadow-soft bp-sm:p-8 bp-lg:p-12"
    >
      ...
      <div className="relative grid gap-9 bp-lg:grid-cols-[minmax(0,1.08fr)_minmax(380px,0.92fr)] bp-lg:items-center">
        <div className="space-y-6">
          ...
          <p className="max-w-xl break-keep text-ui-body text-muted-foreground">
            마지막 교체일과 플레이 빈도만 연결하면 스트링 상태, 교체 시점, 맞춤 추천까지 한 화면에서
            이어집니다.
          </p>
        ...
        <div className="relative">
          ...
          {selected ? (
            <>
              <Card
                variant="feature"
                className="relative mx-auto max-w-md rounded-panel shadow-float"
              >
                <CardContent className="p-5 bp-sm:p-6">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <p className="break-words text-ui-label text-muted-foreground">
                        {selected.racket.brand} {selected.racket.model}
                      </p>
                    </div>
                    {meta ? (
                  ...
                  <div className="mx-auto mt-6 grid w-full max-w-[230px] place-items-center">
                    <div
                      role="progressbar"
                      aria-label={`스트링 상태 점수 ${selected.careStatus.lifeScore}점`}
                      aria-valuemin={0}
                      aria-valuemax={100}
                      aria-valuenow={selected.careStatus.lifeScore}
                      className="grid aspect-square w-full place-items-center rounded-full border border-border shadow-soft"
                      style={{
                        background: `conic-gradient(hsl(var(--brand-highlight)) ${selected.careStatus.lifeScore}%, hsl(var(--muted)) 0)`,
                      }}
                    >
                  ...
                  <p className="mt-6 text-center text-ui-body-sm text-muted-foreground">
                    ...
                    <span className="font-semibold text-foreground">
                      {dateLabel(selected.careStatus.nextRecommendedAt)}
                    </span>
                  </p>
                </CardContent>
              </Card>
              <div className="relative mt-3 grid grid-cols-3 gap-2">
                <Card variant="floating" className="min-w-0">
                  <CardContent className="h-full p-2.5 bp-sm:p-3">
                    <p className="text-ui-micro text-muted-foreground">D-day</p>
                    <p className="mt-1 min-w-0 break-words text-ui-label font-semibold leading-tight">
                      {dday(selected.careStatus.daysRemaining)}
                    </p>
                  </CardContent>
                </Card>
                <Card variant="floating" className="min-w-0">
                  <CardContent className="h-full p-2.5 bp-sm:p-3">
                    <p className="text-ui-micro text-muted-foreground">플레이 빈도</p>
                    <p className="mt-1 min-w-0 break-words text-ui-label font-semibold leading-tight">
                      {freqLabels[selected.playFrequency] ?? selected.playFrequency}
                    </p>
                  </CardContent>
                </Card>
                <Card variant="floating" className="min-w-0">
                  <CardContent className="h-full p-2.5 bp-sm:p-3">
                    <p className="text-ui-micro text-muted-foreground">최근 스트링</p>
                    <p className="mt-1 min-w-0 line-clamp-2 break-words text-ui-label font-semibold leading-tight">
                      {selected.stringSnapshot?.name || "기록 없음"}
                    </p>
                  </CardContent>
                </Card>
              </div>
            ...
            <Card
              ...
              className="relative mx-auto max-w-md rounded-panel shadow-float"
            >
              <CardContent className="grid gap-4 p-6 text-center bp-sm:p-8">
                <div className="mx-auto grid h-24 w-24 place-items-center rounded-full bg-brand-highlight-muted text-brand-highlight-ink">
                ...
                <p className="break-keep text-ui-body-sm text-muted-foreground">
                  프로필과 완료된 교체 이력을 가져와 더 빠르게 시작할 수 있어요.
                </p>
              </CardContent>
            </Card>
          )}
