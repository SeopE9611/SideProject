...
export default function OrderShippingInfoDialog({
  ...
  const { data, isLoading } = useSWR<OrderDetail>(
    open ? `/api/orders/${orderId}` : null,
            ...
            {isVisitPickup ? (
              <>
                <p className="text-muted-foreground">
                  방문 수령 주문입니다. 방문 일정/상태는 주문 상세에서
                  확인해주세요.
                </p>
                <p className="text-muted-foreground">
                  운송장 정보가 없는 것이 정상일 수 있습니다.
                </p>
              </>
            ) : (
              <>
                ...
                <p className="text-muted-foreground">
                  관리자가 운송장 입력 후 배송 상태를 변경하면 이곳에서 확인할
                  수 있습니다.
                </p>
              </>
            )}
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1 text-sm">
              ...
              <div className="text-muted-foreground">
                {[data?.shippingInfo?.name, data?.shippingInfo?.phone]
                  .filter(Boolean)
                  .join(" / ") || "-"}
              </div>
            </div>
