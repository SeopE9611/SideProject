...
const LIMIT = 5;
const OrderShippingInfoDialog = dynamic(() => import("./_components/OrderShippingInfoDialog"), {
  loading: () => null,
});
...
function groupAmount(g: ActivityGroup) {
  ...
  if (g.kind === "rental")
    return typeof g.rental?.totalAmount === "number" ? g.rental?.totalAmount : null;
  return null;
}
...
function needsAction(g: ActivityGroup) {
  ...
  if (g.kind === "order" && isTerminalCanceledStatus(g.order?.status)) {
    return false;
  }

  if (g.kind === "rental" && isTerminalCanceledStatus(g.rental?.status)) {
    return false;
  }

  if (g.kind === "application" && isTerminalCanceledStatus(app.status)) {
    return false;
  }

  return isApplicationTrackingNeeded(app);
}
...
export default function ActivityFeed() {
  ...
  const linksOf = (g: ActivityGroup) => {
    ...
    const detailHref = hasOrderLinkedApplication
      ...
      : g.kind === "order"
        ? `/mypage?tab=orders&flowType=order&flowId=${g.order?.id}&from=orders`
        : g.kind === "rental"
    ...
    const shippingHref = appForShipping
      ...
      : "#";
    const shippingLabel =
      appForShipping && appForShipping.hasTracking ? "운송장 수정" : "운송장 등록";
              ...
              <div className="space-y-3">
                {actionTop.map((g) => {
                  ...
                  const showMore = Boolean(
                    appId && (canShowStringingReviewCta(app) || canShowStringingConfirmCta(app)),
                  );
            ...
            <div className="min-w-0 rounded-2xl bg-muted/30 p-4 bp-sm:p-6 border border-border/50 slide-up">
              ...
              <div className="space-y-3">
                {activeTop.map((g) => {
                  ...
                  const date = groupDate(g);
                  const meta = compactPills(metaPills(g), 3);
                  const { detailHref, detailLabel } = linksOf(g);
                  const app = g.application;
