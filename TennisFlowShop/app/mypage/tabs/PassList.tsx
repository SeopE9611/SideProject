...
type PassItem = {
  ...
  status: 'active' | 'expired' | 'suspended' | 'paused' | 'cancelled' | 'pending_payment' | 'pending_activation';
  purchasedAt: string;
...
export default function PassList() {
  ...
  const statusBadge = (p: PassItem) => {
    if (p.status === 'active') return p.isExpiringSoon ? <Badge variant="destructive">만료 임박</Badge> : <Badge>{statusLabel(p.status)}</Badge>;
    if (p.status === 'pending_payment') return <Badge variant="secondary">{statusLabel(p.status)}</Badge>;
    if (p.status === 'pending_activation') return <Badge variant="outline">{statusLabel(p.status)}</Badge>;
    if (p.status === 'suspended' || p.status === 'paused') return <Badge variant="secondary">{statusLabel(p.status)}</Badge>;
  ...
  const renderCard = (p: PassItem) => {
    const remainPct = p.packageSize > 0 ? Math.max(0, Math.min(100, (p.remainingCount / p.packageSize) * 100)) : 0;
    const dday = now && p.expiresAt ? Math.ceil((new Date(p.expiresAt).getTime() - now) / 86400000) : null;
