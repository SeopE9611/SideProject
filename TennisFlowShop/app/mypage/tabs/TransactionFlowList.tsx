...
export default function TransactionFlowList() {
  ...
  const handleApplicationCancelRequest = async (params: {
    reasonCode: string;
    reasonText?: string;
    ...
    <div className="space-y-4">
          ...
          {FLOW_SCOPE_OPTIONS.map((option) => {
            ...
            return (
              ...
              >
                ...
                {isActive && (
                  <span className="absolute -bottom-1 left-1/2 h-0.5 w-6 -translate-x-1/2 rounded-full bg-primary-foreground/50" />
                )}
              </button>
            );
          })}
      ...
      {scope === "todo" ? (
        <p className="text-xs text-muted-foreground">
          해야 할 일은 구매확정·운송장 등록·교체확정·리뷰 작성처럼 지금 바로
          처리할 항목만 모아 보여줍니다.
        </p>
      ) : null}
      <p className="text-xs text-muted-foreground">
      ...
      {items.length === 0 ? (
        ...
        items.map((g) => {
          ...
          const applicationId =
            g.application?.id ??
            (g.kind === "application" ? g.detailTarget.id : undefined);
          const linkedApps =
            g.kind === "order"
          ...
          const needsTrackingAction = Boolean(
            applicationActionTarget?.needsInboundTracking &&
          ...
          const todoPrimaryReason =
            scope === "todo" ? getTodoPrimaryReason(g) : null;
          ...
          const isApplicationActionContext =
            ...
            (isDirectApplicationCard || scope === "todo");
                    ...
                    <>
                      ...
                      <div className="flex items-center gap-3 rounded-lg bg-muted p-3">
                        ...
                        <div>
                          ...
                          <p className="font-medium text-foreground">
                            {getMypageUserStatusLabel(g.order?.status)}
                          </p>
                        </div>
                      </div>
                      <div className="flex items-center gap-3 rounded-lg bg-muted p-3">
                ...
                <div className="flex flex-wrap items-center justify-end gap-2 border-t border-border/60 pt-3 md:pt-4">
                  {(() => {
                    ...
                    const detailPriority =
                      scope === "todo" || prefersApplicationView ? 10 : 1;
                    actions.push({
                      key: "flow-detail",
                      ...
                      if (orderId) {
                        actions.push({
                          ...
                          node: (
                            ...
                            >
                              <Link
                                href={`/mypage?tab=orders&flowType=order&flowId=${orderId}&${flowQuery}`}
                              >
                                연계 주문 보기
                              </Link>
                            </Button>
                      ...
                      if (rentalId) {
                        actions.push({
                          ...
                          node: (
                            ...
                            >
                              <Link
                                href={`/mypage?tab=orders&flowType=rental&flowId=${rentalId}&${flowQuery}`}
                              >
                                연계 대여 보기
                              </Link>
                            </Button>
                      ...
                      if (primaryLinkedApplicationId) {
                        actions.push({
                          ...
                          node: (
                            ...
                            >
                              <Link
                                href={`/mypage?tab=orders&flowType=application&flowId=${primaryLinkedApplicationId}&${flowQuery}`}
                              >
                                교체서비스 보기
                              </Link>
                            </Button>
                      ...
                      if (g.rental?.stringingApplicationId) {
                        ...
                        actions.push({
                          ...
                          node: (
                            ...
                            >
                              <Link
                                href={`/services/apply?rentalId=${rentalId}`}
                              >
                                교체 신청하기
                              </Link>
                            </Button>
                    ...
                    if (
                      ...
                      actions.push({
                        ...
                        node: (
                          ...
                          >
                            <Link
                              href={`/mypage?tab=orders&flowType=application&flowId=${actionableApplicationId}&${flowQuery}`}
                            >
                              교체서비스 보기
                            </Link>
                          </Button>
                    ...
                    const pinnedInline = sortedActions.filter(
                      (a) => a.pinInline,
                    );
                    const nonPinned = inlineEligible.filter(
                      (a) => !a.pinInline,
                    );
      ...
      `}
                          ...
                          >
                            <Sparkles
                              className={`h-3.5 w-3.5 transition-all duration-200 ${isSecondaryOpen ? "text-primary" : "text-muted-foreground/70 group-hover:text-primary/70"}`}
                            />
                            <span>
                              {isSecondaryOpen
                                ? "기타 작업 닫기"
