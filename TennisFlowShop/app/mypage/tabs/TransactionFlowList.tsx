...
const FLOW_SCOPE_OPTIONS: Array<{ value: FlowScope; label: string; icon: typeof Package; description: string }> = [
  { value: 'all', label: '전체', icon: Package, description: '모든 거래 내역' },
  { value: 'todo', label: '해야 할 일', icon: ListChecks, description: '처리 필요 항목' },
  { value: 'order', label: '주문', icon: ShoppingBag, description: '상품 주문 내역' },
  { value: 'application', label: '서비스 신청', icon: Wrench, description: '교체서비스 신청' },
  { value: 'rental', label: '대여', icon: Calendar, description: '라켓 대여 내역' },
];
          ...
          {FLOW_SCOPE_OPTIONS.map((option) => {
            ...
            return (
              ...
              >
                ...
                {isActive && <span className="absolute -bottom-1 left-1/2 h-0.5 w-6 -translate-x-1/2 rounded-full bg-primary-foreground/50" />}
              </button>
            );
          })}
        </div>
