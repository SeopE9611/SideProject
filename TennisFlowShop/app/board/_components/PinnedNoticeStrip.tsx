...
const pinnedNoticeMobileTitleClampClass = 'line-clamp-2 text-[13px] font-medium leading-snug text-foreground group-hover:text-primary sm:line-clamp-1';
...
export default function PinnedNoticeStrip({ items }: { items: PinnedNoticeItem[] }) {
  ...
  return (
    <section className="rounded-lg border border-primary/25 bg-primary/[0.04] px-3 py-2.5 sm:px-3.5" aria-label="운영 공지">
      <div className="flex items-center gap-2">
        ...
        <div className="min-w-0">
          ...
          <p className="text-[10px] text-muted-foreground">운영팀이 고정한 공지입니다. 게시판 이용 전 한 번 확인해 주세요.</p>
        </div>
      </div>
        ...
        {items.map((notice) => (
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-0 text-[10px] leading-tight text-muted-foreground">{fmtDate(notice.createdAt)}</p>
            </div>
          </Link>
        ))}
      </div>
