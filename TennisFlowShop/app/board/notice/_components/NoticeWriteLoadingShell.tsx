  ...
  return (
    <main className="min-h-screen bg-background text-foreground" aria-busy="true" aria-live="polite">
      <PublicPageHero
        ...
        description={isEventMode ? "할인, 프로모션, 행사 소식을 회원들에게 전달하세요." : "중요한 소식을 회원들에게 전달하세요."}
        actions={
          ...
          <CardContent className="space-y-5 p-4 md:p-8">
            {Array.from({ length: 4 }).map((_, index) => (
              <PublicSurface key={index} variant="feature" padding="md" className="space-y-3">
                ...
                <Skeleton className={index === 1 ? "h-48 w-full rounded-control" : "h-12 w-full rounded-control"} />
              </PublicSurface>
            ))}
          </CardContent>
        </Card>
