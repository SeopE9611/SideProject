...
import {
  ...
  isMarketBrandCategory,
  isValidMarketBrandForCategory,
...
export default function FreeBoardEditClient({ id }: Props) {
  ...
  useEffect(() => {
    if (data && data.ok) {
      ...
      const defaultMarketMeta: MarketMeta = {
        price: null,
        saleStatus: "selling",
        conditionGrade: "B",
        conditionNote: "",
  ...
  const priceLabel =
    typeof marketMeta.price === "number" && marketMeta.price > 0
      ? `${marketMeta.price.toLocaleString("ko-KR")}원`
      : "-";
  const saleStatusLabel =
    marketMeta.saleStatus === "selling"
      ? "판매중"
      : marketMeta.saleStatus === "reserved"
        ? "예약중"
        : "판매완료";
                      ...
                      <Button
                        ...
                        onClick={async () => {
                          ...
                          try {
                            ...
                            const nextContent = item.content ?? "";
                            const nextImages = Array.isArray(item.images) ? item.images : [];
                            const nextCategory = ((item.category as any) ?? "racket") as any;
                            const nextBrand = typeof item.brand === "string" ? item.brand : "";
                            ...
                            const nextMarketMetaRaw = (item as any).marketMeta ?? defaultMarketMeta;

                            const nextMarketMeta =
                              normalizeMarketMeta(nextCategory, nextMarketMetaRaw) ??
                              defaultMarketMeta;
                            ...
                            baselineRef.current = {
                              ...
                              brand: nextBrand,
                              imagesJson: JSON.stringify(nextImages),
                              marketMetaJson: JSON.stringify(nextMarketMeta),
                            };
                            ...
                            console.error("최신 중고거래 글 재조회 실패", error);
...
/>
                ...
                {/* 모바일/태블릿 하단 버튼 */}
                <div className="flex flex-col justify-end gap-2 bp-sm:flex-row bp-sm:flex-wrap lg:hidden">
                  <Button
                    type="button"
                    variant="outline"
                    size="sm"
                ...
                <div className={cn("space-y-4", isCompactSticky && "space-y-3")}>
                  ...
                  <div className="rounded-panel border border-border bg-card shadow-soft">
                    <button
                      ...
                      className={cn(
                        "flex w-full items-center justify-between border-b border-border px-5 py-3 text-left",
                        isCompactSticky && "px-4 py-2.5",
                      )}
                    >
