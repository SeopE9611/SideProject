  ...
  return (
    <Card className="border border-border bg-card shadow-sm">
      <CardHeader className="space-y-3 p-4 sm:p-5 md:p-6">
        <CardTitle className="text-xl sm:text-2xl font-semibold">자주 묻는 질문</CardTitle>
        <p className="text-sm text-muted-foreground">주문, 결제, 배송, 교체서비스 관련 자주 묻는 내용을 먼저 확인해보세요.</p>
        <Input
          type="text"
          value={searchQuery}
        ...
        <div className="flex flex-wrap gap-2">
          {FAQ_CATEGORIES.map((category) => {
            ...
            return (
              ...
              >
                <Badge variant={isActive ? "brand" : "outline"} className="px-3 py-1 text-xs sm:text-sm">
                  {category}
                </Badge>
              </button>
            );
      ...
      <CardContent className="p-4 pt-0 sm:p-5 sm:pt-0 md:p-6 md:pt-0 space-y-4">
        {filteredFaqs.length === 0 ? (
          ...
          <Accordion type="single" className="rounded-lg border border-border px-4">
            {filteredFaqs.map((faq) => (
              <AccordionItem key={faq.id} value={faq.id}>
                <AccordionTrigger value={faq.id} className="gap-3 py-4">
                  <span className="flex min-w-0 flex-col items-start gap-2">
                    <Badge variant="outline" className="px-2 py-0.5 text-xs">
                      {faq.category}
                    </Badge>
                    <span className="text-sm font-medium leading-snug sm:text-base">Q. {faq.question}</span>
                  </span>
                </AccordionTrigger>
                <AccordionContent value={faq.id} className="pb-4 text-sm text-muted-foreground leading-relaxed">
                  A. {faq.answer}
                </AccordionContent>
              </AccordionItem>
            ))}
        ...
        <div className="rounded-lg border border-border bg-muted/20 px-4 py-4">
          <p className="text-sm text-foreground">원하는 답변을 찾지 못했다면 Q&A 문의로 남겨주세요.</p>
