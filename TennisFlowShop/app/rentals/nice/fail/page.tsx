...
const FAIL_GUIDE_MAP: Record<string, { title: string; description: string[]; accent?: "default" | "warning" }> = {
  PAYMENT_PROVIDER_DISABLED: {
    ...
    description: ["운영 설정상 카드/간편결제가 비활성화되어 있어 결제를 진행할 수 없어요.", "다른 결제수단을 이용하거나 관리자에게 문의해주세요."],
  },
  INVALID_QUERY: {
    title: "결제 결과 정보가 올바르지 않아요",
    description: ["결제 결과를 확인하는 중 필수 값이 누락되었어요.", "대여 페이지로 돌아가 다시 결제를 진행해주세요."],
  },
  AUTH_FAILED: {
  ...
  AMOUNT_MISMATCH: {
    ...
    description: ["결제 금액이 대여 정보와 일치하지 않아 결제가 중단되었어요.", "대여 페이지에서 금액을 다시 확인한 뒤 시도해주세요."],
  },
  SESSION_NOT_FOUND: {
    title: "결제 세션을 찾지 못했어요",
    description: ["결제 준비 정보가 없어 결제를 완료할 수 없어요.", "대여 페이지에서 다시 결제를 진행해주세요."],
  },
  SESSION_EXPIRED: {
    title: "결제 유효시간이 만료되었어요",
    description: ["결제 준비 시간이 지나 결제를 이어서 진행할 수 없어요.", "대여 페이지로 돌아가 다시 결제를 시도해주세요."],
  },
  ORDER_CREATION_FAILED_AFTER_PAYMENT_APPROVE: {
    title: "결제 후 대여 주문 처리 중 문제가 발생했어요",
    description: ["결제 승인은 완료됐지만 대여 주문 생성 단계에서 문제가 발생했어요.", "중복 결제를 피하기 위해 대여 내역 또는 관리자 확인이 필요합니다."],
    accent: "warning",
  },
  UNKNOWN: {
...
export default async function RentalNiceFailPage({
  ...
  const checkoutFallback = candidateId && ID_RE.test(candidateId) ? `/rentals/${encodeURIComponent(candidateId)}/checkout` : null;
  const fallbackRaw = (sp.fallback || "").trim();
  const fallbackHref = checkoutFallback || (fallbackRaw.startsWith("/rentals/") || fallbackRaw === "/rentals" ? fallbackRaw : "/rentals");
    ...
    <div className="mx-auto flex min-h-[60vh] w-full max-w-2xl items-center px-6">
      <Card className="w-full">
        ...
        <CardContent className="space-y-4">
          ...
          {guide.accent === "warning" && <div className="rounded-md border border-warning/30 bg-warning/10 px-3 py-2 text-sm text-warning dark:bg-warning/15">중복 결제를 막기 위해 반복 결제를 피하고, 대여 내역 또는 관리자 확인 후 진행해주세요.</div>}
