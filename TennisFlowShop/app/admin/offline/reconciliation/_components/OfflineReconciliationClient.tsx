  ...
  return (
    <div
      className={cn(
        ...
        (!tone || tone === "muted") && "border-border/60 bg-muted/20",
      )}
    >
        ...
        <CardContent className="space-y-4">
          <div className="grid gap-3 md:grid-cols-5">
            <div className="space-y-1.5">
              ...
              <Select id="type" value={filters.type} onChange={(e) => setFilters((prev) => ({ ...prev, type: e.target.value as TypeFilter }))}>
            ...
            <div className="space-y-1.5">
              ...
              <Select id="status" value={filters.status} onChange={(e) => setFilters((prev) => ({ ...prev, status: e.target.value as StatusFilter }))}>
            ...
            <div className="flex items-end gap-2">
              ...
              <Button
                ...
                variant="outline"
                onClick={() => {
                  const empty = { type: "all" as TypeFilter, status: "open" as StatusFilter, from: "", to: "" };
                  setFilters(empty);
                  setSubmitted(empty);
                  setPage(1);
                }}
              ...
              <table className="w-full min-w-[1100px] text-sm">
                <thead className="border-b text-left text-xs text-muted-foreground">
                  <tr>
                    ...
                    <th className="py-3 pr-3">발생일</th>
                    <th className="py-3 pr-3">고객</th>
                    <th className="py-3 pr-3">내용</th>
                    <th className="py-3 pr-3">금액/패키지명</th>
                    <th className="py-3 pr-3">에러/사유</th>
                  ...
                  {data!.items.map((item) => {
                    ...
                    return (
                      <tr key={`${item.type}-${item.id}`} className={cn(updatingId === item.id && "opacity-60")}>
                        ...
                        <td className="py-4">
                          <ItemActions item={item} note={note} setNote={(value) => setNotes((prev) => ({ ...prev, [item.id]: value }))} onUpdate={updateItem} />
                        </td>
                      </tr>
                    );
                  })}
