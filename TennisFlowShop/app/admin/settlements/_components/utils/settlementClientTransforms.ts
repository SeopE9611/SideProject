...
export function isSettlementMatched(
  row: SettlementSnapshot,
  live: SettlementLiveResponse,
): boolean {
  ...
  const orderOk =
    (row.breakdown?.orders || 0) === (live.breakdown?.orders || 0);
  const applicationOk =
    (row.breakdown?.applications || 0) === (live.breakdown?.applications || 0);
  const packageOk =
    (row.breakdown?.packages || 0) === (live.breakdown?.packages || 0);
  return paidOk && refundOk && netOk && orderOk && applicationOk && packageOk;
}

/** YYYYMM 클라이언트 검증: 6자리·월범위·미래금지 */
export function validateYyyymmClient(
  ym: string,
  nowYyyymm = nowYyyymmKst(),
): { ok: true } | { ok: false; reason: string } {
  if (!/^\d{6}$/.test(ym))
    return { ok: false, reason: "YYYYMM 6자리로 입력하세요." };
  const mm = Number(ym.slice(4, 6));
  if (mm < 1 || mm > 12)
    return { ok: false, reason: "월은 01~12만 가능합니다." };
  if (Number(ym) > Number(nowYyyymm))
    return { ok: false, reason: "미래 월은 생성할 수 없습니다." };
  return { ok: true };
}
...
export function nowYyyymmKst(baseDate: Date = new Date()): string {
  ...
  const parts = Object.fromEntries(
    fmt.formatToParts(baseDate).map((p) => [p.type, p.value]),
  );
  return `${parts.year}${parts.month}`;
}

export function toggleYyyymmSelection(
  current: Set<string>,
  yyyymm: string,
): Set<string> {
...
export function buildAllSettlementSelection(
  rows: SettlementSnapshot[],
): Set<string> {
