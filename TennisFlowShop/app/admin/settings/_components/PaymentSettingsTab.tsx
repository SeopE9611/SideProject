...
export function PaymentSettingsTab({
  ...
  error: TabErrorState;
  paymentMeta: { hasPaypalSecret: boolean; hasStripeSecretKey: boolean };
}) {
  return (
    <TabsContent value="payment">
      <Card>
        <CardHeader>
          ...
          <CardDescription>결제 수단 및 키를 관리합니다.</CardDescription>
        </CardHeader>
        {error.message && (
        ...
        <form onSubmit={form.handleSubmit(onSubmit)}>
          <CardContent className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              <Label>페이팔</Label>
              <Switch
                checked={form.watch("enablePaypal")}
                onCheckedChange={(v) =>
                  form.setValue("enablePaypal", v, { shouldDirty: true })
                }
              />
            </div>
            <div>
              ...
              <Input
                id="paypalSecret"
                placeholder={
                  paymentMeta.hasPaypalSecret ? "기존 시크릿 유지 중" : ""
                }
