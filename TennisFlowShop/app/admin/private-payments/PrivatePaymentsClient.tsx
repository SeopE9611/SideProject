...
type Item = { id: string; title: string; amount: number; description?: string; customerName?: string; customerPhone?: string; customerEmail?: string; status: string; paymentStatus: string; createdAt: string };
...
export default function PrivatePaymentsClient() {
  ...
  return <div className="grid gap-4 lg:grid-cols-[380px_1fr]">
    ...
    <Card><CardHeader><CardTitle>개인결제 목록</CardTitle></CardHeader><CardContent className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b text-left"><th className="p-2">결제명</th><th>금액</th><th>고객</th><th>상태</th><th>결제상태</th><th>생성일</th><th>링크 복사</th><th>상세/수정</th></tr></thead><tbody>{items.map((item) => <tr key={item.id} className="border-b"><td className="p-2 font-medium">{item.title}</td><td>{item.amount.toLocaleString()}원</td><td>{item.customerName || "-"}</td><td><Badge variant={item.status === "active" ? "default" : "secondary"}>{item.status}</Badge></td><td><Badge variant={item.paymentStatus === "결제완료" ? "default" : "outline"}>{item.paymentStatus}</Badge></td><td>{new Date(item.createdAt).toLocaleDateString()}</td><td><Button size="sm" variant="outline" onClick={() => copy(item.id)}>복사</Button></td><td><Button size="sm" variant="ghost" onClick={() => edit(item)}>상세/수정</Button></td></tr>)}</tbody></table></CardContent></Card>
  </div>;
}
