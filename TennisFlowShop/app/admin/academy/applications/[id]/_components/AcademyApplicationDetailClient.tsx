            ...
            <AdminPageSection title="선택 클래스 정보" contentClassName="pt-4">
              <div className={`${adminSurface.cardMuted} p-4 ${adminTypography.body}`}>
                ...
                <p className={`mt-2 ${adminTypography.metaMuted}`}>
                  이 신청은 기존 단독 신청으로 생성되어 클래스에 연결되어 있지 않습니다. 등록 확정
                  인원에 집계하려면 클래스를 연결해 주세요.
                </p>
              </div>
            </AdminPageSection>
          )}
          ...
          >
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <Input
                ...
                onChange={(event) =>
                  setEditForm((current) => ({ ...current, applicantName: event.target.value }))
                }
                placeholder="신청자명"
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              >
                <SelectTrigger>
                  <SelectValue placeholder="희망 레슨 유형" />
                </SelectTrigger>
                <SelectContent>
            ...
            <Input
              ...
              onChange={(event) =>
                setEditForm((current) => ({ ...current, preferredTimeText: event.target.value }))
              }
              placeholder="희망 시간대"
              disabled={!canAdminEditApplication || savingEdit}
              maxLength={100}
            ...
            <Button
              ...
              disabled={
                !canAdminEditApplication || savingEdit || editForm.preferredDays.length === 0
              }
            >
              <Save className="mr-2 h-4 w-4" />
                  ...
                  {classOptions.map((classItem) => {
                    ...
                    return (
                      <SelectItem key={classItem._id} value={classItem._id}>
                        {classItem.name} · {getAcademyClassStatusLabel(classItem.status)} ·{" "}
                        {capacity}
          ...
          >
            <div className="space-y-2">
              ...
              <Textarea
                ...
                className="min-h-32"
              />
              <div className={`text-right ${adminTypography.caption}`}>{adminMemo.length}/2000</div>
            </div>
