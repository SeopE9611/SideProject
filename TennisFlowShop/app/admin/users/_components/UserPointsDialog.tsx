...
export default function UserPointsDialog({
  ...
  async function onAdjust(delta: number) {
    ...
    setSubmitting(true);
    try {
      const json = await adminMutator<{ ok?: boolean; error?: string }>(
        ...
        <div className="grid gap-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            <Input
              ...
              placeholder="금액 (예: 1000 또는 -500)"
              inputMode="numeric"
            />
            <Input
              value={reason}
              onChange={(e) => setReason(e.target.value)}
              placeholder="사유(선택) — 예: 이벤트 지급"
            />
          </div>
          ...
          {showAdvanced ? (
            <Input
              ...
              placeholder="중복 방지 키(선택) — 예: order:694e...:manual"
            />
          ) : null}
          ...
          <div className="p-3 border-b flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <Button
                variant="outline"
                size="sm"
                disabled={!shouldShowResolvedPageInfo || page <= 1}
              ...
              <Button
                variant="outline"
                size="sm"
                disabled={!shouldShowResolvedPageInfo || page >= totalPages}
                onClick={() => setPage((p) => p + 1)}
              >
                다음
              </Button>
                ...
                {items?.map((tx) => (
                  ...
                  >
                    <div className="grid gap-1">
                      ...
                      <div className="text-xs text-muted-foreground">
                        {tx.reason && tx.reason.trim().length >= 2
                          ? tx.reason
                          : (fallbackReason(tx.type) ?? "-")}
                      </div>
                      {(() => {
