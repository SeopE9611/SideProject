...
import { Activity, AlertTriangle, Boxes, CircleHelp, ClipboardList, Package, ShoppingCart, Star, Store, TrendingUp, Users, Wrench } from "lucide-react";
...
import { getApplicationStatusBadgeSpec, getOrderStatusBadgeSpec, getPaymentStatusBadgeSpec, getWorkflowMetaBadgeSpec } from "@/lib/badge-style";
  ...
  return (
    <div className="space-y-8 pb-8">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <Card className="border-warning/40 bg-warning/5">
            ...
            <CardContent className="space-y-2 text-sm">
              <div className="flex min-w-0 items-center justify-between gap-2">
                ...
                <Badge className="shrink-0 whitespace-nowrap" variant={data.kpi.queue.cancelRequests > 0 ? "destructive" : "secondary"}>
                  {formatAdminNumber(data.kpi.queue.cancelRequests)}
                </Badge>
              </div>
              <div className="flex min-w-0 items-center justify-between gap-2">
                <span className="min-w-0 break-words">결제 대기 24h+</span>
                <Badge className="shrink-0 whitespace-nowrap" variant={data.kpi.queue.paymentPending24h > 0 ? "destructive" : "secondary"}>
                  {formatAdminNumber(data.kpi.queue.paymentPending24h)}
                </Badge>
              </div>
              <div className="flex min-w-0 items-center justify-between gap-2">
                <span className="min-w-0 break-words">대여 연체</span>
                <Badge className="shrink-0 whitespace-nowrap" variant={data.kpi.queue.rentalOverdue > 0 ? "destructive" : "secondary"}>
          ...
          <Card className="border-primary/30 bg-primary/5">
            ...
            <CardContent className="space-y-2 text-sm">
              <div className="flex min-w-0 items-center justify-between gap-2">
                ...
                <Badge className="shrink-0 whitespace-nowrap" variant={data.kpi.queue.shippingPending > 0 ? "default" : "outline"}>
                  {formatAdminNumber(data.kpi.queue.shippingPending)}
                </Badge>
              </div>
              <div className="flex min-w-0 items-center justify-between gap-2">
                <span className="min-w-0 break-words">교체 3일+</span>
                <Badge className="shrink-0 whitespace-nowrap" variant={data.kpi.queue.stringingAging3d > 0 ? "default" : "outline"}>
                  {formatAdminNumber(data.kpi.queue.stringingAging3d)}
                </Badge>
              </div>
              <div className="flex min-w-0 items-center justify-between gap-2">
                <span className="min-w-0 break-words">반납 임박 48h</span>
                <Badge className="shrink-0 whitespace-nowrap" variant={data.kpi.queue.rentalDueSoon > 0 ? "default" : "outline"}>
          ...
          <Card className="border-info/40 bg-info/5">
            ...
            <CardContent className="space-y-2 text-sm">
              ...
              <div className="flex min-w-0 items-center justify-between gap-2">
                ...
                <Badge className="shrink-0 whitespace-nowrap" variant={offlineMetrics.pendingOfflineCount > 0 ? "destructive" : "secondary"}>
          ...
          <Card className="border-border/60 bg-card">
            ...
            <CardContent className="grid gap-2">
              <Button asChild size="sm" variant="outline" wrap="responsive" className="justify-start bg-transparent">
                <Link href="/admin/orders">주문·교체서비스 처리</Link>
              </Button>
              <Button asChild size="sm" variant="outline" className="justify-start whitespace-nowrap bg-transparent">
      ...
      <section className="space-y-4">
        ...
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          <KpiCard title="오늘 오프라인 매출" value={formatAdminKRW(offlineMetrics.todayOfflineRevenue)} sub={`이번 달 ${formatAdminKRW(offlineMetrics.monthOfflineRevenue)}`} icon={<Store className="h-5 w-5" />} trend="온라인 총매출 미합산" />
          <KpiCard title="오프라인 작업 매출" value={formatAdminKRW(offlineMetrics.offlineServiceRevenue)} sub="이번 달 작업/매출 records" icon={<Wrench className="h-5 w-5" />} />
          <KpiCard title="오프라인 패키지 판매" value={formatAdminKRW(offlineMetrics.offlinePackageRevenue)} sub="이번 달 offline_admin packageOrders" icon={<Package className="h-5 w-5" />} />
          <KpiCard title="오프라인 미결제" value={formatAdminKRW(offlineMetrics.pendingOfflineAmount)} sub={`${formatAdminNumber(offlineMetrics.pendingOfflineCount)}건 · 매출 합계 제외`} icon={<AlertTriangle className="h-5 w-5" />} />
        </div>
      </section>
          ...
          <Card className="border-destructive/40 bg-destructive/10 dark:border-destructive/40 dark:bg-destructive/15">
            ...
            <CardContent className="space-y-3">
              ...
              <div className="flex items-center justify-between rounded-lg bg-background/60 px-3 py-2">
                ...
                <Badge className="shrink-0 whitespace-nowrap" variant={data.kpi.queue.stringingAging3d > 0 ? "destructive" : "secondary"}>
                  {formatAdminNumber(data.kpi.queue.stringingAging3d)}
                </Badge>
              </div>
              <div className="flex items-center justify-between rounded-lg bg-background/60 px-3 py-2">
          ...
          <Card className={adminSurface.tableCard}>
            ...
            <CardContent className="space-y-3">
              <div className="flex items-center justify-between rounded-lg bg-muted/50 px-3 py-2">
                ...
                <Badge className="shrink-0 whitespace-nowrap" variant={data.kpi.inventory.lowStockProducts > 0 ? "destructive" : "outline"}>
                  {formatAdminNumber(data.kpi.inventory.lowStockProducts)}
                </Badge>
              </div>
              <div className="flex items-center justify-between rounded-lg bg-muted/50 px-3 py-2">
                <span className="text-sm">품절</span>
                <Badge className="shrink-0 whitespace-nowrap" variant={data.kpi.inventory.outOfStockProducts > 0 ? "destructive" : "outline"}>
          ...
          <Card className={adminSurface.tableCard}>
            ...
            <CardContent className="space-y-3">
              <div className="flex items-center justify-between rounded-lg bg-muted/50 px-3 py-2">
                ...
                <Badge className="shrink-0 whitespace-nowrap" variant={data.kpi.queue.offlineReconciliationOpen > 0 ? "destructive" : "outline"}>
                  {formatAdminNumber(data.kpi.queue.offlineReconciliationOpen)}
                </Badge>
              </div>
              ...
              {data.queueDetails.cancelRequests.length === 0 ? (
                ...
                <div className="space-y-3">
                  {data.queueDetails.cancelRequests.slice(0, 5).map((it) => {
                    ...
                    const quickSignalTone = quickSignal?.tone === "success" ? getWorkflowMetaBadgeSpec("application_linked").variant : getWorkflowMetaBadgeSpec("action_required").variant;
                      ...
                      <div key={`${it.kind}-${it.id}`} className="group flex items-start gap-3 rounded-lg border border-border/40 bg-background/60 p-3 transition-colors hover:border-border/80 hover:shadow-sm">
                        <div className="min-w-0 flex-1 space-y-1">
                          ...
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            {statusLabel && (
                              <Badge variant={statusLabel === "취소요청" ? getWorkflowMetaBadgeSpec("cancel_requested").variant : "secondary"} className="shrink-0 whitespace-nowrap text-xs">
                                {statusLabel}
                              </Badge>
                            )}
                            {quickSignal && (
          ...
          <Card className="border-border/40 bg-card/50 backdrop-blur">
            ...
            <CardContent className="space-y-3">
              ...
              <div className="flex items-center justify-between rounded-lg bg-muted/30 px-3 py-2">
                ...
                <Badge className="shrink-0 whitespace-nowrap" variant={data.kpi.community.pendingReports > 0 ? "destructive" : "secondary"}>
              ...
              <div className="space-y-3">
                {data.recent.orders.map((o) =>
                  (() => {
                    ...
                    const displayStatusLabel = getOrderStatusLabelForDisplay(rawStatusLabel, { shippingMethod: o.shippingMethod });
                    const paymentSpec = getPaymentStatusBadgeSpec(labelPaymentStatus(o.paymentStatus));
            ...
            <CardContent>
              <div className="space-y-3">
                {data.recent.applications.map((a) =>
                  (() => {
                    const paymentSpec = getPaymentStatusBadgeSpec(labelPaymentStatus(a.paymentStatus));
                    const statusSpec = getApplicationStatusBadgeSpec(labelStringingStatus(a.status));
                    return (
