...
function SparkLine({
  ...
  const toX = (i: number) =>
    padding + (i * (width - padding * 2)) / Math.max(1, safeData.length - 1);
  const toY = (v: number) =>
    height -
    padding -
    ((v - min) * (height - padding * 2)) / Math.max(1, max - min);
      ...
      {data.map((d, i) => {
        ...
        return (
          <rect
            ...
            height={h}
            rx="3"
            className="fill-primary/20 transition-all hover:fill-primary/30"
          />
        );
      })}
    </svg>
  ...
  return (
    <Card className="group relative overflow-hidden border-border/40 bg-card/50 backdrop-blur transition-all hover:border-border/60 hover:shadow-md">
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-3">
        ...
        <div className="rounded-lg bg-primary/10 p-2 text-primary transition-colors group-hover:bg-primary/15 dark:bg-primary/20 dark:group-hover:bg-primary/25">
          {icon}
        </div>
      </CardHeader>
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">
            최근 {data.series.days}일 데이터 · {data.series.fromYmd} ~{" "}
            {data.series.toYmd}
          </p>
        </div>
      ...
      <section className="space-y-4">
        ...
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <KpiCard
            ...
            spark={
              <SparkLine data={data.series.dailyApplications.slice(-30)} />
            }
          />
          <KpiCard
            title="신규 회원"
                ...
                <div className="space-y-3">
                  {data.queueDetails.cancelRequests.slice(0, 5).map((it) => {
                    ...
                    const quickSignalTone =
                      quickSignal?.tone === "success" ? "default" : "secondary";

                    return (
                      <div
                        key={`${it.kind}-${it.id}`}
                        className="group flex items-start gap-3 rounded-lg border border-border/40 bg-background/60 p-3 transition-all hover:border-border/80 hover:shadow-sm"
                      >
              ...
              {data.inventoryList.lowStock.length === 0 ? (
                ...
                <div className="space-y-3">
                  {data.inventoryList.lowStock.map((p) => (
                    <div
                      key={p.id}
                      className="group flex items-center gap-3 rounded-lg border border-border/40 bg-background/60 p-3 transition-all hover:border-border/80 hover:shadow-sm"
                    >
                      <div className="min-w-0 flex-1">
                        <Link
