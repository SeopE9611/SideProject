    ...
    <div className="rounded-lg border border-border/60 bg-popover px-4 py-3 shadow-xl">
      ...
      <div className="space-y-1.5">
        ...
        <div className="flex items-center justify-between gap-6">
          ...
          <span className="text-sm font-medium tabular-nums">
            {formatAdminKRW(data.applications)}
          </span>
        </div>
        <div className="flex items-center justify-between gap-6">
          <div className="flex items-center gap-2">
    ...
    <div style={{ height }}>
      <ResponsiveContainer width="100%" height="100%">
        ...
        >
          ...
          <XAxis
            dataKey="displayDate"
            tick={{ fontSize: 11, fill: "hsl(120, 6%, 45%)" }}
            tickLine={false}
            axisLine={{ stroke: "hsl(120, 6%, 85%)" }}
          ...
          />
          <RechartsTooltip
            content={<ReviewChartTooltip />}
            cursor={{ fill: "hsl(120, 6%, 95%)", radius: 4 }}
          />
          <Bar
            dataKey="value"
            fill={CHART_COLORS.primary}
    ...
    <div className="flex min-w-0 flex-col gap-4 sm:flex-row sm:items-center">
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-1.5 text-sm">
        {chartData.slice(0, 5).map((d, idx) => (
          <div
            key={d.originalLabel}
            className="flex min-w-0 items-center justify-between gap-2 rounded px-2 py-1 transition-colors hover:bg-muted/30"
          >
...
function KpiCard({
  ...
  return (
    <Card
      className={cn("group relative overflow-hidden", adminSurface.kpiCard)}
    >
      <CardHeader className="flex flex-row items-start justify-between gap-3 space-y-0 pb-3">
        <CardTitle className="min-w-0 break-words text-sm font-medium leading-tight text-muted-foreground">
          {title}
        </CardTitle>
        <div className="shrink-0 rounded-lg bg-primary/10 p-2 text-primary transition-colors group-hover:bg-primary/15 dark:bg-primary/20 dark:group-hover:bg-primary/25">
          {icon}
        </div>
      </CardHeader>
      <CardContent className="space-y-3">
        <div className="overflow-hidden text-ellipsis whitespace-nowrap text-3xl font-bold tracking-normal tabular-nums">
          {value}
        </div>
        {sub && (
...
function getCancelRequestStatusLabel(
  status?: string,
): "취소요청" | "취소승인" | "취소거절" | null {
...
export default function AdminDashboardClient() {
  ...
  const periodRevenue = data.series.dailyRevenue.reduce(
      ...
      <section className="space-y-4">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              긴급, 오늘 처리, 확인 필요 순서로 먼저 확인하세요.
            </p>
          </div>
          <Button
            asChild
            variant="outline"
            className="w-fit shrink-0 whitespace-nowrap bg-transparent"
          >
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <Card className="border-warning/40 bg-warning/5">
            ...
            <CardContent className="space-y-2 text-sm">
              <div className="flex min-w-0 items-center justify-between gap-2">
                <span className="min-w-0 break-words">취소 요청</span>
                <Badge
                  className="shrink-0 whitespace-nowrap"
                  variant={
                    data.kpi.queue.cancelRequests > 0
                      ? "destructive"
                      : "secondary"
                  }
                >
                  {formatAdminNumber(data.kpi.queue.cancelRequests)}
                </Badge>
              </div>
              <div className="flex min-w-0 items-center justify-between gap-2">
                <span className="min-w-0 break-words">결제 대기 24h+</span>
                <Badge
                  className="shrink-0 whitespace-nowrap"
                  variant={
                    data.kpi.queue.paymentPending24h > 0
                      ? "destructive"
                      : "secondary"
                  }
                >
                  {formatAdminNumber(data.kpi.queue.paymentPending24h)}
                </Badge>
              </div>
              <div className="flex min-w-0 items-center justify-between gap-2">
                <span className="min-w-0 break-words">대여 연체</span>
                <Badge
                  className="shrink-0 whitespace-nowrap"
                  variant={
                    data.kpi.queue.rentalOverdue > 0
                      ? "destructive"
                      : "secondary"
                  }
                >
                  {formatAdminNumber(data.kpi.queue.rentalOverdue)}
                </Badge>
              </div>
            </CardContent>
          </Card>
          <Card className="border-primary/30 bg-primary/5">
            ...
            <CardContent className="space-y-2 text-sm">
              <div className="flex min-w-0 items-center justify-between gap-2">
                <span className="min-w-0 break-words">송장 등록 대기</span>
                <Badge
                  className="shrink-0 whitespace-nowrap"
                  variant={
                    data.kpi.queue.shippingPending > 0 ? "default" : "outline"
                  }
                >
                  {formatAdminNumber(data.kpi.queue.shippingPending)}
                </Badge>
              </div>
              <div className="flex min-w-0 items-center justify-between gap-2">
                <span className="min-w-0 break-words">교체 3일+</span>
                <Badge
                  className="shrink-0 whitespace-nowrap"
                  variant={
                    data.kpi.queue.stringingAging3d > 0 ? "default" : "outline"
                  }
                >
                  {formatAdminNumber(data.kpi.queue.stringingAging3d)}
                </Badge>
              </div>
              <div className="flex min-w-0 items-center justify-between gap-2">
                <span className="min-w-0 break-words">반납 임박 48h</span>
                <Badge
                  className="shrink-0 whitespace-nowrap"
                  variant={
                    data.kpi.queue.rentalDueSoon > 0 ? "default" : "outline"
                  }
                >
                  {formatAdminNumber(data.kpi.queue.rentalDueSoon)}
                </Badge>
              </div>
            </CardContent>
          </Card>
          <Card className="border-info/40 bg-info/5">
            ...
            <CardContent className="space-y-2 text-sm">
              <Link
                href="/admin/operations?kind=package_purchase"
                className="flex min-w-0 items-center justify-between gap-2 rounded-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
              >
                <span className="min-w-0 break-words">패키지 구매 확인</span>
                <Badge
                  className="shrink-0 whitespace-nowrap"
                  variant={
                    data.kpi.queue.packagePaymentCheck > 0
                      ? "destructive"
                      : "secondary"
                  }
                >
              ...
              <div className="flex min-w-0 items-center justify-between gap-2">
                ...
                <Badge
                  className="shrink-0 whitespace-nowrap"
                  variant={
                    data.kpi.queue.offlineReconciliationOpen > 0
                      ? "destructive"
                      : "secondary"
                  }
                >
              ...
              <div className="flex min-w-0 items-center justify-between gap-2">
                <span className="min-w-0 break-words">오프라인 미결제</span>
                <Badge
                  className="shrink-0 whitespace-nowrap"
                  variant={
                    offlineMetrics.pendingOfflineCount > 0
                      ? "destructive"
                      : "secondary"
                  }
                >
              ...
              <div className="flex min-w-0 items-center justify-between gap-2">
                ...
                <Badge
                  className="shrink-0 whitespace-nowrap"
                  variant={
                    !data.settlements.hasPrevSnapshot
                      ? "destructive"
                      : "secondary"
                  }
                >
                  {!data.settlements.hasPrevSnapshot ? "미생성" : "OK"}
                </Badge>
              </div>
            </CardContent>
          </Card>
          <Card className="border-border/60 bg-card">
            ...
            <CardContent className="grid gap-2">
              ...
              >
                <Link href="/admin/orders">주문·교체서비스 처리</Link>
              </Button>
              <Button
                asChild
                size="sm"
                variant="outline"
                className="justify-start whitespace-nowrap bg-transparent"
              >
                <Link href="/admin/rentals">라켓 대여 처리</Link>
              </Button>
              <Button
                asChild
                size="sm"
                ...
                className="justify-start bg-transparent"
              >
                <Link href="/admin/packages?preset=payment-check">
                  패키지 결제/활성화
                </Link>
              </Button>
              <Button
                ...
                className="justify-start bg-transparent"
              >
                <Link href="/admin/offline">오프라인 접수/매출</Link>
              </Button>
              ...
              <Button
                ...
                className="justify-start bg-transparent"
              >
                <Link href="/admin/offline/reconciliation">
                  오프라인 보정 관리
                </Link>
              </Button>
              <Button
                ...
                className="justify-start whitespace-nowrap bg-transparent"
              >
                <Link href="/admin/academy/applications">아카데미 상담</Link>
              </Button>
            </CardContent>
          </Card>
        </div>
          ...
          <KpiCard
            ...
            spark={
              <InteractiveSparkLine
                data={data.series.dailyRevenue.slice(-30)}
              />
            }
          />
          <KpiCard
            ...
            spark={
              <InteractiveSparkLine data={data.series.dailyOrders.slice(-30)} />
            }
          />
          <KpiCard
            title="교체 서비스"
            ...
            spark={
              <InteractiveSparkLine
                data={data.series.dailyApplications.slice(-30)}
              />
            }
          />
          <KpiCard
            ...
            spark={
              <InteractiveSparkLine
                data={data.series.dailySignups.slice(-30)}
              />
            }
          />
        </div>
        ...
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <CardContent className="space-y-3">
              <div className="rounded-lg bg-background/60 px-3 py-2">
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  계좌 확인 필요{" "}
                  {formatAdminNumber(
                    data.kpi.queue.cancelRequestsNeedingRefundAccount,
                  )}
                  건 / 검토 가능{" "}
                  {formatAdminNumber(
                    data.kpi.queue.cancelRequestsReadyForReview,
                  )}
                  건
                </p>
              </div>
              ...
              <div className="flex items-center justify-between rounded-lg bg-background/60 px-3 py-2">
                ...
                <Badge
                  className="shrink-0 whitespace-nowrap"
                  variant={
                    data.kpi.queue.stringingAging3d > 0
                      ? "destructive"
                      : "secondary"
                  }
                >
          ...
          <Card className={adminSurface.tableCard}>
            ...
            <CardContent className="space-y-3">
              ...
              >
                <Link href="/admin/orders?preset=shippingPending">
                  관리하기
                </Link>
              </Button>
            </CardContent>
            ...
            <CardContent className="space-y-3">
              <div className="flex items-center justify-between rounded-lg bg-muted/50 px-3 py-2">
                ...
                <Badge
                  className="shrink-0 whitespace-nowrap"
                  variant={
                    data.kpi.inventory.lowStockProducts > 0
                      ? "destructive"
                      : "outline"
                  }
                >
                  {formatAdminNumber(data.kpi.inventory.lowStockProducts)}
                </Badge>
              </div>
              <div className="flex items-center justify-between rounded-lg bg-muted/50 px-3 py-2">
                ...
                <Badge
                  className="shrink-0 whitespace-nowrap"
                  variant={
                    data.kpi.inventory.outOfStockProducts > 0
                      ? "destructive"
                      : "outline"
                  }
                >
                  {formatAdminNumber(data.kpi.inventory.outOfStockProducts)}
                </Badge>
              </div>
              <Button
                size="sm"
          ...
          <Card className={adminSurface.tableCard}>
            ...
            <CardContent className="space-y-3">
              <div className="flex items-center justify-between rounded-lg bg-muted/50 px-3 py-2">
                <span className="text-sm">오프라인 보정</span>
                <Badge
                  className="shrink-0 whitespace-nowrap"
                  variant={
                    data.kpi.queue.offlineReconciliationOpen > 0
      ...
      <section className="space-y-4">
        ...
        <Card className="border-border/40 bg-card/50 backdrop-blur">
          <CardHeader>
            ...
            <CardDescription>
              오프라인 패키지 판매를 제외한 주문, 교체 서비스, 패키지 결제완료
              금액
            </CardDescription>
          </CardHeader>
          <CardContent>
            ...
            <div className="mt-6 flex flex-wrap items-center gap-6 rounded-lg bg-muted/30 px-4 py-3">
              ...
              <div className="ml-auto text-sm font-semibold">
                온라인 합계{" "}
                {formatAdminKRW(
                  last14Revenue.reduce((s, d) => s + Number(d.value || 0), 0),
                )}
              </div>
            </div>
          </CardContent>
              ...
              {data.top.products7d.length === 0 ? (
                ...
                <div className="space-y-3">
                  {data.top.products7d.map((p, idx) => (
                    <div
                      key={p.productId}
                      className="group flex items-center gap-4 rounded-lg bg-muted/30 p-3 transition-all hover:bg-muted/50"
                    >
              ...
              {data.top.brands7d.length === 0 ? (
                ...
                <div className="space-y-3">
                  {data.top.brands7d.map((b, idx) => {
                    ...
                    const brandHref = hasBrand
                      ? `/admin/products?brand=${encodeURIComponent(b.brand)}`
                      : undefined;
                    return (
                      <div
                        key={`${b.brand}-${b.qty}-${b.revenue}`}
                        className="group flex items-center gap-4 rounded-lg bg-muted/30 p-3 transition-all hover:bg-muted/50"
                      >
                        <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-primary/10 text-sm font-bold text-primary dark:bg-primary/20">
                          {idx + 1}
              ...
              {data.queueDetails.cancelRequests.length === 0 ? (
                ...
                <div className="space-y-3">
                  {data.queueDetails.cancelRequests.slice(0, 5).map((it) => {
                    ...
                    const quickSignal = getCancelQueueQuickSignal(
                      it.status,
                      it.refundAccountReady,
                    );
                    ...
                    return (
                      <div
                        key={`${it.kind}-${it.id}`}
                        className="group flex items-start gap-3 rounded-lg border border-border/40 bg-background/60 p-3 transition-colors hover:border-border/80 hover:shadow-sm"
                      >
                        <div className="min-w-0 flex-1 space-y-1">
                          <Link
                            href={it.href}
                            className="line-clamp-2 break-keep text-sm font-medium group-hover:underline"
                          >
                            {it.name}
                          </Link>
                              ...
                              <TooltipProvider delayDuration={50}>
                                <Tooltip>
                                  <TooltipTrigger asChild>
                                    <Badge
                                      variant={quickSignalTone}
                                      className="inline-flex shrink-0 cursor-help items-center gap-1 whitespace-nowrap text-xs"
                                    >
                                      {quickSignal.label}
                  ...
                  <Button
                    ...
                    className="mt-2 w-full bg-transparent"
                  >
                    <Link href="/admin/orders?preset=cancelRequests">
                      전체 보기
                    </Link>
                  </Button>
                </div>
                ...
                <div className="space-y-3">
                  {data.queueDetails.shippingPending.slice(0, 5).map((it) => (
                    ...
                    >
                      ...
                      <Badge className="shrink-0 whitespace-nowrap tabular-nums">
                        {formatAdminKRW(it.amount)}
                      </Badge>
                    </div>
                  ))}
              ...
              {data.queueDetails.paymentPending24h.length === 0 ? (
                ...
                <div className="space-y-3">
                  ...
                  <Button
                    ...
                    className="mt-2 w-full bg-transparent"
                  >
                    <Link href="/admin/orders?preset=paymentPending24h">
                      전체 보기
                    </Link>
                  </Button>
                </div>
              ...
              {data.queueDetails.stringingAging.length === 0 ? (
                ...
                <div className="space-y-3">
                  ...
                  <Button
                    ...
                    className="mt-2 w-full bg-transparent"
                  >
                    <Link href="/admin/orders?preset=stringingAging3d">
                      전체 보기
                    </Link>
                  </Button>
                </div>
              ...
              {data.queueDetails.rentalOverdue.length === 0 ? (
                ...
                <div className="space-y-3">
                  {data.queueDetails.rentalOverdue.slice(0, 5).map((r) => (
                    ...
                    >
                      <div className="min-w-0 flex-1 space-y-1">
                        ...
                        <p className="text-xs text-muted-foreground">
                          반납 기한: {formatIsoToKstShort(r.dueAt)}
                        </p>
                      </div>
                      <Badge
                        variant="destructive"
                  ...
                  <Button
                    ...
                    className="mt-2 w-full bg-transparent"
                  >
                    <Link href="/admin/rentals?status=out&due=overdue">
                      전체 보기
                    </Link>
                  </Button>
                </div>
              ...
              {data.queueDetails.rentalDueSoon.length === 0 ? (
                ...
                <div className="space-y-3">
                  ...
                  <Button
                    ...
                    className="mt-2 w-full bg-transparent"
                  >
                    <Link href="/admin/rentals?status=out&due=soon">
                      전체 보기
                    </Link>
                  </Button>
                </div>
            ...
            <CardContent className="space-y-3">
              <div className="flex items-center justify-between rounded-lg bg-muted/30 px-3 py-2">
                ...
                <Badge variant="secondary" className="text-base font-bold">
                  {(Math.round((data.kpi.reviews.avg || 0) * 10) / 10).toFixed(
                    1,
                  )}
                </Badge>
              </div>
              <div className="flex items-center justify-between rounded-lg bg-muted/30 px-3 py-2">
                ...
                <Badge className="shrink-0 whitespace-nowrap" variant="outline">
                  {formatAdminNumber(data.kpi.reviews.delta7d)}
                </Badge>
              </div>
              <div className="flex items-center justify-between rounded-lg bg-muted/30 px-3 py-2">
                <span className="text-sm">5점 리뷰</span>
                <Badge className="shrink-0 whitespace-nowrap" variant="outline">
                  {formatAdminNumber(data.kpi.reviews.five)}
                </Badge>
              </div>
              <Button
                size="sm"
            ...
            <CardContent className="space-y-3">
              <div className="flex items-center justify-between rounded-lg bg-muted/30 px-3 py-2">
                ...
                <Badge
                  ...
                  variant="secondary"
                >
                  {formatAdminNumber(data.kpi.points.issued7d)}P
                </Badge>
              </div>
              <div className="flex items-center justify-between rounded-lg bg-muted/30 px-3 py-2">
                <span className="text-sm">사용</span>
                <Badge className="shrink-0 whitespace-nowrap" variant="outline">
                  {formatAdminNumber(data.kpi.points.spent7d)}P
                </Badge>
              </div>
              <Button
                size="sm"
          ...
          <Card className="border-border/40 bg-card/50 backdrop-blur">
            ...
            <CardContent className="space-y-3">
              <div className="flex items-center justify-between rounded-lg bg-muted/30 px-3 py-2">
                ...
                <Badge className="shrink-0 whitespace-nowrap" variant="outline">
                  {formatAdminNumber(data.kpi.community.posts7d)}
                </Badge>
              </div>
              <div className="flex items-center justify-between rounded-lg bg-muted/30 px-3 py-2">
                <span className="text-sm">댓글 7일</span>
                <Badge className="shrink-0 whitespace-nowrap" variant="outline">
                  {formatAdminNumber(data.kpi.community.comments7d)}
                </Badge>
              </div>
              <div className="flex items-center justify-between rounded-lg bg-muted/30 px-3 py-2">
                ...
                <Badge
                  className="shrink-0 whitespace-nowrap"
                  variant={
                    data.kpi.community.pendingReports > 0
                      ? "destructive"
                      : "secondary"
                  }
                >
                  {formatAdminNumber(data.kpi.community.pendingReports)}
                </Badge>
              </div>
              <Button
                size="sm"
              ...
              {data.inventoryList.lowStock.length === 0 ? (
                ...
                <div className="space-y-3">
                  {data.inventoryList.lowStock.map((p) => (
                    <div
                      key={p.id}
                      className="group flex items-center gap-3 rounded-lg border border-border/40 bg-background/60 p-3 transition-colors hover:border-border/80 hover:shadow-sm"
                    >
                      <div className="min-w-0 flex-1">
                        <Link
                      ...
                      <div className="shrink-0 text-right">
                        <Badge
                          ...
                          variant="destructive"
                        >
                          {formatAdminNumber(p.stock)}개
                        </Badge>
                        <p className="mt-1 text-xs text-muted-foreground">
                          기준:{" "}
                          {p.lowStock === null
                            ? "-"
                            : formatAdminNumber(p.lowStock)}
                        </p>
                      </div>
                    </div>
                  ))}
                      ...
                      >
                        <div className="min-w-0 flex-1 space-y-1">
                          ...
                          <p className="whitespace-nowrap text-xs tabular-nums text-muted-foreground">
                            {formatIsoToKstShort(o.createdAt)}
                          </p>
            ...
            <CardContent>
              <div className="space-y-3">
                {data.recent.applications.map((a) =>
                  (() => {
                    ...
                    return (
                      <Link
                        key={a.id}
                        href={`/admin/applications/stringing/${a.id}`}
                        className="group flex items-start gap-3 rounded-lg border border-border/40 bg-background/60 p-3 transition-colors hover:border-border/80 hover:shadow-sm"
                      >
                        <div className="min-w-0 flex-1 space-y-1">
                          <p className="truncate text-sm font-medium">
                          ...
                          <p className="whitespace-nowrap text-xs tabular-nums text-muted-foreground">
                            {formatIsoToKstShort(a.createdAt)}
                          </p>
                          <div className="flex flex-wrap gap-1">
                            <Badge
                              variant={paymentSpec.variant}
                ...
                <Button
                  ...
                  className="mt-2 w-full bg-transparent"
                >
                  <Link href="/admin/applications/stringing">
                    전체 신청 보기
                  </Link>
                </Button>
              </div>
            ...
            <div className="mt-4 flex flex-wrap items-center gap-4 rounded-lg bg-muted/30 px-4 py-3 text-sm">
              <span className="font-medium">
                ...
                <span className="text-primary">
                  {formatAdminNumber(
                    last14Reviews.reduce((s, d) => s + Number(d.value || 0), 0),
                  )}
                  건
                </span>
              </span>
