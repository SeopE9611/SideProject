...
export default function ProductsClient() {
  ...
  const activeFilterLabels = [
    ...
    statusFilter !== "all" ? `재고 상태: ${STATUS_UI[statusFilter as StatusKey]?.label ?? statusFilter}` : null,
    exposureFilter !== "all" ? `노출: ${exposureFilter === "featured" ? "추천 상품" : exposureFilter === "new" ? "신상품" : exposureFilter === "sale" ? "할인 상품" : exposureFilter}` : null,
  ].filter((label): label is string => Boolean(label));

  // 빠른 보기 라벨은 정확히 해당 조건만 적용된 경우에만 표시합니다.
  const currentViewLabel = !hasActiveTableFilter
    ? "전체 상품"
    : statusFilter === "active" && !debouncedTerm.trim() && brandFilter === "all" && materialFilter === "all" && exposureFilter === "all"
      ? "판매 중"
      : statusFilter === "low_stock" && !debouncedTerm.trim() && brandFilter === "all" && materialFilter === "all" && exposureFilter === "all"
        ? "재고 부족"
        : statusFilter === "out_of_stock" && !debouncedTerm.trim() && brandFilter === "all" && materialFilter === "all" && exposureFilter === "all"
          ? "품절"
          : exposureFilter === "featured" && !debouncedTerm.trim() && brandFilter === "all" && materialFilter === "all" && statusFilter === "all"
            ? "추천 상품"
            : exposureFilter === "new" && !debouncedTerm.trim() && brandFilter === "all" && materialFilter === "all" && statusFilter === "all"
              ? "신상품"
              : exposureFilter === "sale" && !debouncedTerm.trim() && brandFilter === "all" && materialFilter === "all" && statusFilter === "all"
                ? "할인 상품"
                : "사용자 지정 조건";
      ...
      <div className="container py-8 px-6">
        ...
        <Card className={cn("mb-4", adminSurface.cardMuted)}>
          <CardContent className="flex flex-wrap items-center gap-2 p-4">
            ...
            <Button type="button" size="sm" variant={!hasActiveTableFilter ? "default" : "outline"} onClick={resetFilters}>
              전체
            </Button>

            <Button type="button" size="sm" variant={currentViewLabel === "판매 중" ? "default" : "outline"} onClick={() => applyQuickView({ status: "active" })}>
              판매 중
            </Button>

            <Button type="button" size="sm" variant={currentViewLabel === "재고 부족" ? "default" : "outline"} onClick={() => applyQuickView({ status: "low_stock" })}>
              재고 부족
            </Button>

            <Button type="button" size="sm" variant={currentViewLabel === "품절" ? "default" : "outline"} onClick={() => applyQuickView({ status: "out_of_stock" })}>
              품절
            </Button>

            <Button type="button" size="sm" variant={currentViewLabel === "추천 상품" ? "default" : "outline"} onClick={() => applyQuickView({ exposure: "featured" })}>
              추천 상품
            </Button>

            <Button type="button" size="sm" variant={currentViewLabel === "신상품" ? "default" : "outline"} onClick={() => applyQuickView({ exposure: "new" })}>
              신상품
            </Button>

            <Button type="button" size="sm" variant={currentViewLabel === "할인 상품" ? "default" : "outline"} onClick={() => applyQuickView({ exposure: "sale" })}>
              할인 상품
            </Button>
          </CardContent>
        </Card>
        ...
        <div className={cn("mb-4 flex flex-wrap items-center gap-x-4 gap-y-2 rounded-lg border px-4 py-3 text-sm", adminSurface.cardMuted)}>
          ...
          <p className="text-muted-foreground">총 {hasResolvedData ? total.toLocaleString("ko-KR") : "-"}개</p>
        ...
        <Card className={cn(adminSurface.tableCard, "flex-1 min-h-0 flex flex-col")}>
          <CardHeader className="bg-muted/50 border-b border-border pb-4 shrink-0">
            <div className="flex flex-col space-y-4 md:flex-row md:items-center md:justify-between md:space-y-0">
              <div>
                <CardTitle className="text-xl font-semibold text-foreground">스트링 상품 찾기</CardTitle>
