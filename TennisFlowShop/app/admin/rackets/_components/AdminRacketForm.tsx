...
export default function AdminRacketForm({ initial, submitLabel, onSubmit }: { initial?: Partial<RacketForm>; submitLabel: string; onSubmit: (data: RacketForm) => Promise<void> }) {
  ...
  const hasBasicInfo = Boolean(form.brand) && form.model.trim().length >= MODEL_MIN && form.model.trim().length <= MODEL_MAX;
  ...
  const hasValidSalesInfo =
    ...
    (!form.marketing.isSale || (isFiniteNumber(form.marketing.salePrice) && Number(form.marketing.salePrice) >= 1 && Number(form.marketing.salePrice) < Number(form.price)));

  const hasValidRentalInfo = form.rental.enabled ? nonNegative(form.rental.deposit) && nonNegative(form.rental.fee.d7) && nonNegative(form.rental.fee.d15) && nonNegative(form.rental.fee.d30) : Boolean(form.rental.disabledReason?.trim());
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(() => {
      ...
      const f = stepContentRef.current?.querySelector<HTMLElement>("input:not([type='hidden']):not([type='file']):not([disabled]), textarea:not([disabled]), button[role='combobox']:not([disabled]), [data-radix-select-trigger]:not([disabled])");
  ...
  const validateAndBuild = () => {
    ...
    if (!modelTrim || modelTrim.length < MODEL_MIN || modelTrim.length > MODEL_MAX) return [0, `모델명은 ${MODEL_MIN}~${MODEL_MAX}자입니다.`] as const;
    if (!isFiniteNumber(form.price) || Number(form.price) < PRICE_MIN) return [2, `가격은 ${PRICE_MIN}원 이상 입력하세요.`] as const;
    ...
    if (!positiveOrNull(form.spec.weight) || !positiveOrNull(form.spec.balance) || !positiveOrNull(form.spec.headSize) || !positiveOrNull(form.spec.lengthIn) || !positiveOrNull(form.spec.swingWeight) || !positiveOrNull(form.spec.stiffnessRa))
    ...
    if (form.marketing.isSale && (!isFiniteNumber(form.marketing.salePrice) || Number(form.marketing.salePrice) < 1)) return [2, "할인 상품은 할인가를 1원 이상 입력하세요."] as const;
    if (form.marketing.isSale && Number(form.marketing.salePrice) >= Number(form.price)) return [2, "할인가는 정가보다 낮아야 합니다."] as const;
    return [
      null,
      {
        ...form,
  ...
  return (
    ...
    >
      <div className="rounded-2xl border border-border/60 bg-card/80 p-4 shadow-sm">
        ...
        <p className="text-sm text-muted-foreground mt-1">{initial ? "라켓 정보를 수정해주세요." : "새 라켓 정보를 입력해주세요."}</p>
        <div className="mt-3">
          <StepProgress
            steps={STEPS}
        ...
        <div className="rounded-xl border border-border/60 bg-muted/20 p-3 shadow-sm">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-xs leading-relaxed text-muted-foreground">라켓 정보는 단계 이동 중에도 유지됩니다. 저장 전 기본 정보, 스펙, 판매/대여 설정, 이미지를 함께 확인하세요.</p>
            </div>
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_300px]">
        <div ref={stepContentRef}>
          {currentStep.id === "basic" && (
            <FormSection title="기본 정보" description="라켓의 기본 정보를 입력하세요" icon={<FileText className="h-4 w-4" />}>
              <div className="mb-4 rounded-lg border border-primary/20 bg-primary/5 p-4">
                <p className="text-sm font-semibold text-foreground">기본 정보 입력 순서</p>
                <p className="mt-1 text-xs leading-relaxed text-muted-foreground">브랜드와 모델명은 고객 목록과 검색 결과에 바로 노출됩니다. 모델명은 2~80자 범위로 입력하세요.</p>
              </div>
          ...
          {currentStep.id === "specs" && (
            <FormSection title="상세 스펙" description="라켓 상세 스펙" icon={<Settings className="h-4 w-4" />}>
              <div className="mb-4 rounded-lg border border-primary/20 bg-primary/5 p-4">
                ...
                <p className="mt-1 text-xs leading-relaxed text-muted-foreground">무게, 밸런스, 헤드 사이즈 같은 수치는 선택 필드지만, 입력 시 1 이상의 숫자만 허용됩니다. 스트링 패턴과 그립 사이즈는 반드시 선택해야 합니다.</p>
              </div>
              <FormFieldGroup columns={2}>
                ...
                <FormField label="스트링 패턴">
                  <Select value={form.spec.pattern || undefined} onValueChange={(v) => setForm({ ...form, spec: { ...form.spec, pattern: v } })}>
                    <SelectTrigger>
                ...
                <FormField label="그립 사이즈">
                  <Select value={form.spec.gripSize || undefined} onValueChange={(v) => setForm({ ...form, spec: { ...form.spec, gripSize: v } })}>
                    <SelectTrigger>
                      <SelectValue placeholder="선택" />
                    </SelectTrigger>
                    <SelectContent>
          ...
          {currentStep.id === "rental" && (
            <div className="space-y-6">
              <FormSection title="판매/재고 설정" description="가격, 배송비, 판매 상태, 보유 수량" icon={<DollarSign className="h-4 w-4" />}>
                <div className="mb-4 rounded-lg border border-warning/30 bg-warning/10 p-4">
                  ...
                  <p className="mt-1 text-xs leading-relaxed text-muted-foreground">가격, 배송비, 보유 수량은 주문과 대여 가능 여부에 직접 영향을 줍니다. 대여 불가인 경우 고객에게 보일 사유를 반드시 입력하세요.</p>
                </div>
                <FormFieldGroup columns={2}>
              ...
              <FormSection title="대여 설정" description="대여 가능 여부 및 기간별 요금" icon={<Boxes className="h-4 w-4" />}>
              ...
              <FormSection title="노출 옵션" description="추천, 신상품, 할인 배지를 설정합니다." icon={<DollarSign className="h-4 w-4" />}>
          ...
          {currentStep.id === "images" && (
            <FormSection title="이미지" description="이미지 업로드/대표 설정" icon={<ImageIcon className="h-4 w-4" />}>
              <div className="mb-4 rounded-lg border border-primary/20 bg-primary/5 p-4">
                ...
                <p className="mt-1 text-xs leading-relaxed text-muted-foreground">대표 이미지는 목록과 상세 페이지에서 가장 먼저 보입니다. 최소 1장 이상 등록한 뒤 저장하세요.</p>
              </div>
          ...
          <aside className="sticky top-20 rounded-2xl border border-border/60 bg-card/80 p-4 shadow-sm backdrop-blur-sm">
            ...
            <div className="space-y-3">
              <div className="rounded-xl border border-border/60 bg-muted/20 p-3">
                <p className="line-clamp-2 text-base font-bold tracking-tight text-foreground">{form.model.trim() || "모델명 미입력"}</p>
              ...
              <div className="rounded-xl border border-border/60 bg-card p-3">
                ...
                {form.marketing.isSale && form.marketing.salePrice > 0 && form.marketing.salePrice < form.price ? (
                  <div className="mt-1">
                    <p className="text-xl font-bold text-primary">{Number(form.marketing.salePrice).toLocaleString("ko-KR")}원</p>
                    <p className="text-xs text-muted-foreground line-through">{Number(form.price || 0).toLocaleString("ko-KR")}원</p>
                  </div>
                ) : (
                  <p className="mt-1 text-xl font-bold text-foreground">{Number(form.price || 0).toLocaleString("ko-KR")}원</p>
                )}

                <p className="mt-1 text-xs text-muted-foreground">배송비 {Number(form.shippingFee || 0).toLocaleString("ko-KR")}원</p>
              </div>
              ...
              <div className="grid grid-cols-3 gap-2">
                <div className="rounded-lg border border-border/60 bg-muted/20 p-2 text-center">
                  ...
                  <p className="mt-0.5 text-base font-semibold text-foreground">{Number(form.quantity || 0).toLocaleString("ko-KR")}</p>
                </div>

                <div className="rounded-lg border border-border/60 bg-muted/20 p-2 text-center">
                  <p className="text-[11px] text-muted-foreground">대여</p>
                  <p className="mt-0.5 text-xs font-semibold text-foreground">{form.rental.enabled ? "가능" : "불가"}</p>
                </div>
