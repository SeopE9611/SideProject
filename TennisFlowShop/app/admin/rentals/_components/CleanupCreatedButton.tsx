...
export default function CleanupCreatedButton({
  ...
  const run = async () => {
    ...
    setLoading(true);
    try {
      const result = await adminMutator<{
        deleted?: number;
        hours?: number;
        message?: string;
      }>(`/api/admin/rentals/cleanup-created?hours=${hours}`, {
        method: "POST",
      });
      showSuccessToast(result.message ?? "데이터 정리 완료");

      alert(
        `데이터 정리 완료: ${result.deleted ?? 0}건 삭제 (기준: ${result.hours ?? hours}시간 경과)`,
      );
    } catch (error) {
  ...
  return (
    <div className="inline-flex items-center gap-2">
      <button
        ...
        title={
          isFeatureEnabled
            ? "created 상태로 오래 남은 대여 신청을 삭제합니다."
            : RENTAL_CLEANUP_CREATED_DISABLED_MESSAGE
        }
      >
        {loading ? "정리 중…" : "데이터 정리"}
