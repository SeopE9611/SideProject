...
const PAGE_COPY = {
  title: "운영 통합 센터",
  description:
    "주의(실제 오류)와 검수필요(운영 확인 신호)를 구분해 주문·대여·신청을 한 화면에서 점검하는 관리자 운영 허브입니다.",
  ...
  onboarding: {
    ...
    description:
      "페이지 목적을 확인하고, 주요 필터 프리셋으로 업무 대상을 좁힌 뒤, 주의 → 검수필요 → 미처리 순으로 점검하세요.",
    steps: [
      "1) 오늘 해야 할 일 확인",
      "2) 업무 목적형 프리셋 선택",
      "3) 주의 → 검수필요 → 미처리 순 점검",
                ...
                <Table>
                  ...
                  <TableBody>
                    {groupsToRender.map((g, idx) => {
                      ...
                      const childStatusSummary = summarizeByKind(
                        children,
                        (it) => it.statusDisplayLabel ?? it.statusLabel,
                      );
