...
function toErrorLog(error: unknown) {
  ...
  const isMongoTimeout = name === "MongoServerSelectionError" || message.includes("Server selection timed out") || message.includes("timed out");
  return { name, message, code, isMongoTimeout };
}

async function runMongoQueryWithDiagnosis<T>(params: { route: string; orderId: string; collection: string; action: string; run: () => Promise<T> }): Promise<T> {
...
export default async function CheckoutSuccessPage({ searchParams }: { searchParams: Promise<{ orderId?: string }> }) {
  ...
  try {
        ...
        const passDoc =
          packagePassId && ObjectId.isValid(packagePassId)
            ? await runMongoQueryWithDiagnosis({
                ...
                run: () => db.collection("service_passes").findOne({ _id: new ObjectId(packagePassId) }, { projection: { packageSize: 1, usedCount: 1, remainingCount: 1, meta: 1 } }),
              })
            : null;
    ...
    const paymentMethodLabel = isZeroPayment ? "결제 불필요" : isTossPayment ? getTossMethodLabel(paymentMethodRaw, easyPayProviderRaw) : isNicePayment ? getNiceMethodLabel(paymentMethodRaw, easyPayProviderRaw) : "무통장입금";
              ...
              <Card data-cy="checkout-success-order-card" className="overflow-hidden border border-border bg-card shadow-2xl backdrop-blur-sm">
                <div className="border-b border-border bg-background p-6">
                  ...
                  <CardDescription className="mt-2 text-muted-foreground">
                    {withStringService ? "주문 및 교체서비스 진행 정보를 한\u00A0번에 확인하세요." : isVisitPickup ? "주문 정보와 수령 안내를 확인하세요." : "주문 정보와 배송 상태를 확인하세요."}
                  </CardDescription>
                </div>
                            ...
                            <div className="flex-1">
                              ...
                              {(item.selectedColorLabel || item.selectedColor) && (
                                <p className="flex items-center gap-2 text-xs text-muted-foreground">
                                  <span>선택 옵션: 색상</span>
                                  {item.selectedColorHex && <span className="h-3 w-3 rounded-full border border-border" style={{ backgroundColor: item.selectedColorHex }} aria-hidden="true" />}
                    ...
                    <>
                      <div className="mb-6">
                        ...
                        {hasSubmittedApplication && stringingSummary ? (
                            ...
                            <div className="space-y-1.5 rounded-md border border-border bg-card p-3">
                              ...
                              {stringingSummary.packageInfo.applied ? (
                                <>
                                  ...
                                  <p>
                                    <span className="text-muted-foreground">사용 패키지:</span> <span className="font-semibold">{stringingSummary.packageInfo.passTitle ?? "패키지명 확인 불가"}</span>
                                  </p>
                                  <p>
                                    <span className="text-muted-foreground">이번 차감 횟수:</span> <span className="font-semibold">{stringingSummary.packageInfo.useCount}회</span>
                                  </p>
                                  {typeof stringingSummary.packageInfo.remainingCount === "number" && (
                                    <p>
                                      <span className="text-muted-foreground">남은 횟수:</span> <span className="font-semibold">{stringingSummary.packageInfo.remainingCount}회</span>
                                    </p>
                                  )}
                                  <p>
                                ...
                                <>
                                  ...
                                  <p>
                                    <span className="text-muted-foreground">교체서비스 비용:</span> <span className="font-semibold">{formatPrice(stringingSummary.serviceFeeAfter ?? normalizedServiceFee)}원</span>
                                  </p>
                                </>
                              )}
                            </div>
                          ...
                          <div className="space-y-3 rounded-lg border border-border bg-background p-4">
                            <p className="text-sm text-muted-foreground">{isVisitPickup ? "방문 수령 시 현장 장착으로 진행됩니다. 평균 15~20분 소요." : "택배 수령을 선택하셨으므로 수거/반송을 통해 장착 서비스가 진행됩니다."}</p>
                            {shouldShowApplyCta ? (
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex items-start gap-3 rounded-lg border border-border bg-background p-4">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">{isVisitPickup ? "방문 수령 관련 문의사항은 고객센터(010-5218-5248)로 연락주세요." : "배송 관련 문의사항은 고객센터(010-5218-5248)로 연락주세요."}</p>
                        </div>
                      </div>
                    </div>
                  </div>
            ...
            <div className="mx-auto max-w-2xl space-y-4">
              <Card className="border border-border bg-card shadow-xl">
                <CardHeader className="border-b border-border bg-background">
                  ...
                  <CardDescription>주문 상세 정보를 불러오는 중 일시적인 문제가 발생했습니다. 잠시 후 다시 시도하거나 마이페이지에서 확인해주세요.</CardDescription>
                </CardHeader>
