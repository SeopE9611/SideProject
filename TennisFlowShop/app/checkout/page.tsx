...
export default function CheckoutPage() {
  ...
  const orderItemsKey = orderItems
    .map((it) => `${it.kind}:${it.id}:${it.quantity}`)
    .join("|");
  ...
  const hasRacketInOrder = useMemo(
    () => orderItems.some((it) => it.kind === "racket"),
    [orderItemsKey],
  );
  ...
  const isServiceFeeTarget = (it: CartItem) =>
    SERVICE_FEE_KINDS.has((it.kind ?? "product") as any);
  ...
  const checkoutPackageUsage = useMemo(
    ...
    [
      checkoutStringingAdapter.packagePreview?.has,
  ...
  >(() => {
    ...
    withStringService,
    checkoutStringingAdapter.formData,
    checkoutStringingAdapter.linesForSubmit,
    name,
    phone,
    email,
    address,
  ...
  return (
    <div className="min-h-full bg-background">
      ...
      <div className="relative overflow-hidden bg-muted/30 dark:bg-card/40 text-foreground border-b border-border">
        ...
        <SiteContainer
          variant="wide"
          className="relative py-6 bp-sm:py-10 bp-md:py-14"
        >
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <h1 className="text-2xl bp-sm:text-4xl bp-md:text-5xl font-bold mb-2">
                주문/결제
              </h1>
              <p className="text-muted-foreground">
                고객님의 배송/수령/결제정보를 확인 후 주문을 완료하세요
              </p>
            </div>
          </div>
          {/**
          ...
          {(lockServiceMode || withStringService) && (
            <nav aria-label="장착 서비스 진행 단계" className="mt-4">
              <ol className="flex flex-wrap items-center gap-2 text-xs bp-sm:text-sm text-muted-foreground">
                ...
                <li className="flex items-center gap-2">
                  <span className="inline-flex h-6 w-6 items-center justify-center rounded-full border border-border bg-card text-foreground text-xs font-semibold">
                    2
                  </span>
                  <span className="font-medium text-foreground">
                    결제·장착 정보
                ...
                <li className="flex items-center gap-2">
                  <span className="inline-flex h-6 w-6 items-center justify-center rounded-full border border-border bg-card text-muted-foreground text-xs font-semibold">
                    3
                  </span>
              ...
              <p className="mt-2 text-xs text-muted-foreground">
                결제와 함께 교체 서비스 신청이 접수되며, 별도 신청서 작성 없이
                현재 주문에 포함됩니다.
              </p>
            </nav>
          )}
          ...
          <div className="bp-lg:col-span-2 space-y-4 bp-sm:space-y-6">
            ...
            <div className="flex items-start gap-2 rounded-xl border border-border bg-muted px-3 py-2 text-sm text-foreground dark:border-border dark:bg-muted dark:text-foreground">
              ...
              <p className="leading-relaxed">
                <span className="font-semibold">주의:</span> 작성 중에 다른
                  ...
                  {orderItems.map((item) => (
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between bp-sm:flex-col bp-sm:items-end bp-sm:justify-center bp-sm:text-right">
                        ...
                        <div className="font-bold text-base bp-sm:text-lg text-primary">
                ...
                <div className="bg-muted p-4 rounded-lg border border-primary/30">
                  ...
                  {lockServiceMode && (
                    <div className="ml-6 mt-2 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                      <span>
                        ...
                        모드예요. 결제와 함께 교체 서비스 정보가 접수되며 장착
                        정보와 추가 요청도 현재 주문에 함께 저장됩니다.
                      </span>
                  ...
                  {isBundleCheckout && (
                    <p className="text-sm text-foreground ml-6 mt-1">
                      번들 주문은 장착 서비스 포함이{" "}
                      <span className="font-semibold">고정</span>이며, 번들
                      수량은{" "}
                      <span className="font-semibold">{bundleQty}개</span>로{" "}
                      <span className="font-semibold">고정</span>됩니다. 수량
                      변경은{" "}
                      ...
                      (deliveryMethod === "방문수령" ? (
                        ...
                        <div className="ml-7 mt-2 text-sm">
                          <span className="px-2 py-1 rounded bg-primary text-primary-foreground">
                            {collectionMethodLabel("visit")}로 진행됩니다.
                          </span>
                        </div>
                      ) : (
                        ...
                        <div className="ml-7 mt-2 grid gap-2 text-sm">
                          <label className="flex items-center gap-2 text-base bp-sm:text-lg">
                            <input
                              type="radio"
                              name="pickup"
                              checked={servicePickupMethod === "SELF_SEND"}
                              onChange={() =>
                                setServicePickupMethod("SELF_SEND")
                              }
                            />
                    ...
                    <>
                      <div className="space-y-4">
                        ...
                        <Input
                          ...
                          placeholder="우편번호"
                          className={cn(
                            "bg-muted cursor-not-allowed max-w-[200px] border-2",
                            fieldErrors.postalCode && "border-destructive/30",
                          )}
                        />
                      ...
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          placeholder="기본 주소"
                          className={cn(
                            "bg-muted cursor-not-allowed border-2",
                            fieldErrors.postalCode && "border-destructive/30",
                          )}
                        />
                      </div>
                    ...
                    >
                      ...
                      <SelectContent>
                        <SelectItem value="shinhan">
                          신한은행 {bankLabelMap.shinhan.account} (예금주:{" "}
                          {bankLabelMap.shinhan.holder})
                        </SelectItem>
                        <SelectItem value="kookmin">
                          국민은행 {bankLabelMap.kookmin.account} (예금주:{" "}
                          {bankLabelMap.kookmin.holder})
                        </SelectItem>
                        <SelectItem value="woori">
                          우리은행 {bankLabelMap.woori.account} (예금주:{" "}
                          {bankLabelMap.woori.holder})
                        </SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="mt-2 bg-background p-3 bp-sm:p-4 rounded-lg border border-border">
                      ...
                      {user && !pointsFetchError && resolvedPointsDebt > 0 && (
                        <p className="mt-1 text-xs text-destructive">
                          회수 예정 포인트(채무):{" "}
                          {resolvedPointsDebt.toLocaleString()}P → 적립금이 먼저
                          상계됩니다.
                        </p>
                      )}
 ...
 */}
                  ...
                  <Button
                    variant="outline"
                    className="w-full border-2 hover:bg-background dark:hover:bg-muted bg-transparent"
                    asChild
                  >
                    <Link
                      href="/cart"
