...
export default function RacketCompareMobileSections({
  items,
  rows,
  onRemove,
}: {
        ...
        <div className="flex snap-x gap-3 pb-2">
          {items.map((item, index) => {
            ...
            return (
              <article
                key={item.id}
                className="min-w-[220px] max-w-[240px] snap-start rounded-xl border border-border bg-card p-3 shadow-sm"
              >
                ...
                <div className="mt-3 min-w-0 space-y-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-ui-label text-muted-foreground">{brand}</span>
                  </div>
      ...
      {racketCompareRowCategories.map((category) => (
        <section key={category.title} className="space-y-3">
          ...
          {category.rowKeys
            ...
            .filter((row): row is RacketCompareRow => Boolean(row))
            .map((row) => {
              const base = items[0];
              ...
              return (
                ...
                >
                  <div className="space-y-1">
                    <h3 className="text-ui-body-sm font-semibold">{row.label}</h3>
                    {row.hint ? (
                      <p className="break-keep text-ui-label text-muted-foreground">{row.hint}</p>
                    ) : null}
                  </div>
                  <div className="mt-4 divide-y divide-border/60">
                    {items.map((item, index) => {
                      ...
                      const value =
                        row.kind === "text"
                          ? row.getValue(item)
                          : formatNumberValue(row.getValue(item), row);
                      ...
                      return (
                        ...
                        >
                          <div className="min-w-0">
                            <p className="line-clamp-1 text-ui-label text-muted-foreground">
                              {brand} {item.model}
                            </p>
                            {index === 0 ? (
                              <Badge variant="info" className="mt-1 h-5 px-2 text-ui-micro">
                                기준
                              </Badge>
                            ) : null}
                          </div>
