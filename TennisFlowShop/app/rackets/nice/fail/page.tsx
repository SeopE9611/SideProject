...
> = {
  ...
  AMOUNT_MISMATCH: {
    ...
    description: [
      ...
      "라켓 구매 페이지에서 금액을 다시 확인한 뒤 시도해주세요.",
    ],
  },
  SESSION_NOT_FOUND: {
  ...
  ORDER_CREATION_FAILED_AFTER_PAYMENT_APPROVE: {
    ...
    description: [
      "결제 승인은 완료됐지만 라켓 주문 생성 단계에서 문제가 발생했어요.",
  ...
  PAYMENT_PROCESSING_FAILED: {
    ...
    description: [
      "라켓 결제 처리 결과가 아직 명확하게 확정되지 않았어요.",
    ...
    <PaymentFailureResult
      ...
      primaryAction={
        requiresPaymentCheck
          ...
          : { label: "라켓 구매로 돌아가기", href: fallbackHref }
      }
      secondaryAction={
        requiresPaymentCheck
          ? { label: "고객센터로 이동", href: "/support" }
          : { label: "라켓 목록으로 이동", href: "/rackets" }
      }
      warningMessage="결제 승인이 완료됐을 가능성이 있으니 같은 라켓을 바로 반복 결제하지 마시고, 먼저 주문 내역 또는 고객센터에서 상태를 확인해주세요."
    />
  );
}
