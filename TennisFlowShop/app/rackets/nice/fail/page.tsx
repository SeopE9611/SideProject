...
export default async function RacketNiceFailPage({ searchParams }: { searchParams: Promise<{ code?: string; message?: string; fallback?: string }> }) {
    ...
    <div className="mx-auto flex min-h-[60vh] w-full max-w-2xl items-center px-6">
      <Card className="w-full">
        ...
        <CardContent className="space-y-4">
          ...
          {guide.accent === "warning" && <div className="rounded-md border border-warning/30 bg-warning/10 px-3 py-2 text-sm text-warning dark:bg-warning/15">중복 결제를 막기 위해 반복 결제를 피하고, 주문 내역 또는 관리자 확인 후 진행해주세요.</div>}
