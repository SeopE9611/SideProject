  ...
  if (isInvalidLink)
    return (
      <AuthShell
        variant="feature"
        title="유효하지 않은 링크"
        description="비밀번호 재설정 토큰이 없거나 잘못된 접근입니다."
      >
        <div className="space-y-3 text-center">
          <div className="mx-auto grid h-16 w-16 place-items-center rounded-full border border-warning/30 bg-warning/10 text-warning">
            <ShieldAlert aria-hidden="true" className="h-8 w-8" />
          </div>
          <p className="break-keep text-ui-body-sm text-muted-foreground">
            새 링크가 필요하면 비밀번호 재설정을 다시 요청해주세요.
          </p>
          <Button asChild variant="highlight" className="h-12 w-full rounded-control">
            <Link href="/forgot-password">비밀번호 재설정 다시 요청</Link>
          </Button>
          <Button asChild variant="outline" className="h-12 w-full rounded-control">
    ...
    <AuthShell
      ...
      title={
        <span className="inline-flex items-center gap-2">
          <KeyRound aria-hidden="true" className="h-6 w-6 text-brand-highlight-ink" />새 비밀번호
          설정
        </span>
      }
    ...
    >
      {!isDone ? (
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="space-y-3 rounded-control border border-border bg-brand-highlight-muted/35 p-4 text-ui-body-sm text-muted-foreground">
            <p className="text-ui-label font-semibold text-brand-highlight-ink">ACCOUNT RECOVERY</p>
