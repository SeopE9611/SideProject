...
> = {
  ...
  ORDER_CREATION_FAILED_AFTER_PAYMENT_APPROVE: {
    ...
    description: [
      "결제 승인은 완료됐지만 패키지 주문 생성 단계에서 문제가 발생했어요.",
      "중복 결제를 피하기 위해 패키지 내역 또는 관리자 확인이 필요합니다.",
    ],
    accent: "warning",
  },
  UNKNOWN: {
...
export default async function PackageNiceFailPage({
  searchParams,
}: {
        ...
        <div className="space-y-3 text-ui-body-sm text-muted-foreground">
          {guide.accent === "warning" && (
            <p className="text-warning">
              중복 결제를 막기 위해 같은 패키지로 반복 결제하지 마시고, 패키지 내역 또는 관리자 확인
              후 진행해주세요.
            </p>
          )}
