...
const FAIL_GUIDE_MAP: Record<string, { title: string; description: string[]; accent?: "default" | "warning" }> = {
  ...
  INVALID_QUERY: {
    ...
    description: ["결제 결과를 확인하는 중 필수 값이 누락되었어요.", "패키지 체크아웃으로 돌아가 다시 결제를 진행해주세요."],
  },
  AUTH_FAILED: {
  ...
  AMOUNT_MISMATCH: {
    ...
    description: ["결제 금액이 패키지 정보와 일치하지 않아 결제가 중단되었어요.", "패키지 체크아웃으로 돌아가 금액을 다시 확인한 뒤 시도해주세요."],
  },
  SESSION_NOT_FOUND: {
    title: "결제 세션을 찾지 못했어요",
    description: ["결제 준비 정보가 없어 결제를 완료할 수 없어요.", "패키지 체크아웃에서 다시 결제를 진행해주세요."],
  },
  SESSION_EXPIRED: {
    title: "결제 유효시간이 만료되었어요",
    description: ["결제 준비 시간이 지나 결제를 이어서 진행할 수 없어요.", "패키지 체크아웃으로 돌아가 다시 결제를 시도해주세요."],
  },
  ORDER_CREATION_FAILED_AFTER_PAYMENT_APPROVE: {
    title: "결제 후 주문 처리 중 문제가 발생했어요",
    description: ["결제 승인은 완료됐지만 패키지 주문 생성 단계에서 문제가 발생했어요.", "중복 결제를 피하기 위해 패키지 내역 또는 관리자 확인이 필요합니다."],
    accent: "warning",
  },
  UNKNOWN: {
    title: "패키지 결제를 완료하지 못했어요",
    description: ["결제 처리 중 문제가 발생했어요.", "패키지 체크아웃으로 돌아가 다시 시도해주세요."],
  },
};

export default async function PackageNiceFailPage({ searchParams }: { searchParams: Promise<{ code?: string; message?: string }> }) {
      ...
      <Card className="w-full">
        ...
        <CardContent className="space-y-4">
          ...
          {guide.accent === "warning" && (
            <div className="rounded-md border border-warning/30 bg-warning/10 px-3 py-2 text-sm text-warning dark:bg-warning/15">중복 결제를 막기 위해 같은 패키지로 반복 결제하지 마시고, 패키지 내역 또는 관리자 확인 후 진행해주세요.</div>
          )}
