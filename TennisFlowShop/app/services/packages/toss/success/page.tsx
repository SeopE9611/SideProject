...
export default function PackageTossSuccessPage() {
  ...
  useEffect(() => {
    ...
    if (!paymentKey || !orderId || !Number.isFinite(amount)) {
      router.replace("/services/packages/toss/fail?code=INVALID_QUERY&message=잘못된 결제 결과입니다.");
      return;
    }
    ...
    fetch("/api/payments/toss/package/confirm", {
      ...
      .then(async (res) => {
        ...
        if (!res.ok || !json?.packageOrderId) {
          ...
          router.replace(`/services/packages/toss/fail?code=${encodeURIComponent(nextCode)}&message=${nextMessage}`);
          return;
        }
        router.replace(`/services/packages/success?packageOrderId=${encodeURIComponent(json.packageOrderId)}`);
      })
      .catch((error: any) => {
        router.replace(`/services/packages/toss/fail?code=CONFIRM_FAILED&message=${encodeURIComponent(error?.message || "카드/간편결제 승인에 실패했습니다.")}`);
      });
  }, [router, sp]);
