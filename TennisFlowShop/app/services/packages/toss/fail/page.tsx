...
const FAIL_GUIDE_MAP: Record<string, { title: string; description: string[]; accent?: "default" | "warning" }> = {
  ...
  ORDER_CREATION_FAILED_AFTER_PAYMENT_CONFIRM: {
    ...
    description: ["결제 승인 이후 패키지 주문 생성 단계에서 오류가 발생했어요.", "이미 결제가 완료되었을 수 있으니 주문 내역 또는 관리자 확인이 필요합니다."],
    accent: "warning",
  },
  UNKNOWN: {
    title: "패키지 결제를 완료하지 못했어요",
...
export default async function PackageTossFailPage({ searchParams }: { searchParams: Promise<{ code?: string; message?: string }> }) {
      ...
      <Card className="w-full">
        ...
        <CardContent className="space-y-4">
          ...
          {guide.accent === "warning" && (
            <div className="rounded-md border border-warning/30 bg-warning/10 px-3 py-2 text-sm text-warning dark:bg-warning/15">중복 결제를 막기 위해 같은 패키지로 반복 결제하지 마시고, 주문 내역 또는 관리자 확인 후 진행해주세요.</div>
          )}
