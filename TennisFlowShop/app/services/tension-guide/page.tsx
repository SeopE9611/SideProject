...
export default function TensionGuidePage() {
  ...
  const basisRows = [
    ...
    {
      label: "플레이 스타일 보정",
      value: `${styleAdjust > 0 ? `+${styleAdjust}` : styleAdjust}LB`,
    },
  ...
  return (
    <div className="min-h-screen bg-background">
      <section className="border-b border-border bg-muted/30">
        <SiteContainer className="grid gap-8 py-10 bp-lg:grid-cols-[minmax(0,1fr)_24rem] bp-lg:items-center bp-lg:py-14">
          <div className="space-y-6">
            <Badge
              variant="outline"
              className="w-fit gap-2 border-brand-highlight-ink/30 bg-brand-highlight-muted text-brand-highlight-ink"
            >
            ...
            <div className="max-w-3xl space-y-4">
              ...
              <p className="text-ui-body leading-relaxed text-muted-foreground bp-md:text-ui-body-lg">
                성별, 스윙 스피드, 스트링 타입과 플레이 스타일을 기준으로 시작 장력을 확인하고 실제
                타구감에 맞춰 1LB 단위로 조정해 보세요.
              </p>
            </div>
      ...
      <SiteContainer className="py-6 bp-md:py-10 bp-lg:pb-16">
        <Tabs
          id="tension-guide-workspace"
          value={activeSection}
          onValueChange={setActiveSection}
          className="mb-8 scroll-mt-[calc(var(--header-h)+1rem)] bp-md:mb-12"
        >
          <TabsList className="grid h-auto w-full grid-cols-2 gap-1 bg-muted p-1 bp-sm:grid-cols-4">
            {[
              ...
              <TabsTrigger
                key={tab.value}
                value={tab.value}
                className="gap-1.5 rounded-lg border border-transparent py-2.5 text-ui-label text-muted-foreground data-[state=active]:border-border data-[state=active]:bg-card data-[state=active]:text-foreground data-[state=active]:shadow-sm bp-sm:text-ui-body-sm"
              >
          ...
          <TabsContent value="calculator" className="mt-8 space-y-6">
            ...
            <PublicSurface variant="feature" padding="none" className="overflow-hidden">
              <div className="grid bp-lg:grid-cols-[minmax(0,1fr)_minmax(22rem,0.9fr)]">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {[
                          ...
                          { id: "male", label: "남자", desc: "남성 추천 범위" },
                        ].map((option) => (
                          <button
                            key={option.id}
                            type="button"
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        {swingSpeedOptions.map((option) => (
                          <button
                            key={option.id}
                            type="button"
                            aria-pressed={swingSpeed === option.id}
                          ...
                          >
                            ...
                            {swingSpeed === option.id && (
                              <CheckCircle2
                                className="absolute right-2 top-2 h-3.5 w-3.5 text-brand-highlight-ink"
                                aria-hidden
                              />
                            )}
                          </button>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {stringTypes.map((st) => (
                          ...
                          >
                            ...
                            <span className="mt-1 block text-ui-label text-muted-foreground tabular-nums">
                              여 {st.ranges.female.min}~{st.ranges.female.max} · 남{" "}
                              {st.ranges.male.min}~{st.ranges.male.max}LB
                            </span>
                            {stringType === st.id && (
                              <CheckCircle2
                                className="absolute right-2 top-2 h-4 w-4 text-brand-highlight-ink"
                                aria-hidden
                              />
                            )}
                          </button>
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        {playStyleOptions.map((option) => (
                          <button
                            key={option.id}
                            type="button"
                            aria-pressed={playStyle === option.id}
                ...
                <div
                  className="bg-surface-inverse p-4 text-surface-inverse-foreground bp-sm:p-6 bp-lg:p-8"
                  aria-live="polite"
                >
                  <div className="space-y-6">
                    <div>
                      <p className="text-ui-label font-medium uppercase tracking-[0.14em] text-surface-inverse-muted">
                        추천 텐션
                      </p>
                      ...
                      <p className="mt-2 text-ui-body text-surface-inverse-muted">
                    ...
                    <div
                      ...
                      aria-valuemin={42}
                      aria-valuemax={56}
                      aria-valuenow={calculatedTension}
                      aria-label={`추천 텐션 ${calculatedTension}LB, 전체 축 42LB에서 56LB 사이`}
                      className="space-y-2"
                    >
                      <div className="flex justify-between text-ui-label text-surface-inverse-muted">
                        <span>42LB</span>
                        <span>56LB</span>
                      </div>
                      <div className="relative h-3 rounded-full bg-surface-inverse-foreground/15">
                        <div
                          className="absolute top-1/2 h-5 w-5 -translate-x-[10px] -translate-y-1/2 rounded-full border-2 border-brand-highlight bg-surface-inverse transition-[left] duration-500"
                    ...
                    <div>
                      ...
                      <ul className="mt-2 divide-y divide-surface-inverse-foreground/15">
                        {basisRows.map((row) => (
                          <li
                            key={row.label}
                            className="flex items-center justify-between gap-4 py-2 text-ui-body-sm"
                          >
                            <span
                              className={
                                row.emphasis
            ...
            <PublicSurface className="space-y-4">
              ...
              />{" "}
              <div className="grid bp-sm:grid-cols-2">
                {environmentFactors.map((factor, index) => (
                  <div
                    key={factor.factor}
                    className="flex gap-3 border-t border-border pt-4 bp-sm:border-l bp-sm:border-t-0 bp-sm:pl-5 bp-sm:pt-0 bp-sm:first:border-l-0 bp-sm:first:pl-0"
                  >
                    <factor.icon
          ...
          <TabsContent value="levels" className="mt-8 space-y-6">
            ...
            <PublicSurface padding="none" className="overflow-hidden">
              {playerTypes.map((player, index) => {
                ...
                return (
                  <div key={player.type} className="border-t border-border first:border-t-0">
                    <button
                      type="button"
                      aria-expanded={isSelected}
                      onClick={() => setSelectedLevel(isSelected ? null : index)}
                      className="flex w-full flex-col gap-4 p-4 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring bp-md:p-6"
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex gap-3">
                          <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-border bg-muted/30 text-muted-foreground">
                        ...
                        <ChevronRight
                          className={`h-5 w-5 shrink-0 text-muted-foreground transition-transform ${isSelected ? "rotate-90" : ""}`}
                          aria-hidden
                        />
                      </div>
                      <div className="grid gap-4 bp-md:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
                        <div className="grid grid-cols-2 gap-2 text-ui-body-sm">
                          ...
                          <Badge variant="outline" className="justify-center tabular-nums">
                            남자 {player.maleTension}
                          </Badge>
                        </div>
                        <div className="space-y-3">
                          <div className="space-y-1">
                            <div className="flex items-center justify-between gap-2 text-ui-label text-muted-foreground">
                              <span className="font-medium text-foreground">
                                여자 · {player.femaleTension}
                              </span>
                              <span>
                                {tensionAxis.min}~{tensionAxis.max}LB 축
                              </span>
                            </div>
                            <div className="relative h-2 rounded-full bg-muted">
                              <div
                                className="absolute h-full rounded-full bg-brand-highlight-muted ring-1 ring-brand-highlight-ink/30"
                                style={getRangeBarStyle(
                                  player.femaleRange[0],
                                  player.femaleRange[1],
                                )}
                              />
                            </div>
                          </div>
                          <div className="space-y-1">
                            ...
                            <div className="relative h-2 rounded-full bg-muted">
                              <div
                                className="absolute h-full rounded-full bg-muted-foreground/35"
                                style={getRangeBarStyle(player.maleRange[0], player.maleRange[1])}
                              />
                            </div>
                          </div>
                        </div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {player.characteristics.map((char) => (
                          <span
                            key={char}
                            className="rounded-full bg-muted/50 px-2.5 py-1 text-ui-label text-muted-foreground"
                          >
                            {char}
                          </span>
                        ))}
                    ...
                    {isSelected && (
                      <div className="border-t border-border bg-muted/30 p-4 bp-md:px-6">
                        <p className="flex gap-2 text-ui-body-sm text-foreground">
                          <Lightbulb
          ...
          <TabsContent value="strings" className="mt-8 space-y-6">
            ...
            />
            <div className="grid grid-cols-2 gap-2 bp-lg:grid-cols-5">
              {stringTypes.map((st) => (
                <button
                  key={st.id}
                  type="button"
              ...
              <div className="flex flex-col gap-3 bp-md:flex-row bp-md:items-start bp-md:justify-between">
                <div>
                  ...
                  <p className="mt-2 text-ui-body-sm leading-relaxed text-muted-foreground">
                    {selectedString.characteristics}
                  </p>
                </div>
                <div className="flex flex-wrap gap-2">
                  <Badge variant="secondary" className="tabular-nums">
                    여자 {selectedString.ranges.female.min}~{selectedString.ranges.female.max}LB
                  </Badge>
                  <Badge variant="outline" className="tabular-nums">
                    남자 {selectedString.ranges.male.min}~{selectedString.ranges.male.max}LB
                  </Badge>
                </div>
              </div>
              {selectedString.helperText && (
                <p className="rounded-xl bg-muted/30 p-3 text-ui-body-sm text-muted-foreground">
                  {selectedString.helperText}
                </p>
              )}
          ...
          <TabsContent value="tips" className="mt-8 space-y-6">
            ...
            />
            <PublicSurface padding="none" className="overflow-hidden">
              <div className="grid bp-md:grid-cols-2">
                {[
                  {
                    title: "낮은 텐션",
                    desc: "파워와 편안함 중심",
                    icon: <TrendingUp className="h-5 w-5 rotate-180" aria-hidden />,
                    items: [
                      "스트링 베드가 더 많이 휘어져 볼에 더 많은 에너지를 전달합니다",
                      "스윗스팟이 넓어져 미스히트 시에도 괜찮은 샷이 나옵니다",
                      "팔과 어깨에 가해지는 충격이 줄어들어 부상 위험이 감소합니다",
                    ],
                    caution: "정밀한 컨트롤이 어려울 수 있습니다",
                  },
                  {
                    title: "높은 텐션",
                    desc: "컨트롤과 정밀함 중심",
                    ...
                    caution: "팔에 무리가 갈 수 있으며 파워가 줄어들 수 있습니다",
                  },
                ].map((tip) => (
                  <div
                    key={tip.title}
                    className="space-y-4 border-t border-border p-4 first:border-t-0 bp-md:border-l bp-md:border-t-0 bp-md:p-6 bp-md:first:border-l-0"
                  >
                    <div className="flex items-center gap-3">
                      <span className="flex h-10 w-10 items-center justify-center rounded-xl border border-border bg-muted/30 text-muted-foreground">
                        {tip.icon}
                      </span>
                      <div>
                        <h3 className="text-ui-card-title font-semibold text-foreground">
                    ...
                    <ul className="space-y-2">
                      {tip.items.map((item) => (
                        <li key={item} className="flex gap-2 text-ui-body-sm text-muted-foreground">
                          <CheckCircle2
                            className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground"
                            aria-hidden
                          />
                      ...
                      <li className="flex gap-2 text-ui-body-sm text-muted-foreground">
                        <AlertTriangle
                          className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground"
                          aria-hidden
                        />
            ...
            <PublicSurface padding="none" className="overflow-hidden">
              <div className="grid bp-lg:grid-cols-3">
                {[
                  {
                    title: "텐션 손실 이해하기",
                    icon: Gauge,
                    body: "스트링은 장착 후 지속적으로 텐션이 감소합니다.",
                    items: [
                      "첫 24시간: 10-15% 손실",
                      "첫 주: 추가 5-10% 손실",
                      "이후: 점진적 안정화",
                  ...
                  },
                  {
                    title: "스트링 보관 팁",
                    icon: Shield,
                    body: "올바른 보관은 스트링 수명과 텐션 유지에 중요합니다.",
                    items: ["직사광선 피하기", "극단적 온도 피하기", "라켓 커버 사용"],
                  },
                  {
                    title: "교체 시기 판단",
                    icon: Target,
                    body: "다음 신호가 나타나면 교체를 고려하세요.",
                    items: ["탄력 감소 느낌", "노칭(홈) 발생", "변색 또는 보풀"],
                  },
                ].map((tip) => (
                  <div
                    key={tip.title}
                    className="border-t border-border p-4 first:border-t-0 bp-lg:border-l bp-lg:border-t-0 bp-lg:p-6 bp-lg:first:border-l-0"
                  >
              ...
              <Accordion type="single" className="mt-2">
                <AccordionItem value="main-cross">
                  <AccordionTrigger value="main-cross" className="gap-3 text-foreground">
                    <span className="min-w-0">메인과 크로스 텐션을 다르게 해야 하나요?</span>
                  </AccordionTrigger>
                  <AccordionContent
                    value="main-cross"
                    className="pb-4 text-ui-body-sm leading-relaxed text-muted-foreground"
                  >
                    반드시 필요하지는 않지만, 일부 선수들은 메인을 크로스보다 2-4LB 높게 설정합니다.
                    이는 스윗스팟을 확장하고 컨트롤과 파워의 균형을 맞추는 데 도움이 됩니다.
                  </AccordionContent>
                </AccordionItem>
                <AccordionItem value="new-racket">
                  <AccordionTrigger value="new-racket" className="gap-3 text-foreground">
                    <span className="min-w-0">새 라켓에는 어떤 텐션으로 시작해야 하나요?</span>
                  </AccordionTrigger>
                  <AccordionContent
                    value="new-racket"
                    className="pb-4 text-ui-body-sm leading-relaxed text-muted-foreground"
                ...
                <AccordionItem value="change-often">
                  <AccordionTrigger value="change-often" className="gap-3 text-foreground">
                    <span className="min-w-0">텐션을 자주 바꿔도 되나요?</span>
                  </AccordionTrigger>
                  <AccordionContent
                    value="change-often"
                    className="pb-4 text-ui-body-sm leading-relaxed text-muted-foreground"
                  >
                    일관된 텐션을 유지하는 것이 플레이 향상에 도움이 됩니다. 하지만 계절 변화나 코트
                    조건에 따라 2-4LB 정도 조절하는 것은 권장됩니다.
                  </AccordionContent>
                </AccordionItem>
                <AccordionItem value="pro">
                  <AccordionTrigger value="pro" className="gap-3 text-foreground">
                    <span className="min-w-0">프로 선수들은 어떤 텐션을 사용하나요?</span>
                  </AccordionTrigger>
                  <AccordionContent
                    value="pro"
                    className="pb-4 text-ui-body-sm leading-relaxed text-muted-foreground"
        ...
        <PublicSurface className="flex flex-col gap-5 bg-muted/30 bp-lg:flex-row bp-lg:items-center bp-lg:justify-between">
          <div className="space-y-2">
            ...
            <p className="text-ui-body-sm text-muted-foreground">
              계산값을 기준으로 스트링 서비스를 신청하거나, 라켓 검색으로 나에게 맞는 라켓을 함께
              확인할 수 있습니다.
            </p>
          </div>
