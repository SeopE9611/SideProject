...
function isHybridMaterial(material: string | undefined): boolean {
  ...
  const hasMulti = value.includes('multi') || value.includes('멀티') || value.includes('인조') || value.includes('synthetic');
  const hasGut = value.includes('gut') || value.includes('거트') || value.includes('내추럴') || value.includes('natural');
...
function toMaterialKey(material: string | undefined): MaterialKey {
  const value = String(material ?? '').toLowerCase();
  if (value.includes('natural') || value.includes('내추럴') || value.includes('천연')) return 'naturalGut';
  if ((value.includes('gut') || value.includes('거트')) && !(value.includes('synthetic') || value.includes('인조'))) return 'naturalGut';
  if (value.includes('synthetic') || value.includes('인조') || value.includes('multi') || value.includes('멀티')) return 'syntheticGut';
...
export async function getHybridGuideSummary(): Promise<HybridGuideSummary> {
  ...
  return {
    ...
    representativeProducts: hybridRows.map((row) => String(row.name ?? '').trim()).filter(Boolean).slice(0, 3),
  };
}

export async function getStringingPricingView() {
  const [summaries, hybridGuide] = await Promise.all([getStringingMaterialSummaries(), getHybridGuideSummary()]);
  return {
