...
async function fire(
  ...
  const rendered = await renderForEvent(eventType, {
    ...
    adminDetailUrl: ctx.adminDetailUrl,
  });
  const outboxId = await upsertOutbox({
    eventType,
    channels: ctx.channels,
    payload: { user: ctx.user, application: ctx.application },
    rendered,
...
export async function onStatusUpdated(params: {
  user: UserCtx;
  application: ApplicationCtx;
  adminDetailUrl: string;
}) {
...
export async function onScheduleConfirmed(params: {
  ...
  await fire("stringing.schedule_confirmed", {
    ...params,
...
export async function onScheduleUpdated(params: {
  user: UserCtx;
  application: ApplicationCtx;
}) {
...
export async function onApplicationCanceled(params: {
  user: UserCtx;
  application: ApplicationCtx;
}) {
...
export async function onScheduleCanceled(params: {
  user: UserCtx;
  application: ApplicationCtx;
}) {
