...
export async function renderForEvent(
  ...
  if (event === "stringing.application_submitted") {
    ...
    const ctasBase = [
      {
        label: "신청서 상세 보기",
        url: `${baseUrl}/mypage?tab=orders&flowType=application&flowId=${app.applicationId}&from=orders`,
      },
      {
        label: "일정 변경",
        url: `${baseUrl}/services/apply?orderId=${app.orderId}`,
      },
    ];
  ...
  if (event === "stringing.schedule_updated") {
    ...
    const html = wrapEmail({
      ...
      ctas: [
        {
          label: "신청서 상세 보기",
          url: `${baseUrl}/my/applications/${app.applicationId}`,
        },
      ],
    });
    const ics = buildICS(app);
