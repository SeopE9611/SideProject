...
export default function FinalRequestSection({ formData, setFormData, handleInputChange, orderId, isMember, usingPackage, packageInsufficient, context = 'apply' }: FinalRequestSectionProps) {
    ...
    <div className="space-y-6">
        ...
        <div className="bg-muted/40 dark:bg-muted/30 border border-border rounded-lg p-4">
          <div className="flex items-start space-x-3">
            ...
            <div className="text-sm">
              ...
              <p className="leading-relaxed text-foreground">
                배송/연락처 정보는 상단에서 입력한 <span className="font-semibold">현재 checkout 정보</span> 기준으로 함께 접수됩니다.
                <br />
                장착 관련 추가 요청사항이 있으면 아래 <span className="font-semibold text-primary">추가 요청사항</span>에 남겨주세요.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="space-y-2">
        ...
        <p className="text-xs text-muted-foreground">
          {context === 'checkout' ? '장착 관련 요청이나 전달이 필요한 메모가 있다면 이곳에 남겨주세요.' : '요청사항이 많거나 중요한 정보(주소 변경, 연락처 변경 등)가 있다면 이곳에 남겨주세요.'}
        </p>
      </div>
    </div>
  );
