...
export default function StringingApplicationDetailClient({
  ...
  const handleConfirmCancelRequest = (params: {
    ...
    startTransition(async () => {
      try {
        ...
        console.error(err);
        showErrorToast(
          err instanceof Error ? err.message : "취소 요청 처리 중 오류가 발생했습니다.",
        );
      }
    });
  };
  ...
  const resolveLinkedOrderItemPriceDisplay = (item: {
    price?: number | null;
    salePrice?: number | null;
    regularPrice?: number | null;
  }) => {
    const displayUnitPrice = toFinitePriceOrNull(item.salePrice) ?? toFinitePriceOrNull(item.price);

    const regularPrice = toFinitePriceOrNull(item.regularPrice);
    const hasDiscount =
      displayUnitPrice !== null && regularPrice !== null && regularPrice > displayUnitPrice;
    ...
    return {
      ...
      hasDiscount,
      discountRate:
        hasDiscount && displayUnitPrice !== null && regularPrice
              ...
              <>
                ...
                {!isAdmin && (
                  ...
                  >
                    <div
                      className={
                        isAdmin ? summaryCardClass : "rounded-xl bg-muted/15 p-3 bp-sm:p-4"
                      }
                    >
                        ...
                        <div className="rounded-md border border-border/60 bg-background px-3 py-2">
                          ...
                          {needsCancelRefundAccount || cancelInfo.refundAccount ? (
                            ...
                            <p className="mt-2 text-ui-label text-muted-foreground">
                              ...
                              결제/패키지/결제대기 건은 별도 계좌 정보 없이 처리됩니다.
                            </p>
                          )}
                        </div>
                      }
              ...
              <section className="mb-6 space-y-4" aria-label="관리자 운영 콘솔">
                <div className={adminSurface.statusGrid}>
                  ...
                  <AdminStatusCard
                    ...
                    description={
                      packageApplied
                        ? "결제대기가 아니라 패키지 회차 차감으로 처리됩니다."
                        : applicationContext.payment
                    }
                    icon={CreditCard}
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-ui-label text-foreground/75">
                                    {item.selectedGauge && (
                                      <span>게이지(굵기): {item.selectedGauge}</span>
                                    )}
                                    {colorLabel && <span>색상: {colorLabel}</span>}

                                    {(() => {
                                      const priceDisplay = resolveLinkedOrderItemPriceDisplay(item);

                                      if (item.priceSnapshotStatus === "needs_review") {
                                        return <span>가격 스냅샷 확인 필요</span>;
                                      }

                                      if (priceDisplay.displayUnitPrice === null) return null;

                                      return (
                                        <>
                                          <span>
                                            판매가: {priceDisplay.displayUnitPrice.toLocaleString()}
                                            원
                                          </span>

                                          {priceDisplay.hasDiscount &&
                                            priceDisplay.regularPrice !== null && (
                                              <span className="text-muted-foreground">
                                                정가:{" "}
                                                <span className="line-through">
                                                  {priceDisplay.regularPrice.toLocaleString()}원
                                                </span>
                                    ...
                                    {typeof item.stringPrice === "number" && (
                                      <span>
                                        스트링 가격: {item.stringPrice.toLocaleString()}원
                                      </span>
                                    )}
                        ...
                        <section className="space-y-3">
                          <div className="flex items-center gap-2 text-foreground">
                            ...
                            <span className="font-medium">
                              {isAdmin ? "라켓·스트링별 작업 정보" : "스트링/텐션 요약"}
                            </span>
                          </div>
                          <div
                            className={cn(
                          ...
                          >
                            <div className="flex flex-wrap gap-x-4 gap-y-1.5 text-ui-label leading-relaxed text-foreground/75">
                              ...
                              {(isAdmin || lineSummary.notedCount > 0) && (
              ...
              <div className={detailColumnClass}>
                ...
                >
                  ...
                  >
                    <CardTitle
                      className={cn(
                        "text-ui-card-title font-medium flex items-center gap-2",
                        detailCardTitleClass,
                      )}
                    >
                            ...
                            <AdminCompactField
                              ...
                              valueClassName={
                                isAdmin
                                  ...
                                  : "font-semibold text-brand-highlight-ink"
                              }
                            />
                            <AdminCompactField
                              label="결제 상태"
                          ...
                          <div className="space-y-3">
                            ...
                            {shouldShowCustomerBankAccount && standaloneBankInfo && (
                              <div className="rounded-xl border border-border/70 bg-muted/20 px-3 py-2 text-ui-body-sm text-foreground/85">
                                ...
                                <p className="text-muted-foreground">
                                  예금주: {standaloneBankInfo.holder}
                                </p>
                                {standaloneDepositor && (
                                  <p className="text-muted-foreground">
                                    입금자명: {standaloneDepositor}
                                  </p>
                                )}
                              </div>
                            )}
                  ...
                  <CardContent className={cn("grid gap-4 p-4 md:grid-cols-2 bp-sm:p-6", isAdmin && "!grid-cols-2 !p-6")}>
                    ...
                    <div className={cn("border-l-2 border-border bg-background/60 px-3 py-3 bp-sm:px-4", isAdmin && "!px-4")}>
                      ...
                      {data.shippingInfo?.selfShip?.trackingNo ? (
                        ...
                        <AdminInlineEmpty className="mt-2">
                          라켓 발송 운송장 미등록
                        </AdminInlineEmpty>
                      )}
                    </div>
