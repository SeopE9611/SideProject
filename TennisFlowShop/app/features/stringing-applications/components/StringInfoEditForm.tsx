...
export default function StringInfoEditForm({
  ...
  const isDirty = useMemo(() => {
    ...
    const stringsChanged =
      curStrings.length !== baseline.stringTypes.length ||
      curStrings.some((v, i) => v !== baseline.stringTypes[i]);

    return (
      curDesired !== baseline.desiredDateTime ||
      stringsChanged ||
      (customStringType ?? "") !== baseline.customStringName ||
      (racketType ?? "") !== baseline.racketType
    );
