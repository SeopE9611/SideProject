...
export async function handleUpdateShippingInfo(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    if (isOriginalVisitContext) {
      if (nextMethodNormalized && nextMethodNormalized !== 'visit') {
        return NextResponse.json(
          {
            ok: false,
            message: '방문 수령 신청은 배송 방식 변경이 불가합니다. 예외 전환은 별도 관리자 절차를 통해 진행해주세요.',
          },
          { status: 400 },
        );
      }
    ...
    if ((app as any).orderId) {
      ...
      const shouldAutoTransitLinkedOrder = !isOriginalVisitContext && isStoreRegister && linkedOrderPaid && !ORDER_TERMINAL_STATUSES.has(linkedOrderStatus) && !ORDER_SHIPPING_PHASE_STATUSES.has(linkedOrderStatus);
      ...
      await db.collection('orders').updateOne({ _id: new ObjectId((app as any).orderId) }, orderUpdateDoc);
    }
...
export async function handleGetReservedTimeSlots(req: Request) {
  try {
    ...
    if (!win.ok) {
      return NextResponse.json(
        {
          message: win.message ?? `예약 가능 기간을 벗어났습니다. (오늘부터 ${win.windowDays}일 이내만 예약 가능)`,
        },
        { status: 400 },
      );
    }
...
export async function handleSubmitStringingApplication(req: Request) {
  ...
  const guestOrderId = guestClaims && 'orderId' in guestClaims && typeof guestClaims.orderId === 'string' ? guestClaims.orderId : null;
  const guestRentalId = guestClaims && 'rentalId' in guestClaims && typeof guestClaims.rentalId === 'string' ? guestClaims.rentalId : null;
  ...
  try {
    ...
    const response = NextResponse.json({ message: 'success', applicationId: result.applicationId }, { status: 201 });
    ...
    const hasRentalIdInPayload = typeof body?.rentalId === 'string' && body.rentalId.trim().length > 0;
    const isGuestStandaloneSubmit = !userId && !hasGuestOrderToken && !hasGuestRentalToken && !hasOrderIdInPayload && !hasRentalIdInPayload;
