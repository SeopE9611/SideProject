...
export const normalizeOrderShippingMethod = (
  ...
  if (["visit", "pickup", "방문", "방문수령", "매장", "shop_visit"].includes(s))
    return "visit";
...
export const collectionVisitNotice =
  "방문 접수는 주소 입력이 필요하지 않습니다.";

export const getStringingAddressReadLabels = (
  raw: unknown,
): {
  sectionTitle: string;
  primaryLabel: string;
  ...
  return {
    ...
    primaryValue: isVisit
      ? `${collectionMethodLabel(method)} (주소 입력 불필요)`
      : "-",
...
export const getOrderShippingReadLabels = (
  raw: unknown,
): {
  sectionTitle: string;
  primaryLabel: string;
  ...
  return {
    ...
    primaryValue: isVisit
      ? `${orderShippingMethodLabel(method)} (주소 입력 불필요)`
      : "-",
  };
};

export const withAddressValue = (
  raw: unknown,
  address: string | null | undefined,
): string => {
  const method = normalizeCollection(raw);
  if (method === "visit")
    return `${collectionMethodLabel(method)} (주소 입력 불필요)`;
  return address?.trim() || "-";
};

export const withPostalValue = (
  raw: unknown,
