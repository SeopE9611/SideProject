...
export default function PrivatePaymentClient({ item }: { item: Item }) {
  ...
  if (item.status === "inactive") return <ResultState title="현재 사용할 수 없는 결제 링크입니다." description="관리자에게 문의해 주세요."><Button asChild><Link href="/">홈으로 이동</Link></Button></ResultState>;
  if (item.paymentStatus === "결제완료") return <ResultState title="이미 결제가 완료된 링크입니다." description="결제 상태는 관리자에게 문의해 주세요."><Button asChild><Link href="/">홈으로 이동</Link></Button></ResultState>;
  if (item.paymentStatus === "결제취소") return <ResultState title="취소된 결제 링크입니다." description="관리자에게 문의해 주세요."><Button asChild><Link href="/">홈으로 이동</Link></Button></ResultState>;
  if (item.paymentStatus !== "결제대기") return <ResultState title="결제할 수 없는 링크입니다." description="관리자에게 문의해 주세요."><Button asChild><Link href="/">홈으로 이동</Link></Button></ResultState>;
