...
export default function SocialAuthButtons({
  ...
  const baseButtonClass =
    "h-12 w-full justify-start rounded-control border border-border bg-card px-4 text-ui-body-sm font-semibold text-foreground shadow-soft transition-colors hover:border-brand-highlight-ink/30 hover:bg-brand-highlight-muted/50 disabled:cursor-not-allowed disabled:opacity-50";
  const iconClass =
    "inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-control border border-brand-highlight-ink/15 bg-brand-highlight-muted text-brand-highlight-ink";
      ...
      {onNaverClick ? (
        <Button type="button" onClick={onNaverClick} className={baseButtonClass}>
          <span className={iconClass}>
            ...
            >
              <path
                d="M13.6202 10.7393L6.91699 2H3V18H6.37983V9.26071L13.083 18H17V2H13.6202V10.7393Z"
                fill="currentColor"
              />
            </svg>
          </span>
