...
import { getVariantsByColor, getVisibleColorRows, hasSelectableStringStock, isSellableVariant, normalizeColorRows, normalizeGaugeRows } from "@/lib/products/string-stock";
import { cn } from "@/lib/utils";
import { ArrowLeft, Check, ChevronDown, Filter, Grid3X3, List, Minus, Plus, Search, ShoppingBag, X } from "lucide-react";
...
type SelectStringLayoutProps = {
  ...
  onSelectString: (params: { stringProduct: any; selectedGauge?: string; selectedColor?: string; workCount: number }) => void;
...
export default function SelectStringLayout({
  ...
  const filteredProducts = useMemo(() => {
    ...
    return (products ?? []).filter((product: any) => {
      const haystack = [product?.name, product?.brand, product?.material].map((value) => String(value ?? "").toLowerCase()).join(" ");
    ...
    <div className="overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
      ...
      <div className="border-b border-border bg-secondary/30 px-5 py-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-foreground">{flowType === "rental" ? "대여 라켓 요약" : "선택된 라켓"}</span>
        </div>
      </div>
      ...
      <div className="p-4 bp-md:p-5">
        <div className="flex gap-3 bp-md:gap-4">
          <div className="relative h-16 w-16 flex-shrink-0 overflow-hidden rounded-xl bg-secondary/50 bp-md:h-20 bp-md:w-20">
            {racket.image ? (
              <Image src={racket.image} alt={racket.name} fill sizes="80px" className="object-cover" />
            ) : (
          ...
          <div className="min-w-0 flex-1">
            <h3 className="line-clamp-2 min-w-0 break-keep text-sm font-semibold leading-tight text-foreground">{racket.name}</h3>
            {flowType === "rental" && rentalPeriod && (
              <div className="mt-2 inline-flex rounded-full border border-border bg-muted/30 px-2.5 py-1 text-xs text-muted-foreground">
                대여 기간 <span className="ml-1 font-medium tabular-nums text-foreground">{rentalPeriod}일</span>
              </div>
            )}
            {flowType === "purchase" && racket.price != null && (
              <div className="mt-2 space-y-1 tabular-nums">
                <div className="flex flex-wrap items-baseline gap-x-2 gap-y-1">
                  <span className="text-xs font-medium text-muted-foreground">{racket.regularPrice && racket.regularPrice > racket.price ? "할인가" : "판매가"}</span>
                  <span className="text-base font-bold text-foreground">{racket.price.toLocaleString()}원</span>
                </div>
                {racket.regularPrice && racket.regularPrice > racket.price && (
                  <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-xs">
                    <span className="text-muted-foreground">정가</span>
                    <span className="text-muted-foreground line-through">{racket.regularPrice.toLocaleString()}원</span>
        ...
        {showQuantityControls && (
          <div className="mt-5 rounded-xl border border-border bg-secondary/30 p-4">
            ...
            <div className="flex items-center justify-between gap-3">
              <p className="text-xs text-muted-foreground leading-relaxed">라켓과 스트링이 동일 수량으로 결제됩니다</p>
              <div className="flex items-center gap-1">
                <Button type="button" variant="outline" size="icon" className="h-8 w-8" onClick={() => setWorkCount((prev) => clampWorkCount(prev - 1))} disabled={workCount <= 1}>
                  <Minus className="h-3.5 w-3.5" />
                </Button>
                <Input type="number" inputMode="numeric" min={1} max={maxQty} value={workCount} onChange={(e) => setWorkCount(clampWorkCount(Number(e.target.value)))} className="h-8 w-14 text-center text-sm" />
                <Button type="button" variant="outline" size="icon" className="h-8 w-8" onClick={() => setWorkCount((prev) => clampWorkCount(prev + 1))} disabled={workCount >= maxQty}>
        ...
        {/* Skip Button */}
        {onSkipString && (
          <Button variant="outline" className="mt-4 h-10 w-full rounded-xl break-keep" onClick={onSkipString}>
        ...
        <div className="mt-4 rounded-xl border border-border bg-muted/20 p-3 text-xs leading-relaxed text-muted-foreground">
          ...
          <p className="mt-1 break-keep">{flowType === "rental" ? "대여 라켓과 선택한 스트링 옵션은 체크아웃에서 한 번 더 확인할 수 있습니다." : "선택한 라켓, 스트링 옵션, 수량은 체크아웃에서 한 번 더 확인할 수 있습니다. 장착 정보까지 입력하면 신청이 완료됩니다."}</p>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-background">
      <SiteContainer variant="wide" className={cn("py-5 bp-md:py-8", !isRacketPurchaseDesign && !isRentalDesign && "bp-md:py-10")}>
        {/* Header */}
        <div className={cn("rounded-2xl border border-border bg-card shadow-sm", isRacketPurchaseDesign || isRentalDesign ? "mb-5 p-4 bp-md:mb-6 bp-md:p-5" : "mb-8 p-5 bp-md:p-6")}>
          {backLink && (
          ...
          <div className={cn(isRacketPurchaseDesign || isRentalDesign ? "mt-3 space-y-3" : "space-y-4")}>
            ...
            <div>
              <h1 className={cn("break-keep font-bold tracking-tight text-foreground", isRacketPurchaseDesign || isRentalDesign ? "text-2xl bp-md:text-3xl" : "text-2xl bp-md:text-3xl bp-lg:text-4xl")}>{headerTitle}</h1>
        ...
        <div className={cn("grid gap-5 bp-lg:gap-6", isRacketPurchaseDesign || isRentalDesign ? "bp-lg:grid-cols-[minmax(0,1fr)_340px]" : "bp-lg:grid-cols-[minmax(0,1fr)_360px]")}>
          {/* Main Content */}
          <div className="space-y-6">
              ...
              <div className="flex w-full items-center gap-2 bp-sm:w-auto">
                ...
                <div className="hidden items-center gap-1 rounded-lg border border-border p-1 bp-md:flex">
                  <button
                    ...
                    className={cn("rounded-md p-1.5 transition-colors", viewMode === "grid" ? "bg-secondary text-foreground" : "text-muted-foreground hover:text-foreground")}
                    aria-label="그리드 뷰"
                  >
                  ...
                  <button
                    ...
                    className={cn("rounded-md p-1.5 transition-colors", viewMode === "list" ? "bg-secondary text-foreground" : "text-muted-foreground hover:text-foreground")}
                    aria-label="리스트 뷰"
                  >
            ...
            <div className="flex flex-col gap-1 rounded-2xl border border-border bg-muted/30 px-4 py-3 text-sm text-muted-foreground bp-sm:flex-row bp-sm:items-center bp-sm:justify-between">
              {isLoadingInitial ? (
                ...
                <span>
                  총 <span className="font-semibold text-foreground">{filteredProducts.length}</span>
                  개의 스트링
                </span>
              )}
              <span className="break-keep text-xs">{flowType === "rental" ? "대여 라켓에 장착할 스트링을 선택하세요" : "브랜드·가격·옵션을 확인한 뒤 원하는 스트링을 선택하세요"}</span>
            </div>

            {/* Product Grid/List */}
            {isLoadingInitial ? (
              <div className={cn("grid gap-4", viewMode === "grid" ? "grid-cols-1 bp-sm:grid-cols-2 bp-lg:grid-cols-3 bp-xl:grid-cols-4" : "grid-cols-1")}>
              ...
              <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-border bg-card py-16 shadow-sm text-center">
                ...
                <p className="text-lg font-semibold text-foreground">{products?.length === 0 ? "사용 가능한 스트링이 없습니다" : "조건에 맞는 스트링이 없습니다"}</p>
                <p className="mt-2 text-sm text-muted-foreground">{products?.length === 0 ? "스트링 상품의 장착 서비스 설정을 확인해주세요" : "검색어 또는 필터를 변경해보세요"}</p>
              ...
              <>
                <div className={cn("grid gap-4", viewMode === "grid" ? "grid-cols-1 bp-sm:grid-cols-2 bp-lg:grid-cols-3 bp-xl:grid-cols-4" : "grid-cols-1 bp-md:grid-cols-2")}>
                ...
                {hasMore && (
                  <div className="flex justify-center pt-4">
                    <Button variant="outline" onClick={loadMore} disabled={isFetchingMore} className="min-w-[200px]">
                      {isFetchingMore ? (
          ...
          <div className={cn("hidden bp-lg:block bp-lg:h-fit", isRacketPurchaseDesign || isRentalDesign ? "bp-lg:sticky bp-lg:top-20" : "bp-lg:sticky bp-lg:top-24")}>{renderSelectedRacketSummary()}</div>
        </div>
      </SiteContainer>
    </div>
  );
