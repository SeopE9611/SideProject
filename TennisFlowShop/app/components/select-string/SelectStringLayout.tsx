...
export default function SelectStringLayout({
  ...
  const headerDescription =
    flowType === "rental"
      ? "스트링을 선택하거나 스트링 없이 대여 신청 단계로 이동할 수 있습니다."
      : isCartEditMode
        ? "새 스트링과 옵션을 선택하면 장바구니 구성에 반영됩니다."
        : "색상과 게이지를 확인한 뒤 선택하면 결제·장착 정보 단계로 이동합니다.";
  const resultHelper =
    flowType === "rental"
      ? `${finalCtaSubLabel}.`
      : isCartEditMode
        ? "선택한 옵션을 반영한 뒤 장바구니로 돌아갑니다."
        : finalCtaSubLabel;
    ...
    <SelectedRacketSummary
      ...
      price={
        flowType === "purchase" && racket.price != null ? (
          <CatalogPrice
            regularPrice={racket.regularPrice ?? racket.price ?? 0}
            salePrice={
              racket.price != null &&
              racket.regularPrice != null &&
              racket.price < racket.regularPrice
                ? racket.price
                : null
            }
            size="list"
            ...
            {racket.condition ? (
              <p>
                ...
                <span className="font-medium text-foreground">
                  {racket.conditionLabel ?? racket.condition}
                </span>
              </p>
            ) : null}
          </div>
        ...
        <>
          ...
          <p className="mt-1 break-keep">
            {flowType === "rental"
              ...
              : "선택한 라켓, 스트링 옵션, 수량은 체크아웃에서 한 번 더 확인할 수 있습니다."}
          </p>
        </>
      }
    />
