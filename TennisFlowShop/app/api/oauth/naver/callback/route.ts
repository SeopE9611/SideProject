...
export async function GET(req: NextRequest) {
  ...
  if (!clientId || !clientSecret) {
    return NextResponse.json(
      { message: "네이버 env(NAVER_CLIENT_ID/SECRET)가 누락되었습니다." },
      { status: 500 },
    );
  }
  ...
  const name =
    (meJson?.response?.name ? String(meJson.response.name) : "") ||
    (meJson?.response?.nickname ? String(meJson.response.nickname) : "") ||
    (email ? email.split("@")[0] : "");
