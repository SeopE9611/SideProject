...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (hasTrackingNumber) {
      return jsonError(400, '이미 배송이 진행 중이어서 취소 요청을 철회할 수 없습니다.', 'SHIPPING_IN_PROGRESS');
    }

    // 2-3) 현재 cancelRequest 상태가 'requested' 인지 확인
    const existingReq: any = existing.cancelRequest || {};
    if (existingReq.status !== 'requested') {
      return jsonError(409, '현재 취소 요청 상태가 아니어서 철회할 수 없습니다.', 'CANCEL_REQUEST_NOT_REQUESTED');
    }
    ...
    if (result.matchedCount === 0) {
      return jsonError(409, '이미 상태가 변경되어 취소 요청을 철회할 수 없습니다.', 'CANCEL_REQUEST_RACE_CONDITION');
    }
