...
export async function GET(req: Request) {
  ...
  const textRaw = url.searchParams.get("text") || "[도깨비테니스] SMS 테스트";
  ...
  if (!apiKey || !apiSecret || !fromEnv) {
    return NextResponse.json(
      { ok: false, error: "SOLAPI env is not set" },
      { status: 500 },
    );
  }
  ...
  if (allowlist.length === 0) {
    return NextResponse.json(
      { ok: false, error: "SMS allowlist is empty (set SAFE_SMS_ALLOWLIST)" },
      { status: 400 },
    );
  }
