...
import { finalizeAppsInTossPayment, AppsPaymentFinalizationError } from "@/lib/apps-in-toss/server/payment-finalization";
...
export async function POST(request: Request, context: { params: Promise<{ attemptId: string }> }) {
  ...
  try {
    ...
    return response(origin, await finalizeAppsInTossPayment({ db, attemptId, userId: authenticated.user._id, identityId: authenticated.session.identityId }), 200);
  } catch (error) {
    if (error instanceof AppsInTossSessionError) return response(origin, { success: false, code: "AUTH_REQUIRED", message: "인증이 필요합니다." }, 401);
    if (error instanceof AppsPaymentFinalizationError) return response(origin, { success: false, code: error.code, message: error.message }, error.status);
    console.error("[Apps in Toss payment finalize 실패]", error instanceof Error ? error.name : "UnknownError");
    return response(origin, { success: false, code: "PAYMENT_FINALIZATION_FAILED", message: "주문 확정 상태를 처리하지 못했습니다." }, 500);
  }
}
