...
import { AppsPaymentCompletionError, completeAppsInTossPayment } from "@/lib/apps-in-toss/server/payment-completion";
import { AppsPaymentExecutionError } from "@/lib/apps-in-toss/server/payment-execution";
import { AppsPaymentFinalizationError } from "@/lib/apps-in-toss/server/payment-finalization";
import { AppsPaymentRefundError } from "@/lib/apps-in-toss/server/payment-refund";
...
export async function POST(request: Request, context: { params: Promise<{ attemptId: string }> }) {
  ...
  try {
    ...
    return response(origin, await completeAppsInTossPayment({ db, attemptId, userId: authenticated.user._id, identityId: authenticated.session.identityId }), 200);
  } catch (error) {
    ...
    if (error instanceof AppsPaymentCompletionError || error instanceof AppsPaymentExecutionError || error instanceof AppsPaymentFinalizationError || error instanceof AppsPaymentRefundError) return response(origin, { success: false, code: error.code, message: error.message }, error.status);
    console.error("[Apps in Toss payment completion 실패]", error instanceof Error ? error.name : "UnknownError");
    return response(origin, { success: false, code: "PAYMENT_COMPLETION_FAILED", message: "결제 완료 상태를 처리하지 못했습니다." }, 500);
  }
}
