...
import { executeAppsInTossPayment, AppsPaymentExecutionError } from "@/lib/apps-in-toss/server/payment-execution";
...
function response(origin: string | null, body: unknown, status: number) {
  const result = NextResponse.json(body, { status }); result.headers.set("Cache-Control", "no-store");
...
export async function POST(request: Request, context: { params: Promise<{ attemptId: string }> }) {
  ...
  if (!isAppsInTossAllowedOrigin(origin)) return response(origin, { success: false, code: "ORIGIN_NOT_ALLOWED", message: "허용되지 않은 Origin입니다." }, 403);
  try {
    const db = await getDb(); const authenticated = await authenticateAppsSession(db, request.headers.get("authorization"));
    ...
    return response(origin, await executeAppsInTossPayment({ db, attemptId, userId: authenticated.user._id, identityId: authenticated.session.identityId }), 200);
  } catch (error) {
    ...
    if (error instanceof AppsPaymentExecutionError) return response(origin, { success: false, code: error.code, message: error.message }, error.status);
    console.error("[Apps in Toss payment execute 실패]", error instanceof Error ? error.name : "UnknownError");
    return response(origin, { success: false, code: "PAYMENT_EXECUTION_FAILED", message: "결제 승인 상태를 처리하지 못했습니다." }, 500);
  }
}
