...
export async function POST(
  ...
  if (post.isSecret) {
    ...
    if (!isAdmin && !isOwner) {
      ...
      return NextResponse.json(
        { ok: false, version: API_VERSION, error: "forbidden" },
  ...
  if (post.status !== "published") {
    return NextResponse.json(
      { ok: true, version: API_VERSION, firstView: false, viewCount: current },
  ...
  return NextResponse.json(
    { ok: true, version: API_VERSION, firstView: acquired, viewCount },
    { headers: { "Cache-Control": "no-store" } },
  );
}
