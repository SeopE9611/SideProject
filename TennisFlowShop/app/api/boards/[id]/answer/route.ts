...
export async function POST(
  ...
  if (!parsed.success)
    return NextResponse.json(
      { ok: false, version: API_VERSION, error: "invalid_body" },
      { status: 400 },
    );
  ...
  await db.collection("board_posts").updateOne(
    { _id: postId },
    {
      $set: {
        answer: {
          content: clean,
          authorId: String(guard.admin._id),
          authorName: guard.admin.name,
          createdAt: now,
        },
        updatedAt: now,
      },
...
export async function PATCH(
  ...
  await db
    ...
    .updateOne(
      { _id: postId },
      { $set: { "answer.content": clean, "answer.updatedAt": new Date() } },
    );
...
export async function DELETE(
  ...
  await db
    ...
    .updateOne(
      { _id: new ObjectId(id) },
      { $unset: { answer: "" }, $set: { updatedAt: new Date() } },
    );
  return NextResponse.json({ ok: true, version: API_VERSION });
}
