...
const TARGET_COLLECTION_MAP: Record<AllowedTargetType, string> = {
  order: "orders",
  rental: "rental_orders",
  stringingApplication: "stringing_applications",
  user: "users",
};
...
const mapNoteItem = (doc: any) => ({
  ...
  editedAt: doc.editedAt instanceof Date ? doc.editedAt.toISOString() : null,
});
...
export async function GET(req: Request) {
  ...
  const andConditions: Record<string, unknown>[] = [
    { targetType, targetId, isDeleted: { $ne: true } },
  ];
...
export async function POST(req: Request) {
  ...
  const doc = {
    ...
    createdByEmail: guard.admin.email ?? null,
    createdByName: guard.admin.name ?? null,
    createdByRole: guard.admin.role ?? "admin",
    createdAt: now,
    updatedAt: now,
    isDeleted: false,
  ...
  await appendAdminAudit(
    guard.db,
    {
      ...
      diff: {
        ...
        noteId: String(result.insertedId),
        targetType,
        targetId: String(targetId),
        bodyLength: bodyParsed.body.length,
      },
    },
    req,
  );
  return NextResponse.json({
    success: true,
    item: mapNoteItem({ ...doc, _id: result.insertedId }),
  });
}
