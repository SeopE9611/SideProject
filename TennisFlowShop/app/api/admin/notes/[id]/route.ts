...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  await coll.updateOne(
    { _id: noteId },
    { $set: { body: parsed.body, updatedAt: now, editedAt: now } },
  );
      ...
      diff: {
        metadata: {
          ...
          target: {
            ...
            id: String(note.targetId),
            scope: "admin_note",
          },
        },
        change: {
          before: { bodyLength: String(note.body ?? "").length },
          after: { bodyLength: parsed.body.length },
        },
        noteId: String(noteId),
        targetType: note.targetType,
        targetId: String(note.targetId),
        beforeBodyLength: String(note.body ?? "").length,
        afterBodyLength: parsed.body.length,
      },
    },
    req,
  );
  ...
  await coll.updateOne(
    { _id: noteId },
    {
      $set: {
        isDeleted: true,
        deletedAt: now,
        deletedBy: guard.admin._id,
        deletedByEmail: guard.admin.email ?? null,
        deletedByName: guard.admin.name ?? null,
        deletedByRole: guard.admin.role ?? "admin",
        updatedAt: now,
      },
    },
  );
