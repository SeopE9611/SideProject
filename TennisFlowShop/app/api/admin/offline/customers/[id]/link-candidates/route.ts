...
export async function GET(
  ...
  if (phone) {
    const phoneRegex = phoneNormalized
      ? buildLoosePhoneRegex(phoneNormalized)
      : new RegExp(escapeRegex(phone), "i");
    or.push({ phone: phoneRegex });
  }

  const users = await guard.db
    .collection("users")
  ...
  for (const linked of linkedCustomers) {
    if (linked.linkedUserId instanceof ObjectId)
      linkedByUserId.set(String(linked.linkedUserId), String(linked._id));
  }
