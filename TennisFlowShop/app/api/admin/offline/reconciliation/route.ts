...
function buildStatusFilter(path: string, status: string): Filter<Document> {
  ...
  if (status === "open") {
    return {
      $or: [
        { [`${path}.reconcileStatus`]: { $exists: false } },
        { [`${path}.reconcileStatus`]: null },
        { [`${path}.reconcileStatus`]: "open" },
      ],
    };
  }
...
function serializePackageIssue(doc: Record<string, any>): ReconciliationItem {
  ...
  const customerName =
    doc.offlineCustomer?.name ??
    doc.serviceInfo?.name ??
    doc.userSnapshot?.name ??
    "고객 정보 없음";
  const failedAt =
    serializeDate(meta.offlineIssueFailedAt) ??
    serializeDate(doc.updatedAt) ??
    serializeDate(doc.createdAt);
  return {
    id: String(doc._id),
    type: "package_issue",
...
function serializePackageUsage(doc: Record<string, any>): ReconciliationItem {
  ...
  return {
    ...
    updatedAt: serializeDate(doc.updatedAt),
    source: {
      offlineRecordId: String(doc._id),
      offlineCustomerId: customerId,
      passId: objectIdString(usage.passId),
    },
    customer: {
      id: customerId,
      name: String(customerName),
      phoneMasked: customerPhone ? maskPhone(String(customerPhone)) : null,
    },
    metadata: {
      passId: objectIdString(usage.passId),
      usedCount: usage.usedCount ?? 1,
      occurredAt: serializeDate(doc.occurredAt),
      lineSummary: lineSummary(doc.lines),
      memo: doc.memo ?? null,
    },
    links: {
      customerDetailUrl: customerId ? `/admin/offline/customers/${customerId}` : null,
...
export async function GET(req: Request) {
  ...
  const type: ReconcileType = TYPES.includes(typeParam as ReconcileType)
    ? (typeParam as ReconcileType)
    : "all";
      ...
      .aggregate([
        { $match: packageIssueMatch(status, from, to) },
        {
          $lookup: {
            ...
            localField: "meta.offlineCustomerId",
            foreignField: "_id",
            as: "offlineCustomerDocs",
          },
        },
