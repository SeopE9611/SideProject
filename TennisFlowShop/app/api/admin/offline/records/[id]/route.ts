...
export async function PATCH(
  ...
  const existingRecord = await guard.db
    .collection("offline_service_records")
    .findOne({ _id });
  if (!existingRecord)
  ...
  if (parsed.data.occurredAt)
    $set.occurredAt = new Date(parsed.data.occurredAt);
  if ("lines" in parsed.data) $set.lines = parsed.data.lines ?? [];
  if ("memo" in parsed.data) $set.memo = parsed.data.memo ?? "";
  if (parsed.data.payment?.status)
    $set["payment.status"] = parsed.data.payment.status;
  if (parsed.data.payment?.method)
    $set["payment.method"] = parsed.data.payment.method;
  if (typeof parsed.data.payment?.amount === "number")
    $set["payment.amount"] = parsed.data.payment.amount;
  if (parsed.data.payment?.paidAt)
    $set["payment.paidAt"] = new Date(parsed.data.payment.paidAt);
  const oldPayment = existingRecord.payment ?? {};
  const nextPayment = { ...oldPayment, ...(parsed.data.payment ?? {}) };
  const oldPaidAmount =
    oldPayment.status === "paid" ? Number(oldPayment.amount || 0) : 0;
  const newPaidAmount =
    nextPayment.status === "paid" ? Number(nextPayment.amount || 0) : 0;
