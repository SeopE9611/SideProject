...
const emptyPackageUsageFilter = {
  $and: [
    {
      $or: [
        { "packageUsage.passId": { $exists: false } },
        { "packageUsage.passId": null },
        { "packageUsage.passId": "" },
      ],
    },
    {
...
export async function POST(
  ...
  try {
    await session.withTransaction(async () => {
      ...
      const user = await guard.db
        .collection("users")
        .findOne({ _id: linkedUserId }, { projection: { _id: 1 }, session });
      if (!user) throw new PackageUseError(404, "user not found");

      const pass = await guard.db
        .collection<ServicePass>("service_passes")
        .findOne({ _id: passId }, { session });
      ...
      const updateResult = await records.updateOne(
        ...
        {
          $set: {
            "packageUsage.passId": passId,
            "packageUsage.usedCount": usedCount,
            "packageUsage.consumptionId": consumption._id,
            updatedAt: now,
  ...
  const nextPackageUsage = {
    passId,
    usedCount,
    consumptionId: finalConsumption._id,
  };
  await appendAudit(
    guard.db,
    {
  ...
  return NextResponse.json({
    ...
    consumption: {
      id: String(finalConsumption._id),
      usedCount: Number(finalConsumption.count ?? usedCount),
    },
  });
}
