...
export async function POST(
  ...
  try {
    await session.withTransaction(async () => {
      ...
      if (!record.packageUsage)
        throw new PackageRevertError(404, "package usage not found");
      if (
        record.packageUsage.revertedAt ||
        record.packageUsage.reverted === true
      ) {
        throw new PackageRevertError(409, "package usage already reverted");
      }

      passId =
        record.packageUsage.passId instanceof ObjectId
          ? record.packageUsage.passId
          : toObjectId(String(record.packageUsage.passId ?? ""));
      consumptionId =
        record.packageUsage.consumptionId instanceof ObjectId
          ? record.packageUsage.consumptionId
          : toObjectId(String(record.packageUsage.consumptionId ?? ""));
      if (!passId || !consumptionId)
        throw new PackageRevertError(404, "package usage not found");
      ...
      if (!offlineCustomerId)
        throw new PackageRevertError(404, "offline customer not found");
      ...
      linkedUserId =
        customer.linkedUserId instanceof ObjectId
          ...
          : null;
      if (!linkedUserId)
        throw new PackageRevertError(400, "linked user required");

      const consumption = await consumptions.findOne(
        { _id: consumptionId },
        { session },
      );
      if (!consumption)
        throw new PackageRevertError(404, "consumption not found");
      if (consumption.reverted === true)
        throw new PackageRevertError(409, "consumption already reverted");
      if (
        !consumption.passId ||
        String(consumption.passId) !== String(passId)
      ) {
        throw new PackageRevertError(409, "consumption pass mismatch");
      }
      if (
        !consumption.applicationId ||
        String(consumption.applicationId) !== String(recordId)
      ) {
      ...
      updatedConsumption = await consumptions.findOne(
        { _id: consumptionId },
        { session },
      );
    ...
    return NextResponse.json(
      { message: "package usage revert failed" },
      { status: 500 },
    );
  } finally {
  ...
  if (
    ...
    console.error(
      "[offline package revert] transaction completed without response payload",
