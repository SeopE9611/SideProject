...
function buildCommonRows(report: RevenueReportResponse): unknown[][] {
  return [
    ...
    [
      "패키지 발급 보정 필요 건수",
      "",
      report.offline.issueFailedCount ?? 0,
      "오프라인 패키지 발급 확인",
    ],
    [
      "패키지 발급 보정 필요 금액",
      report.offline.issueFailedAmount ?? 0,
      "",
      "오프라인 패키지 발급 확인",
    ],
    [],
    ...
    ...Object.entries(METHOD_LABELS).map(([key, label]) => [
      label,
      report.offline.byMethod[key as keyof typeof METHOD_LABELS],
      "오프라인 결제완료 매출",
    ]),
    [],
    ["추이"],
    ...
    ...report.series.map((point) => [
      point.date,
      point.onlinePaidAmount,
      point.onlineRefundAmount ?? 0,
      point.onlineNetAmount ?? point.onlinePaidAmount,
      point.offlinePaidAmount,
      point.combinedPaidAmount,
    ]),
  ];
}
...
export function buildRevenueReportSnapshotCsv(
  report: RevenueReportResponse,
  meta: RevenueReportSnapshotCsvMeta,
) {
  const rows: unknown[][] = [
    ["온라인/오프라인 매출 리포트 스냅샷"],
    ["스냅샷 월", meta.yyyymm],
