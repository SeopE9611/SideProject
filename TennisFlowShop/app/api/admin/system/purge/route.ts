...
import {
  buildDangerousActionToken,
  createDangerousActionHash,
  getDangerousActionReconfirmText,
  isDangerousActionConfirmationValid,
} from "@/lib/admin/adminDangerousAction";
...
async function listPurgeCandidates(db: any) {
  ...
  return {
    ...
    candidateIds: candidates
      .map((item: { _id?: unknown }) => String(item?._id ?? ""))
      .filter(Boolean),
  };
}
...
export async function DELETE(req: Request) {
  ...
  const limited = await enforceAdminRateLimit(
    ...
    ADMIN_EXPENSIVE_ENDPOINT_POLICIES.adminSystemMutation,
  );
  ...
  if (mode !== "execute") {
    return NextResponse.json({
      ...
      confirmationToken: buildDangerousActionToken(
        "admin.system.purge",
        String(guard.admin._id),
        previewHash,
      ),
    ...
    return NextResponse.json(
      {
        ...
        error: {
          ...
          message: "동일 영구 정리 작업이 이미 실행 중입니다.",
        },
      },
      { status: 409 },
    );
