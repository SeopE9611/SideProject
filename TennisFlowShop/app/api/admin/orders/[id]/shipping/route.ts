  ...
  try {
      ...
      return NextResponse.json(
        {
          ok: false,
          message:
            "방문 수령 주문은 배송 방식 변경이 불가합니다. 예외 전환은 별도 관리자 절차를 통해 진행해주세요.",
        },
        { status: 400 },
      );
    }
    if (!isOriginalVisitPickup && normalizedMethod === "visit") {
      return NextResponse.json(
        {
          ok: false,
          message: "일반 배송 주문은 방문 수령으로 변경할 수 없습니다.",
        },
        { status: 400 },
      );
    }
    ...
    if (!Number.isFinite(est.getTime()))
      return NextResponse.json(
        { success: false, message: "예상 수령일 값이 올바르지 않습니다." },
        { status: 400 },
      );
    ...
    const historyStatus = isVisitPickup
      ? isRegistered
        ? "방문수령정보수정"
        : "방문수령정보등록"
      : isRegistered
    ...
    if (shouldAutoTransitToShipping) {
      historyEntries.push({
        ...
        date: new Date().toISOString(),
        description:
          "최초 배송정보 등록이 확인되어 주문 상태가 자동으로 배송중으로 전환되었습니다.",
      });
      setOps.status = "배송중";
    }
    ...
    await appendAdminAudit(
      guard.db,
      {
        ...
        diff: {
          ...
          estimatedDate,
          ...(shouldAutoTransitToShipping
            ? {
                status: {
                  from: currentStatusRaw,
                  to: "배송중",
                  reason: "first-shipping-registration",
                },
              }
            : {}),
        },
