...
export async function GET(req: Request) {
  ...
  const pipeline: Record<string, unknown>[] = [
    ...
    },
    {
      $addFields: {
        ...
        resolvedUserEmail: {
          $ifNull: ["$userEmail", { $arrayElemAt: ["$_user.email", 0] }],
        },
        resolvedUserName: {
          $ifNull: [{ $arrayElemAt: ["$_user.name", 0] }, ""],
        },
        contentStr: {
          $cond: [{ $eq: [{ $type: "$content" }, "string"] }, "$content", ""],
        },
      ...
      $addFields: {
        resolvedType: {
          $cond: [
            ...
            "$type",
            { $cond: ["$hasProductId", "product", "service"] },
          ],
        },
      },
    },
  ...
  const shaped: AdminReviewListItemDto[] = items.map((d) => ({
    ...
    photos: Array.isArray(d.photosPreview)
      ? d.photosPreview.filter((p): p is string => typeof p === "string")
      : [],
