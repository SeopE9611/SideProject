...
function buildFilter(dto: AdminProductsListRequestDto): Filter<Document> {
  ...
  if (dto.status === "active") {
    ...
    filter.$and = [
      ...
      {
        $or: [
          { "inventory.lowStock": { $exists: false } },
          { "inventory.lowStock": null },
          { $expr: { $gt: ["$inventory.stock", "$inventory.lowStock"] } },
        ],
      },
    ];
  }
...
export async function GET(req: Request) {
  try {
    ...
    const items: AdminProductListItemDto[] = rawItems.map((doc) =>
      toProductListItem(doc as ProductDoc),
    );
                  ...
                  },
                  {
                    case: {
                      $and: [
                        ...
                        { $ne: ["$inventory.lowStock", null] },
                        { $lte: ["$inventory.stock", "$inventory.lowStock"] },
                      ],
                    },
                    then: "low_stock",
                  },
