...
export async function GET(req: Request) {
    ...
    q.$and = (q.$and ?? []).concat([
      {
        $or: [
          {
            "shipping.outbound.trackingNumber": { $in: [null, "", undefined] },
          },
      ...
      {
        $or: [
          { "shipping.return.trackingNumber": { $in: [null, "", undefined] } },
  ...
  const items: AdminRentalListItemDto[] = docs.map((rentalDoc) => {
    ...
    const cust =
      (rentalDoc.userId && userMap.get(String(rentalDoc.userId))) ||
      (rentalDoc.guest
    ...
    const withStringService =
      Boolean(rentalDoc?.stringing?.requested) ||
      Boolean(record.isStringServiceApplied) ||
      Boolean(rawAppId);
    const stringingApplicationId = rawAppId
    ...
    return {
      ...
      cancelRequest: rentalDoc.cancelRequest
        ? {
            status:
              rentalDoc.cancelRequest.status === "요청" ||
              rentalDoc.cancelRequest.status === "requested"
                ? "requested"
                : rentalDoc.cancelRequest.status === "승인" ||
                    rentalDoc.cancelRequest.status === "approved"
                  ? "approved"
                  : "rejected",
            refundAccountReady: hasRefundAccount(
              (rentalDoc as any)?.cancelRequest?.refundAccount ?? null,
            ),
            refundBankLabel: resolveRefundBankLabel(
              (rentalDoc as any)?.cancelRequest?.refundAccount ?? null,
            ),
          }
        : null,
  ...
  const response: AdminRentalsListResponseDto = {
    page: parsed.page,
