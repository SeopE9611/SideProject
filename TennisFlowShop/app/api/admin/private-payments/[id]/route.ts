...
export async function PATCH(req: Request, ctx: Ctx) {
  ...
  if (action === "archive" || action === "unarchive") {
    if (current.paymentStatus === "결제대기") {
      return NextResponse.json({ ok: false, message: "결제대기 건은 보관할 수 없습니다. 필요 시 삭제해 주세요." }, { status: 400 });
    }
    const now = new Date();
    const set = action === "archive"
      ? { archivedAt: now, archivedBy: guard.admin._id, updatedAt: now }
      : { archivedAt: null, archivedBy: null, updatedAt: now };
    await col.updateOne({ _id: current._id }, { $set: set, $push: { history: { status: action, date: now, description: action === "archive" ? "관리자 보관" : "관리자 보관 해제" } } });
    await appendAdminAudit(guard.db, { type: action === "archive" ? "private_payment.archive" : "private_payment.unarchive", actorId: guard.admin._id, targetId: current._id, message: action === "archive" ? "개인결제 보관" : "개인결제 보관 해제", diff: set }, req);
  ...
  const set = { ...editableInput, updatedAt: new Date() } as Partial<PrivatePayment> & { updatedAt: Date };
...
export async function DELETE(req: Request, ctx: Ctx) {
  ...
  if (!current) return NextResponse.json({ ok: false, message: "개인결제를 찾을 수 없습니다." }, { status: 404 });
  if (current.paymentStatus !== "결제대기") {
    return NextResponse.json({ ok: false, message: "결제대기 건만 삭제할 수 있습니다." }, { status: 400 });
  }
  await col.deleteOne({ _id: current._id, paymentStatus: "결제대기" });
  await appendAdminAudit(guard.db, { type: "private_payment.delete_pending", actorId: guard.admin._id, targetId: current._id, message: "결제대기 개인결제 삭제", diff: { title: current.title, amount: current.amount } }, req);
