...
export async function POST(req: Request, ctx: Ctx) {
  ...
  if (payment.offlineLink?.status === "linked")
    return NextResponse.json(
      { ok: false, message: "이미 오프라인 기록과 연결된 개인결제입니다." },
      { status: 409 },
    );
  if (payment.paymentStatus !== "결제완료" || !payment.paidAt || Number(payment.amount || 0) <= 0) {
    return NextResponse.json(
      { ok: false, message: "결제완료된 개인결제만 오프라인 기록과 연결할 수 있습니다." },
      { status: 400 },
    );
  }
  ...
  if (customerId) {
    if (!ObjectId.isValid(customerId))
      return NextResponse.json(
        { ok: false, message: "오프라인 고객 ID가 올바르지 않습니다." },
        { status: 400 },
      );
    ...
    if (!customer)
      return NextResponse.json(
        { ok: false, message: "오프라인 고객을 찾을 수 없습니다." },
        { status: 404 },
      );
  } else {
    ...
    customer = or.length
      ? await guard.db.collection("offline_customers").findOne({ $or: or })
      : null;
    if (!customer) {
      if (!phoneNormalized)
        return NextResponse.json(
          { ok: false, message: "신규 오프라인 고객 생성에는 연락처가 필요합니다." },
          { status: 400 },
        );
      const now = new Date();
      const doc = {
  ...
  let updated = await col.findOneAndUpdate(
    ...
    {
      $set: { offlineLink, updatedAt: now },
      $push: {
        history: {
          status: "offline_linked",
          date: now,
          description: "오프라인 고객/작업 기록 연결",
        },
      },
    },
    ...
    try {
      ...
      if (offlineRecordId) {
        await guard.db
          ...
          .deleteOne({ _id: offlineRecordId })
          .catch((cleanupError) => {
            console.error("[private-payments/link-offline] offline record cleanup failed", {
              paymentId: paymentId.toString(),
              offlineRecordId: offlineRecordId?.toString(),
              cleanupError,
            });
          });
      }
      await col
        ...
        .catch((rollbackError) => {
          console.error("[private-payments/link-offline] offline link rollback failed", {
            paymentId: paymentId.toString(),
            rollbackError,
          });
        });
      return NextResponse.json(
        { ok: false, message: "오프라인 작업 기록 생성 중 오류가 발생했습니다." },
        { status: 500 },
      );
    }
  }
  ...
  await appendAdminAudit(
    guard.db,
    {
      type: "private_payment.link_offline",
      actorId: guard.admin._id,
      targetId: paymentId,
      message: "개인결제 오프라인 고객/작업 기록 연결",
      diff: {
        offlineCustomerId: String(customer._id),
        offlineRecordId: offlineRecordId ? String(offlineRecordId) : null,
      },
    },
    req,
  );
