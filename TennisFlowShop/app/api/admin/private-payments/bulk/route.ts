...
export async function POST(req: Request) {
  ...
  if (action === "archive") {
    const result = await col.updateMany(
      ...
      {
        $set: { archivedAt: now, archivedBy: guard.admin._id, updatedAt: now },
        $push: { history: { status: "archive", date: now, description: "관리자 선택 보관" } },
      },
    );
    await appendAdminAudit(
      guard.db,
      {
        type: "private_payment.bulk_archive",
        actorId: guard.admin._id,
        message: "개인결제 선택 보관",
        diff: { ids, modifiedCount: result.modifiedCount },
      },
      req,
    );
  ...
  if (action === "unarchive") {
    const result = await col.updateMany(
      { _id: { $in: objectIds }, paymentStatus: { $in: ["결제완료", "결제취소"] } },
      {
        ...
        $push: {
          history: { status: "unarchive", date: now, description: "관리자 선택 보관 해제" },
        },
      },
    );
    await appendAdminAudit(
      guard.db,
      {
        type: "private_payment.unarchive",
        actorId: guard.admin._id,
        message: "개인결제 선택 보관 해제",
        diff: { ids, modifiedCount: result.modifiedCount },
      },
      req,
    );
  ...
  if (action === "delete_pending") {
    ...
    await appendAdminAudit(
      guard.db,
      {
        type: "private_payment.bulk_delete_pending",
        actorId: guard.admin._id,
        message: "결제대기 개인결제 선택 삭제",
        diff: { ids, deletedCount: result.deletedCount },
      },
      req,
    );
