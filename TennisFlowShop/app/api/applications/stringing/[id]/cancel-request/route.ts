...
async function canAccessStringingCancelRequest(id: string) {
  ...
  if (!isAdmin && !isMemberOwner && !isGuestOrderOwner) {
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const auth = await canAccessStringingCancelRequest(resolvedParams.id);
