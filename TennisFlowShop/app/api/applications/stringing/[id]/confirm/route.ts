...
function resolveOrderRewardBaseAmount(order: any): number {
  ...
  return toSafeRewardBaseAmount(
    paymentInfo?.total ?? paymentInfo?.amount ?? (order as any)?.totalPrice,
  );
}
...
export async function POST(
  ...
  try {
    ...
    if (app.orderId) {
      ...
      if (!order) {
        return NextResponse.json(
          { ok: false, message: "연결된 주문을 찾을 수 없습니다." },
          { status: 404 },
        );
      }
      ...
      const linkedApps = await db
        .collection("stringing_applications")
        .find(
          { orderId: orderObjectId, status: { $nin: ["draft"] } },
          { projection: { _id: 1, status: 1, userConfirmedAt: 1 } as any },
        )
        .toArray();
      ...
      if (blocking.length) {
        return NextResponse.json(
          {
            ok: false,
            message: `아직 완료되지 않은 교체 서비스가 있습니다. (미완료 ${blocking.length}건)`,
          },
          { status: 400 },
        );
      }
    ...
    const rentalObjectId =
      app.rentalId && ObjectId.isValid(String(app.rentalId))
        ? app.rentalId instanceof ObjectId
          ? app.rentalId
          : new ObjectId(String(app.rentalId))
        : null;
    ...
    return NextResponse.json(
      { ok: false, message: "서비스 확정 처리 중 오류가 발생했습니다." },
      { status: 500 },
    );
  }
}
