...
export async function GET(
  ...
  const [postsCount, commentsCount] = await Promise.all([
    postsCol.countDocuments({ userId: authorObjectId, status: "public" }),
    commentsCol.countDocuments({ userId: authorObjectId, status: "public" }),
  ]);
  ...
  const firstDates = [
    firstPost?.createdAt instanceof Date ? firstPost.createdAt : null,
    firstComment?.createdAt instanceof Date ? firstComment.createdAt : null,
  ].filter(Boolean) as Date[];

  const firstActivityAt =
    firstDates.length > 0
      ? new Date(Math.min(...firstDates.map((d) => d.getTime()))).toISOString()
      : null;
