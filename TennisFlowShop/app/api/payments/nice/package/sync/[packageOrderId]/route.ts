...
export async function POST(
  _req: Request,
  { params }: { params: Promise<{ packageOrderId: string }> },
) {
  try {
    ...
    if (!ObjectId.isValid(packageOrderId)) {
      return NextResponse.json(
        {
          success: false,
          code: "INVALID_PACKAGE_ORDER_ID",
          error: "유효하지 않은 패키지 주문 ID입니다.",
        },
        { status: 400 },
      );
    }
    ...
    if (!packageOrder) {
      return NextResponse.json(
        {
          success: false,
          code: "PACKAGE_ORDER_NOT_FOUND",
          error: "패키지 주문을 찾을 수 없습니다.",
        },
        { status: 404 },
      );
    }

    const provider = String((packageOrder as any)?.paymentInfo?.provider ?? "")
      .trim()
      .toLowerCase();
    ...
    const currentCardDisplayName = String(
      (packageOrder as any)?.paymentInfo?.cardDisplayName ?? "",
    ).trim();
