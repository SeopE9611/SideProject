...
function failUrl(code: string, message: string) {
...
async function handleNiceStringingReturn(req: Request) {
  ...
  try {
    ...
    if (
      ...
      return NextResponse.redirect(
        new URL(failUrl("AUTH_FAILED", "결제 인증에 실패했습니다."), req.url),
      );
    }
    ...
    if (pick(approvedRaw, "resultCode", "ResultCode") !== "0000") {
      ...
      return NextResponse.redirect(
        new URL(
          failUrl("APPROVE_FAILED", "결제 승인에 실패했습니다."),
          req.url,
        ),
      );
    }
    ...
    if (!updateResult.modifiedCount) {
      ...
      return NextResponse.redirect(
        new URL(failUrl("UPDATE_FAILED", "결제 반영에 실패했습니다."), req.url),
      );
    }
