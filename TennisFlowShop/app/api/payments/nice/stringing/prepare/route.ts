  ...
  try {
    if (!isNicePaymentsEnabled()) {
      return NextResponse.json(
        { success: false, error: "현재 카드결제를 사용할 수 없습니다." },
        { status: 503 },
      );
    }
    ...
    if (!clientId) {
      return NextResponse.json(
        { success: false, error: "NicePay client key 설정이 누락되었습니다." },
        { status: 500 },
      );
    }
    ...
    if (
      ...
      return NextResponse.json(
        { success: false, error: "카드결제 가능한 신청 상태가 아닙니다." },
        { status: 409 },
      );
    }
    ...
    return NextResponse.json(
      {
        success: false,
        error: error?.message || "Nice 결제 준비에 실패했습니다.",
      },
      { status: 500 },
    );
  }
