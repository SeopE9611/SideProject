...
function clientId() { return String(process.env.NICEPAY_CLIENT_KEY ?? process.env.NICEPAY_CLIENT_ID ?? "").trim(); }
function appUrl() { return String(getBaseUrl()).trim().replace(/\/+$/, ""); }

export async function POST(req: Request) {
  try {
    if (!isNicePaymentsEnabled()) return NextResponse.json({ success: false, code: "PAYMENT_PROVIDER_DISABLED", error: "현재 해당 결제수단은 사용할 수 없습니다." }, { status: 503 });
    const cid = clientId();
    if (!cid) return NextResponse.json({ success: false, code: "NICE_CONFIG_MISSING", error: "카드/간편결제 설정이 누락되었습니다." }, { status: 500 });
    const body = await req.json().catch(() => ({}));
    const privatePaymentId = String(body?.privatePaymentId ?? "").trim();
    if (!ObjectId.isValid(privatePaymentId)) return NextResponse.json({ success: false, error: "잘못된 결제 링크입니다." }, { status: 400 });
    const buyer = body?.buyerInfo ?? {};
    const buyerInfo = { name: String(buyer?.name ?? "").trim(), phone: onlyDigits(buyer?.phone), email: String(buyer?.email ?? "").trim() };
    if (buyerInfo.name.length < 1 || buyerInfo.phone.length < 8 || !emailOk(buyerInfo.email)) return NextResponse.json({ success: false, error: "구매자 정보를 확인해 주세요." }, { status: 400 });
    ...
    await tossPaymentSessions(db).insertOne({
      provider: "nicepay", niceOrderId, amount: doc.amount, status: "ready", flowType: "private_payment", userId: null,
      privatePaymentId, privatePaymentPayload: { title: doc.title, amount: doc.amount, buyerInfo },
      nicePrepared: { clientId: cid, orderId: niceOrderId, returnUrl, goodsName: doc.title, buyerName: buyerInfo.name, buyerTel: buyerInfo.phone, buyerEmail: buyerInfo.email },
