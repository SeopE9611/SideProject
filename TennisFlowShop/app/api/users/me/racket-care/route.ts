...
export async function GET() {
  ...
  const profile = await db.collection("player_profiles").findOne({ userId }, { projection: { level: 1, mainRacket: 1, mainString: 1 } });
  const importCandidates: RacketCareImportCandidate[] = [];
  const profileRacket = profile?.mainRacket && typeof profile.mainRacket === "object" ? profile.mainRacket as Record<string, unknown> : null;
  const profileString = profile?.mainString && typeof profile.mainString === "object" ? profile.mainString as Record<string, unknown> : null;
  const brand = String(profileRacket?.brand ?? "").trim();
  const model = String(profileRacket?.model ?? "").trim();
  const profileStringName = [profileString?.brand, profileString?.model].map((value) => String(value ?? "").trim()).filter(Boolean).join(" ");
  ...
  return NextResponse.json({ items: serialized, importCandidates: cleanedImportCandidates, suggestedImport, maxItems: RACKET_CARE_MAX_ITEMS, remainingSlots, profileLevel });
}
