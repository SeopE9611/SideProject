import type { CollectionMethod, FlowLabel, ServiceFlowTrace } from '../support/serviceFlowAssertions';
...
function buildTrace(scenario: ServiceScenarioKey, method: CollectionMethod): ServiceFlowTrace {
  ...
  const scenarioTemplate: Record<ServiceScenarioKey, { flowLabel: FlowLabel; summaryLabel: string; myPageTab: '주문' | '신청' | '대여'; linkedTab?: '주문' | '신청' | '대여'; adminSection?: '주문' | '신청' | '대여'; status?: string; bundleQuantity?: number; linkTarget: string; paymentLabel: string }> = {
    'string-only': {
