...
async function check(path, okStatuses = [200]) {
  ...
  if (!okStatuses.includes(status)) {
    ...
    throw new Error(
      `[SMOKE FAIL] ${path} -> ${status} (expected ${okStatuses.join(", ")})\n` +
        text.slice(0, 200),
    );
  }
...
async function checkAuthGate(path) {
  ...
  console.log(
    `[SMOKE OK][AUTH_GATE] ${path} -> ${status}${location ? `, location=${location}` : ""}`,
  );
}
...
async function main() {
  ...
  const typeDebtReport = execSync("node scripts/report-admin-any-count.mjs", {
    encoding: "utf8",
  }).trim();
