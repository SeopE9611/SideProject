...
function findAdminDirectFetchViolations(filePath) {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    violations.push({
      ...
      text: line.trim(),
      reason:
        'app/admin 에서는 직접 fetch("/api/admin/**") 대신 adminFetcher/adminMutator를 사용해야 합니다.',
    });
  }
...
function findLegacyMutationViolations() {
  ...
  for (const relativePath of LEGACY_ADMIN_MUTATION_ROUTES) {
    ...
    violations.push({
      file: relativePath,
      reason:
        "관리자 변경성 엔드포인트가 비-admin 경로에 구현 상태로 남아 있습니다. 307/410 정책 래퍼만 허용됩니다.",
    });
  }
  ...
  for (const fullPath of files) {
    ...
    if (!allowedHandlers) {
      violations.push({
        file: relativePath,
        reason:
          "허용 목록에 없는 legacy 비-admin API 라우트입니다. 신규 구현 대신 307/410 이관 정책 문서화를 먼저 수행하세요.",
      });
      continue;
    }
...
try {
  ...
  if (
    ...
    adminDirectFetchViolations.length === 0 &&
    legacyMutationViolations.length === 0 &&
    legacyWrapperBoundaryViolations.length === 0 &&
    adminMutationProxyViolations.length === 0
  ) {
    console.log(
      "✅ app/admin 비-admin API 호출이 없고, 관리자 변경성 비-admin 경로 잔존도 없습니다.",
    );
    process.exit(0);
  }
