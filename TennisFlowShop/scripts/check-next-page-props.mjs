...
for (const file of pageFiles) {
  ...
  if (hasSearchParamsRecord) {
    violations.push({
      file,
      reason:
        "searchParams is typed as plain Record. In Next.js 15 page props, use Promise<...> and await it.",
    });
  }
  ...
  if (hasParamsObject && /export\s+default\s+async\s+function/m.test(source)) {
    violations.push({
      file,
      reason:
        "params appears to be typed as plain object in async page. In Next.js 15 page props, use Promise<...> and await it.",
    });
  }
}
...
console.log(
  `[check-next-page-props] OK - scanned ${pageFiles.length} page files, no Promise typing regressions found.`,
);
