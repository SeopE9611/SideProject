    ...
    <Card variant="feature" className={className}>
      <CardContent className="space-y-4 p-4 bp-sm:p-5">
        ...
        <ul className="space-y-2 text-ui-body-sm">
          {items.map((item) => <li key={item.label} className="flex gap-2">{item.ready ? <CheckCircle2 aria-hidden="true" className="mt-0.5 h-4 w-4 shrink-0 text-success" /> : <Circle aria-hidden="true" className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground" />}<span><span className="font-medium text-foreground">{item.label}</span><span className="block text-ui-label text-muted-foreground">{item.detail}</span></span></li>)}
          <li className="flex gap-2" aria-live="polite">{isUploading ? <Loader2 aria-hidden="true" className="mt-0.5 h-4 w-4 shrink-0 animate-spin text-warning" /> : photosCount > 0 ? <CheckCircle2 aria-hidden="true" className="mt-0.5 h-4 w-4 shrink-0 text-success" /> : <Circle aria-hidden="true" className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground" />}<span><span className="font-medium text-foreground">사진 첨부 — 선택 사항</span><span className="block text-ui-label text-muted-foreground">{isUploading ? "사진 업로드 처리 중" : photosCount > 0 ? `${photosCount}장 첨부 완료` : "사진 없이도 등록할 수 있어요"}</span></span></li>
        </ul>
      </CardContent>
    </Card>
  );
