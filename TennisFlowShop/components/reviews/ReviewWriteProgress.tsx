...
export default function ReviewWriteProgress({
  targetVerified,
  formStarted,
  isSubmitting,
  isLoading,
  ...
  return (
    <ol
      className="grid grid-cols-3 gap-2 rounded-panel border border-border bg-card p-3 shadow-soft"
      aria-label="후기 작성 진행 단계"
    >
      {steps.map((step) => {
        ...
        return (
          <li
            ...
            className={cn(
              "rounded-control border px-3 py-2 text-ui-label",
              step.active
                ? "border-foreground bg-background text-foreground"
                : "border-border bg-muted/30 text-muted-foreground",
              step.complete && "border-success/45 bg-success/10 text-success",
            )}
          >
