...
const TYPE_LABELS: Record<
  NonNullable<CanonicalReviewTarget["relatedItems"]>[number]["type"],
  string
> = {
  product: "상품",
  racket: "라켓",
...
export default function ReviewTargetSummary({ target }: { target: CanonicalReviewTarget }) {
  ...
  const fallbackItems = relatedItems.length
    ? relatedItems
    : ([
        target.primaryProductId
          ? { type: "product" as const, id: target.primaryProductId, name: "상품 후기 대상" }
          : null,
        target.primaryRacketId
          ? { type: "racket" as const, id: target.primaryRacketId, name: "라켓 후기 대상" }
          : null,
        target.primaryApplicationId
          ? { type: "service" as const, id: target.primaryApplicationId, name: "교체서비스" }
          : null,
        target.rentalId
          ? { type: "rental" as const, id: target.rentalId, name: "대여 후기 대상" }
          : null,
  ...
  return (
    <section aria-labelledby="review-target-summary-title" className="space-y-3">
      <div>
        <h3
          id="review-target-summary-title"
                ...
                <article className="flex min-w-0 gap-3 rounded-control border border-border bg-background p-3">
                  <div className="relative h-16 w-16 shrink-0 overflow-hidden rounded-xl border border-border bg-muted bp-lg:h-14 bp-lg:w-14">
                    {item.imageUrl ? (
                      ...
                      <div
                        aria-hidden="true"
                        className="grid h-full w-full place-items-center text-ui-label font-semibold text-muted-foreground"
                      >
                        {label.slice(0, 2)}
                      </div>
                    )}
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-0.5 line-clamp-2 break-words text-ui-body-sm font-semibold text-foreground">
                      {displayName}
                    </p>
                    {item.optionLabel && (
                      <p className="mt-1 line-clamp-2 break-words text-ui-label text-muted-foreground">
                        {item.optionLabel}
                      </p>
                    )}
                  </div>
          ...
          <details className="mt-3 rounded-control border border-border bg-muted/30 px-3 py-2 text-ui-label text-muted-foreground">
            ...
            <ul className="mt-2 space-y-1">
              {fallbackItems.map((item, index) => {
                ...
                return idSuffix ? (
                  <li key={`${item.type}-id-${item.id ?? index}`}>
                    {TYPE_LABELS[item.type]} 식별번호 끝 {idSuffix}
                  </li>
                ) : null;
              })}
            </ul>
