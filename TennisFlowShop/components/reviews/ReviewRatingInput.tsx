...
export default function ReviewRatingInput({
  ...
  return (
    <div className={cn("space-y-3", disabled && "opacity-60")}>
        ...
        {SCORES.map((score) => {
          ...
          return (
            <button
              key={score}
              ref={(element) => {
                buttonRefs.current[score - 1] = element;
              }}
              type="button"
              role="radio"
              ...
              aria-label={`${score}점`}
              disabled={disabled}
              tabIndex={disabled ? -1 : selected || (value === 0 && score === 1) ? 0 : -1}
              onMouseEnter={() => setPreview(score)}
              onMouseLeave={() => setPreview(null)}
              onFocus={() => setPreview(score)}
              onBlur={() => setPreview(null)}
              onClick={() => commitAndFocus(score)}
              onKeyDown={(event) => {
            ...
            >
              <Star
                ...
                className={cn(
                  ...
                  filled ? "fill-current text-warning" : "fill-transparent text-muted-foreground",
                )}
              />
            </button>
          );
      ...
      <p className="text-center text-ui-body-sm font-semibold text-foreground" aria-live="polite">
        {value > 0 ? `${value}점 선택됨` : "아직 별점을 선택하지 않았어요"}
      </p>
    </div>
  );
}
