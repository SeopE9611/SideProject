...
type CommerceMediaGalleryProps = {
  images: string[];
  alt: string;
...
export function CommerceMediaGallery({
  images,
  alt,
  overrideImage,
  badges,
  ...
  className,
  stickyClassName,
}: CommerceMediaGalleryProps) {
  const galleryImages = useMemo(
    () => Array.from(new Set([overrideImage, ...images].filter(Boolean) as string[])),
  ...
  return (
    <div
      className={cn(
        "self-start bp-md:sticky bp-md:top-[calc(var(--header-h,64px)+1rem)]",
        stickyClassName,
      )}
    >
      <div className={cn("space-y-3", className)}>
        <div className="relative aspect-square overflow-hidden rounded-panel border border-border bg-muted/20">
          {current ? (
            ...
            <div className="flex h-full flex-col items-center justify-center gap-3 text-muted-foreground">
              ...
              <span className="text-ui-body-sm">{emptyLabel}</span>
            </div>
          )}
          {badges ? (
        ...
        {galleryImages.length > 1 ? (
          <div className="flex gap-2 overflow-x-auto pb-1" aria-label="상품 이미지 썸네일">
              ...
              return (
                ...
                >
                  <Image
                    ...
                    fill
                    sizes="80px"
                    className={objectFit === "cover" ? "object-cover" : "object-contain p-2"}
                  />
                </button>
              );
            })}
