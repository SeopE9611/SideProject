  ...
  return (
    <div className="min-h-screen bg-background">
      <p className="sr-only" role="status" aria-live="polite">
        {isRental ? "대여 스트링 선택 화면을 불러오는 중입니다." : "구매 스트링 선택 화면을 불러오는 중입니다."}
      </p>
      <SiteContainer variant="wide" className="space-y-6 py-5 bp-md:py-8">
        <section className="rounded-panel border border-border bg-card p-4 shadow-sm bp-md:p-5" aria-hidden="true">
          ...
          <Skeleton className="mt-4 h-8 w-2/3" /><Skeleton className="mt-2 h-5 w-full max-w-2xl" />
        </section>
        <div className="grid gap-6 bp-lg:grid-cols-[minmax(0,1fr)_360px]">
          <main className="space-y-6">
            <div className="bp-lg:hidden"><SummarySkeleton showQuantityControls={showQuantityControls} showSecondaryAction={isRental} /></div>
            <Skeleton className="h-32 w-full rounded-2xl" />
            <StringSelectionCardSkeleton viewMode="grid" />
          </main>
          <div className="hidden bp-lg:block"><StickyAside><SummarySkeleton showQuantityControls={showQuantityControls} showSecondaryAction={isRental} /></StickyAside></div>
        </div>
      </SiteContainer>
    </div>
  );
