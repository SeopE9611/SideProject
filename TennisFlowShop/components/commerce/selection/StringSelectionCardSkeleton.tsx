  ...
  return (
    <div aria-hidden="true" className={cn("grid gap-4", viewMode === "grid" ? "grid-cols-1 bp-sm:grid-cols-2 bp-lg:grid-cols-3 bp-2xl:grid-cols-4" : "grid-cols-1")}>
      {Array.from({ length: count }).map((_, index) => (
        <div key={index} className={cn("overflow-hidden rounded-2xl border border-border bg-card shadow-sm", viewMode === "list" ? "flex flex-col bp-md:grid bp-md:grid-cols-[210px_minmax(0,1fr)_200px]" : "flex h-full flex-col")}>
          ...
          <div className={cn("flex flex-col justify-end gap-2 border-t border-border bg-muted/10 p-4", viewMode === "list" && "bp-md:border-l bp-md:border-t-0 bp-md:bg-muted/10")}>
