'use client';
import { AlertTriangle, ChevronRight, Tags } from 'lucide-react';
    ...
    <div className="flex h-full flex-col rounded-2xl border border-border bg-card shadow-sm overflow-hidden">
      ...
      <div className="flex items-center justify-between border-b border-border bg-muted/40 px-5 bp-sm:px-6 py-4 bp-sm:py-5">
        <div className="flex items-center gap-2.5">
          ...
          <h2 className="text-base bp-sm:text-lg font-bold text-foreground">중고 거래 최신글</h2>
        </div>
        <Link className="inline-flex items-center gap-1 rounded-lg px-3 py-1.5 text-xs bp-sm:text-sm font-medium text-muted-foreground transition-colors hover:bg-muted hover:text-foreground" href="/board/market">
          더보기
      ...
      <div className="flex-1 px-2 bp-sm:px-3 py-2 bp-sm:py-3">
        {isLoading ? (
          ...
          <div className="flex flex-col gap-0.5">
            {items.map((p, idx) => (
              <Link key={p.id ?? `${p.createdAt}-${idx}`} className="group flex items-center justify-between gap-3 rounded-xl px-3 bp-sm:px-4 py-3 bp-sm:py-3.5 transition-colors hover:bg-muted/60" href={`/board/market/${p.id}`}>
          ...
          <div className="flex flex-1 items-center justify-center py-12 bp-sm:py-16">
            <div className="text-center">
              ...
              <p className="mt-3 text-sm text-muted-foreground">등록된 게시글이 없습니다</p>
            </div>
          </div>
        )}
      </div>
