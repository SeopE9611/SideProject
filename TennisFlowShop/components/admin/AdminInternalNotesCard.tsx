    ...
    <Card className={cn("border-0 bg-muted/30 shadow-xl", className)}>
      ...
      <CardContent className="space-y-4">
        <div className="space-y-2">
          ...
          <div className="flex justify-end"><Button onClick={onCreate} disabled={!draft.trim()}>저장</Button></div>
        </div>
              ...
              {editingId === note.id ? (
                <div className="space-y-2">
                  ...
                  <div className="flex justify-end gap-2">
                    <Button variant="outline" size="sm" onClick={() => { setEditingId(null); setEditingBody(""); }}>취소</Button>
                    <Button size="sm" onClick={() => onUpdate(note.id)} disabled={!editingBody.trim()}>저장</Button>
                  </div>
                </div>
              ) : (
                <>
                  ...
                  <div className="mt-2 flex items-center justify-between text-xs text-muted-foreground">
                    ...
                    <div className="flex gap-2"><Button variant="ghost" size="sm" onClick={() => { setEditingId(note.id); setEditingBody(note.body); }}>수정</Button><Button variant="ghost" size="sm" onClick={() => setDeleteId(note.id)}>삭제</Button></div>
                  </div>
                </>
              )}
            </div>
      ...
      <AdminConfirmDialog open={Boolean(deleteId)} onOpenChange={(o) => !o && setDeleteId(null)} title="내부 메모 삭제" description="삭제한 메모는 목록에서 사라집니다. 계속할까요?" severity="danger" confirmText="삭제" onConfirm={onDelete} />
    </Card>
  );
}
