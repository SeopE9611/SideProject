...
export default function LinkedFlowStageCard({ orderId, orderStatus, applicationStatus, className, disabled = false, disabledReason = null, onSaved }: Props) {
  ...
  const handleSave = () => {
    ...
    startTransition(async () => {
      try {
        const result = await adminMutator<LinkedFlowStagePatchResponse>(`/api/admin/linked-flows/order-stringing/${orderId}/stage`, {
          method: 'PATCH',
  ...
  return (
    <Card className={className}>
      <CardHeader className="pb-3">
        ...
        <CardDescription>연결된 주문/신청서의 일반 진행 단계를 한 번에 변경합니다.</CardDescription>
      </CardHeader>
