...
export default function SideMenu() {
  ...
  const hybridFilter = isProductsCatalog
    ? NAV_LINKS.strings.brands.find((brand) => brand.href.includes("material=hybrid"))
    : null;
  const brandFilters = hybridFilter ? brands.filter((brand) => brand !== hybridFilter) : brands;
  const catalogTitle = isProductsCatalog ? "스트링 카탈로그" : "중고 라켓 카탈로그";
  const catalogRoot = isProductsCatalog ? NAV_LINKS.strings.root : NAV_LINKS.rackets.root;
      ...
      <div className="h-full space-y-3 overflow-y-auto px-5 py-4 scrollbar-hide">
        ...
        <Link href={catalogRoot} className={linkClass(catalogRoot)}>
          <span className="flex items-center justify-between">
            {isProductsCatalog ? "전체 스트링" : "전체 중고 라켓"}
        ...
        {NAV_FLAGS.SHOW_BRAND_MENU && brandFilters.length > 0 && (
              ...
              <AccordionContent value="brands" className="pb-0 pt-1">
                <div className="grid grid-cols-2 gap-1 px-1">
                  {brandFilters.map((brand) => (
                    <Link key={brand.href} href={brand.href} className={brandLinkClass(brand.href)}>
                      {brand.name}
                    </Link>
                  ))}
                </div>
        ...
        {hybridFilter && <Link href={hybridFilter.href} className={linkClass(hybridFilter.href)}>하이브리드 필터</Link>}
        ...
        <div className="space-y-0.5 border-t border-border/60 pt-3">
          {isProductsCatalog ? (
            <>
              ...
              <Link href="/services" className={linkClass("/services")}>교체서비스 홈</Link>
              ...
              <Link href="/services/packages" className={linkClass("/services/packages")}>교체 패키지</Link>
            </>
          ) : (
            <>
