...
export function UserNavMobile({ setOpen, unreadCount }: UserNavMobileProps) {
  ...
  if (loading) {
    return (
      <p
        className="text-ui-body-sm text-center text-muted-foreground py-2"
        aria-live="polite"
        aria-busy="true"
      >
        사용자 정보를 확인하고 있습니다.
    ...
    <>
      ...
      >
        ...
        {resolvedUnreadCount !== null && resolvedUnreadCount > 0 && (
          <span className="shrink-0 rounded-full bg-destructive text-destructive-foreground text-ui-micro leading-none px-1.5 py-[2px]">
