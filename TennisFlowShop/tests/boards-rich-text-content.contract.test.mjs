...
test("공지 수정 API는 저장된 post.type을 기준으로 검증하고 낙관적 락을 유지한다", () => {
  ...
  assert.match(
    patchSource,
    /import\s*\{[\s\S]*?prepareRichTextHtmlForSanitization[\s\S]*?richTextToValidationText[\s\S]*?\}\s*from\s*["']@\/components\/editor\/rich-text-utils["']/,
  );
...
test("공지 상세 GET은 레거시 일반 텍스트를 HTML로 정규화한 뒤 리치 텍스트 본문을 다시 정제한다", () => {
  ...
  assert.match(
    getSource,
    /await\s+sanitizeRichTextHtml\(\s*prepareRichTextHtmlForSanitization\(\s*String\(post\.content\s*\?\?\s*["']["']\)\s*\)\s*,?\s*\)/s,
  );
  assert.doesNotMatch(
    getSource,
    /await\s+sanitizeRichTextHtml\(\s*String\(post\.content\s*\?\?\s*["']["']\)\s*,?\s*\)/s,
  );
  ...
  assert.match(
    getSource,
    new RegExp(
      `\\.\\.\\.post[\\s\\S]*?content\\s*:\\s*await\\s+sanitizeRichTextHtml\\(\\s*prepareRichTextHtmlForSanitization`,
    ),
  );
