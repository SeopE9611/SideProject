...
test("checkout SDK와 pending marker 계약은 기존 형태를 유지한다", () => {
  ...
  assert.match(
    pending,
    /Object\.keys\(value\)\.sort\(\)\.join\(","\) !== "attemptId,authorizedAt"/,
  );
});
...
test("라켓 구매 draft의 개인정보는 URL이나 storage에 기록하지 않는다", () => {
  ...
  for (const source of [flow, app]) {
    assert.doesNotMatch(
      source,
      /searchParams\.set\("(?:applicant|name|email|phone|postalCode|address|note)"/,
    );
  }
...
test("라켓 구매는 스트링·장착서비스 필수이며 대여 결제를 추가하지 않는다", () => {
  ...
  const picker = read("TossMiniApp/src/components/RacketStringProductPicker.tsx");
  const payment = read("TossMiniApp/src/api/payments.ts");
  const purchaseRequest = payment.slice(
    payment.indexOf("export function prepareRacketPurchasePayment"),
    payment.indexOf("export function prepareRacketRentalPayment"),
  );

  assert.match(stepOne, /RacketStringProductPicker/);
  assert.match(stepOne, /mode="racket-purchase"/);
  assert.match(picker, /라켓 수량과 같은 수량의 스트링 및 장착서비스가 함께 주문됩니다/);
  assert.match(purchaseRequest, /stringProductId/);
  assert.doesNotMatch(purchaseRequest, /racket_rental|rental_purchase/);
});
...
test("수량·옵션·navigation과 새로고침 guard를 고정한다", () => {
  ...
  assert.match(picker, /Math\.min\(availability\.available, 10\)/);
  ...
  assert.match(
    flow,
    /window\.history\.replaceState\(\{ view: "racket-purchase", racketId, step: 1 \}/,
  );
...
test("결제 오류는 재고·옵션·방문예약·pending 복구 경로를 구분한다", () => {
  ...
  for (const code of [
    ...
    "RACKET_UNAVAILABLE",
    "RACKET_RENTAL_RESERVED",
    "RACKET_INSUFFICIENT_STOCK",
    "PRODUCT_NOT_AVAILABLE",
    "VARIANT_NOT_FOUND",
    "VARIANT_SOLD_OUT",
    "VARIANT_INSUFFICIENT_STOCK",
    "VISIT_SLOT_UNAVAILABLE",
    "PACKAGE_PASS_UNAVAILABLE",
    "ATTEMPT_PAYLOAD_MISMATCH",
    "PAYMENT_INTENT_EXPIRED",
    "PAYMENT_CREATION_IN_PROGRESS",
    "PAYMENT_EXECUTION_IN_PROGRESS",
    "PAYMENT_FINALIZATION_SNAPSHOT_INVALID",
  ...
  assert.match(
    flow,
    /const pending = readPendingAppsPayment\(\);[\s\S]*if \(pending\)[\s\S]*setPendingPayment\(pending\)/,
  );
