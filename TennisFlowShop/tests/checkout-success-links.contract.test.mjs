...
test("회원 소유 주문이면 게스트 주문조회 링크 대신 마이페이지 주문 탭 링크를 사용한다", () => {
  ...
  assert.equal(result.orderDetailHref, "/mypage?tab=orders");
});
...
test("신청서 ID가 있고 로그인 상태면 마이페이지 신청내역 링크를 노출한다", () => {
  ...
  assert.equal(
    result.stringingApplicationHref,
    "/mypage?tab=orders&flowType=application&flowId=apply-789&from=orders",
  );
});
