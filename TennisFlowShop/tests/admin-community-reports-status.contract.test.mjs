...
test("관리자 신고 상태 변경 API는 액션별 타겟 outcome 기록 계약을 유지한다", () => {
  const source = normalized(read("app/api/admin/community/reports/[id]/status/route.ts"));
  ...
  assert.ok(
    !source.includes("const targetOutcome = hideTargetResult?.ok ? 'updated' : 'updated';"),
  );
