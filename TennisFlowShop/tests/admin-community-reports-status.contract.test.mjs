...
test("관리자 신고 상태 변경 API는 액션별 타겟 outcome 기록 계약을 유지한다", () => {
  ...
  assert.ok(
    source.includes("const targetOutcome = TARGET_OUTCOME_BY_ACTION[action];"),
  );
  assert.ok(
    !source.includes(
  ...
  assert.ok(
    source.includes("action === 'resolve_hide_target' && hideTargetResult?.ok"),
  );
});
...
test("신고 타입은 실제 moderationAudit target outcome 정책과 일치한다", () => {
  ...
  assert.ok(
    source.includes("outcome: CommunityReportModerationTargetOutcome;"),
  );
});
