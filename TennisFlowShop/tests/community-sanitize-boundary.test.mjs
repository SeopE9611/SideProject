...
test("커뮤니티 작성/수정 API는 본문을 sanitize 후 길이를 검증한다", () => {
  ...
  assert.ok(
    postCreate.includes("validateSanitizedLength(sanitizedContent, { min: 1, max: 5000 })"),
  );
  assert.ok(postCreate.includes("content: sanitizedContent"));
  assert.ok(
    postCreate.indexOf("sanitizeHtml(body.content)") <
      postCreate.indexOf("validateSanitizedLength(sanitizedContent, { min: 1, max: 5000 })"),
  );
  ...
  assert.ok(
    postPatch.indexOf("sanitizeHtml(body.content)") <
      postPatch.indexOf("validateSanitizedLength(sanitizedContent, { min: 1, max: 5000 })"),
  );
});
...
test("커뮤니티 댓글 작성/수정 API는 본문을 sanitize 후 길이를 검증한다", () => {
  ...
  assert.ok(
    commentCreate.includes("validateSanitizedLength(sanitizedContent, { min: 1, max: 1000 })"),
  );
  assert.ok(commentCreate.includes("content: sanitizedContent"));
  assert.ok(
    commentCreate.indexOf("sanitizeHtml(body.content)") <
      commentCreate.indexOf("validateSanitizedLength(sanitizedContent, { min: 1, max: 1000 })"),
  );
  ...
  assert.ok(
    commentPatch.includes("validateSanitizedLength(sanitizedContent, { min: 1, max: 1000 })"),
  );
  assert.ok(commentPatch.includes("content: sanitizedContent"));
  assert.ok(
    commentPatch.indexOf("sanitizeHtml(parsed.data.content)") <
      commentPatch.indexOf("validateSanitizedLength(sanitizedContent, { min: 1, max: 1000 })"),
  );
});
