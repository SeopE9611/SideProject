...
test("커뮤니티 작성/수정 API는 본문을 sanitize 후 길이를 검증한다", () => {
  ...
  assert.ok(
    postCreate.includes(
      "validateSanitizedLength(sanitizedContent, { min: 1, max: 5000 })",
    ),
  );
...
test("관리자 게시글 상세는 sanitize 신뢰 경계를 문서화한다", () => {
  ...
  assert.ok(
    adminDetail.includes("dangerouslySetInnerHTML={{ __html: post.content }}"),
  );
});
