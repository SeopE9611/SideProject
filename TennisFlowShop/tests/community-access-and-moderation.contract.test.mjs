...
test("게시글 권한 계약: 읽기(GET)에서 비밀글은 비회원 401, 비작성자 회원 403으로 분기한다", () => {
  ...
  assert.ok(boardDetailRouteN.includes("error: { code: 'unauthorized', message: 'Unauthorized' }"));
  assert.match(boardDetailRoute, /status:\s*401/);
  assert.ok(boardDetailRouteN.includes("error: { code: 'forbidden', message: 'Forbidden' }"));
...
test("신고 중복 제한 계약: 댓글 신고도 동일 사용자의 5분 내 중복 신고를 429로 제한한다", () => {
  ...
  assert.match(commentReportRoute, /reporterUserId:\s*reporter\.reporterUserId/);
  assert.match(commentReportRoute, /createdAt:\s*\{\s*\$gte:\s*fiveMinutesAgo\s*\}/);
...
test("관리자 상태 변경 계약: 대상 없음/이미 삭제됨(중복 처리)/업데이트 실패를 각각 409 또는 422로 구분한다", () => {
  ...
  assert.ok(adminReportStatusRouteN.includes("status: 409, error: 'target_already_processed'"));
