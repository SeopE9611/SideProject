...
const postReportRoute = readRoute(
  "app/api/community/posts/[id]/report/route.ts",
);
const commentReportRoute = readRoute(
  "app/api/community/comments/[id]/report/route.ts",
);
const adminReportListRoute = readRoute(
...
test("게시글 권한 계약: 삭제(DELETE)는 비회원 401, 작성자 아니면 관리자 검증을 요구한다", () => {
  ...
  assert.match(
    boardDetailRoute,
    /if \(!isOwner\) \{\s*const guard = await requireAdmin\(req\);/s,
  );
});
...
test("조회수 dedupe 계약: board view는 로그인/비로그인 각각 viewerKey를 생성해 1회만 카운트한다", () => {
  ...
  assert.match(
    boardViewRoute,
    /BoardRepo\.tryAcquireViewSlot\(db, id, viewerKey\)/,
  );
...
test("관리자 상태 변경 계약: 댓글 삭제 시 commentsCount 하한(0)을 보장하는 파이프라인 감소를 사용한다", () => {
  ...
  assert.match(
    adminReportStatusRoute,
    /commentsCount 감소는 0 미만으로 내려가지 않도록/,
  );
});
...
test("관리자 상태 변경 계약: 대상 없음/이미 삭제됨(중복 처리)/업데이트 실패를 각각 409 또는 422로 구분한다", () => {
  assert.ok(
    adminReportStatusRouteN.includes("status: 409, error: 'target_not_found'"),
  );
  assert.ok(
    adminReportStatusRouteN.includes(
      "status: 409, error: 'target_already_processed'",
    ),
  );
  assert.ok(
    adminReportStatusRouteN.includes(
      "status: 422, error: 'target_update_failed'",
    ),
  );
});
...
test("관리자 상태 반영 조회 계약: 신고 목록 API는 신고/게시글/댓글의 최신 status 필드를 응답으로 노출한다", () => {
  ...
  assert.ok(
    adminReportListRouteN.includes("status: d.post.status ?? 'public',"),
  );
  assert.ok(
    adminReportListRouteN.includes("status: d.comment.status ?? 'active',"),
  );
});
