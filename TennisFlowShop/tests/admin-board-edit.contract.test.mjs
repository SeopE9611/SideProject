...
test("관리자 게시글 단건 API는 requireAdmin + CSRF + community_posts 계약을 따른다", () => {
  ...
  assert.ok(
    src.includes("if (!guard.ok) return guard.res;"),
  ...
  assert.ok(
    src.includes("collection<EditableCommunityPost>('community_posts')") ||
      src.includes("db.collection('community_posts')"),
...
test("관리자 게시글 단건 API는 400/404/422 실패 분기를 고정한다", () => {
  ...
  // 400: 잘못된 ObjectId
  assert.ok(
    src.includes("{ error: 'Invalid id' }, { status: 400 }"),
    "잘못된 id는 400 + Invalid id를 반환해야 합니다.",
  );

  // 404: 대상 문서 없음
  assert.ok(
    src.includes("{ error: 'Not found' }, { status: 404 }"),
    "대상 문서가 없으면 404 + Not found를 반환해야 합니다.",
  );

  // 422: 입력 검증 실패
  assert.ok(
    src.includes("{ error: '제목을 입력해 주세요.' }, { status: 422 }"),
    "제목 누락은 422를 반환해야 합니다.",
  );
  assert.ok(
    src.includes("{ error: '내용을 입력해 주세요.' }, { status: 422 }"),
