...
} from "../lib/board-secret-policy.js";
...
test("비밀글은 작성자/관리자 외에는 제목을 마스킹한다", () => {
  ...
  const maskedForOther = maskSecretTitle(secretPost, {
    viewerId: "other-user",
    isAdmin: false,
  });
  assert.equal(maskedForOther.title, "비밀글입니다");

  const visibleForAuthor = maskSecretTitle(secretPost, {
    viewerId: "author-1",
    isAdmin: false,
  });
  assert.equal(visibleForAuthor.title, "원본 제목");

  const visibleForAdmin = maskSecretTitle(secretPost, {
    viewerId: "admin-user",
    isAdmin: true,
  });
