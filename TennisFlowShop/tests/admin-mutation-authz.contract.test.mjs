...
function hasMutationMethod(routeSource) {
  return /export\s+(async\s+function\s+(POST|PATCH|PUT|DELETE)|\{\s*(POST|PATCH|PUT|DELETE)\s*\})/.test(
    routeSource,
  );
}

function needsCsrf(routeSource) {
  return /export\s+async\s+function\s+(POST|PATCH|PUT|DELETE)\s*\(/.test(
    routeSource,
  );
}
...
test("관리자 변경성 API는 비로그인/일반유저/admin 권한 계약(401/403/200)을 강제한다", () => {
  ...
  for (const route of files) {
    ...
    const isDelegatingRoute =
      route.src.includes("export { POST }") ||
      route.src.includes("export { PATCH }") ||
      route.src.includes("export { PUT }") ||
      route.src.includes("export { DELETE }");
...
test("패키지 주문 관리자 라우트는 requireAdmin 표준 경로만 사용한다", () => {
  ...
  for (const relPath of targets) {
    ...
    assert.ok(
      ...
      `${relPath}: requireAdmin 표준 가드를 사용해야 합니다.`,
    );
    assert.ok(
      src.includes("if (!guard.ok) return guard.res;") ||
        src.includes("if (!g.ok) return g.res;"),
    ...
    for (const snippet of forbiddenAuthSnippets) {
      assert.ok(
        ...
        `${relPath}: 레거시 인증 분기(${snippet})를 사용하면 안 됩니다.`,
      );
    }
  }
});
