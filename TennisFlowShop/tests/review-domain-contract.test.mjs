...
test("관리자 원문 후기 API는 작성자·관리자·최종 공개 상태 DTO를 내려준다", () => {
  const adminRoute = read("app/api/reviews/admin/route.ts");

  assert.ok(adminRoute.includes("moderationStatus: 1"));
  assert.ok(adminRoute.includes('const authorStatus = review.status === "hidden" ? "hidden" : "visible"'));
  assert.ok(adminRoute.includes('const moderationStatus = review.moderationStatus === "hidden" ? "hidden" : "visible"'));
  assert.ok(adminRoute.includes("const effectiveStatus ="));
  assert.ok(adminRoute.includes("status: authorStatus"));
  assert.ok(adminRoute.includes("authorStatus,"));
  assert.ok(adminRoute.includes("moderationStatus,"));
  assert.ok(adminRoute.includes("effectiveStatus,"));
  assert.ok(adminRoute.includes("masked: false"));
  assert.ok(adminRoute.includes("adminView: true"));
});

test("상품·라켓 관리자 후기 upsert는 API 상태 필드를 우선 사용하고 관리자 숨김을 유지한다", () => {
  const productUtils = read("app/products/[id]/ProductDetailReviewData.utils.ts");
  const racketClient = read("app/rackets/[id]/_components/RacketDetailClient.tsx");

  for (const source of [productUtils, racketClient]) {
    assert.ok(source.includes("authorStatus: review.authorStatus ??"));
    assert.ok(source.includes("moderationStatus: review.moderationStatus ?? current?.moderationStatus ?? \"visible\""));
    assert.ok(source.includes("review.effectiveStatus ??"));
    assert.ok(source.includes("review.moderationStatus !== \"hidden\""));
    assert.ok(source.includes("adminView: true"));
  }
});
