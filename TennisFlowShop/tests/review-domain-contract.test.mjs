...
test("후기 GET/POST canonical 정책 통일 계약: reviewed 우선, 공용 helper, 중복 방어를 유지한다", () => {
  ...
  assert.ok(reviewsRoute.includes("{ serviceApplicationId: { $in: applicationIdCandidates } }"));
...
test("관리자 원문 후기 API는 작성자·관리자·최종 공개 상태 DTO를 내려준다", () => {
  ...
  assert.ok(
    adminRoute.includes('const authorStatus = review.status === "hidden" ? "hidden" : "visible"'),
  );
  assert.ok(
    adminRoute.includes(
