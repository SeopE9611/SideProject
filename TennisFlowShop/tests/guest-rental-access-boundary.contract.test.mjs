...
test("대여 접근 경계는 회원 본인과 올바른 guest rental token만 허용한다", () => {
  ...
  assert.deepEqual(
    getRentalAccessDecision({
      rental: {}, rentalId, accessClaims: null, hasGuestRentalAccess: true,
    }),
    { ok: true, isGuestRental: true },
  );
});

test("대여 접근 경계는 token 없음·잘못된 token·다른 rental token을 동일 코드로 거부한다", () => {
  for (const hasGuestRentalAccess of [false, false, false]) {
    assert.deepEqual(
      getRentalAccessDecision({
        rental: {}, rentalId, accessClaims: null, hasGuestRentalAccess,
      }),
  ...
  assert.deepEqual(
    getRentalAccessDecision({
      rental: { userId: "member-1" }, rentalId, accessClaims: null, hasGuestRentalAccess: true,
    }),
    { ok: false, reason: "RENTAL_NOT_AVAILABLE" },
  );
  assert.deepEqual(
    getRentalAccessDecision({
      rental: null, rentalId, accessClaims: null, hasGuestRentalAccess: false,
    }),
    { ok: false, reason: "RENTAL_NOT_AVAILABLE" },
  );
});
