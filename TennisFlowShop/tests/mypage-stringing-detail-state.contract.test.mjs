...
test("마이페이지 교체서비스 상세는 미해결 연결의 결제와 입금계좌를 확정하지 않는다", () => {
  assert.match(client, /isUnresolvedLinkedApplication[\s\S]*\? "결제 상태 확인 중"/);
  assert.match(
    client,
    /isUnresolvedLinkedApplication[\s\S]*\? null[\s\S]*: applicationPaymentMethod/,
  );
  assert.match(
    client,
...
test("마이페이지 교체서비스 상세는 서비스 금액과 실제 거래 결제금액을 분리한다", () => {
  ...
  assert.match(client, /primaryLinkedSource === "order"[\s\S]*linkedOrderSummary\?\.totalAmount/);
  assert.match(client, /primaryLinkedSource === "rental"[\s\S]*linkedRentalSummary\?\.totalAmount/);
