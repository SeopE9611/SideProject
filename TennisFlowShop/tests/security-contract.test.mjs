...
test("표시 계약: 후기/교체서비스/관리자 대여 화면은 raw 상태와 결제수단을 직접 렌더링하지 않는다", () => {
  ...
  const applicationBadge = read("app/features/stringing-applications/components/ApplicationStatusBadge.tsx");
  const stringingDetail = read("app/features/stringing-applications/components/StringingApplicationDetailClient.tsx");
  const paymentDetail = read("app/features/stringing-applications/components/PaymentMethodDetail.tsx");
  ...
  assert.ok(stringingDetail.includes('label="결제 방식" value={getCustomerPaymentMethodLabel(paymentMethodForDisplay, packageApplied)}'));
  assert.ok(paymentDetail.includes("getMypagePaymentStatusLabel(paymentStatus)"));
  assert.ok(paymentDisplay.includes('["bank_transfer", "manual_bank_transfer", "bank", "virtual_account"].includes(lowered)'));
...
test("후기 대상 통합 계약: 서비스 연결 주문/대여는 통합 reviewContext로 처리한다", () => {
  ...
  assert.ok(!orderDetail.includes("/reviews/write?service=stringing&applicationId=${reviewableStringingAppId}"));
  assert.ok(!orderDetail.includes('import OrderReviewCTA from "@/components/reviews/OrderReviewCTA";'));
  ...
  assert.ok(!orderDetail.includes("border-warning/60 bg-warning/10"));
  ...
  assert.ok(orderDetail.includes("!isReviewItemsLoading && reviewTotal !== null && reviewRemaining !== null"));
  assert.ok(userOrders.includes("reviewContext") && userOrders.includes("reviewNextApplicationId"));
  assert.ok(activityRoute.includes('reviewContext === "product_stringing"'));
  assert.ok(!activityRoute.includes("isConfirmed && !serviceLinkedOrder ? reviewPendingProductIds.length : 0"));
  ...
  assert.ok(transactionFlowList.includes("!applicationActionTarget.orderId") && transactionFlowList.includes("!applicationActionTarget.rentalId"));
...
test("마이페이지 거래 카드 계약: 상태 배지와 액션 배치를 단순화한다", () => {
  ...
  assert.ok(transactionFlowList.includes("const primaryAction = primaryActionCandidates[0] ?? null;"));
