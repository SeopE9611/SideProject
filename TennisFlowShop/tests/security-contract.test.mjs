...
test("보안 계약: CSRF 실패, Origin 불일치, 토큰 오류, 재전송(멱등성) 케이스를 강제한다", () => {
  ...
  assert.ok(
    adminCsrf.includes(
      "export const ADMIN_CSRF_COOKIE_KEY = 'adminCsrfToken';",
    ),
  );
  assert.ok(
    adminCsrf.includes(
      "export const ADMIN_CSRF_HEADER_KEY = 'x-admin-csrf-token';",
    ),
  );
  ...
  assert.ok(
    adminGuard.includes("if (!at) return { ok: false, res: authError(401) };"),
  );
  assert.ok(adminGuard.includes("payloadRaw = verifyAccessToken(at);"));
  assert.ok(
    adminGuard.includes("if (!payload || !ObjectId.isValid(payload.sub))"),
  );
