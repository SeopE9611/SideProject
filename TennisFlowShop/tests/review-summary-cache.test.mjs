...
function compileTs(rel, stubs = {}) {
  ...
  const filename = join(mkdtempSync(join(tmpdir(), "review-summary-")), rel.replaceAll("/", "_") + ".cjs");
...
test("public summary stage와 fake aggregate 변환을 실제 운영 함수로 확인한다", async () => {
  ...
  const db = makeDb({ products: [{ _id: oid("1") }], "reviews:aggregate": [{ average: 4.236, count: "3" }] });
  assert.deepEqual(await surface.getPublicReviewSummary(db, { type: "product", id: String(oid("1")) }), { average: 4.24, count: 3 });
  assert.deepEqual(await surface.getPublicReviewSummary(makeDb({ products: [{ _id: oid("2") }] }), { type: "product", id: String(oid("2")) }), { average: 0, count: 0 });
  ...
  assert.match(read("lib/reviews/public-review-surface.server.ts"), /summary: buildPublicReviewSummaryStages\(\)/);
});

test("영향 대상 수집은 상품·라켓·신청서·부모 주문/대여와 중복 제거를 실행한다", async () => {
  const product = oid("1");
  const racket = oid("2");
  assert.deepEqual(await targets({ productId: String(product) }, { products: [{ _id: product }] }), { productIds: [String(product)], racketIds: [] });
  assert.deepEqual(await targets({ productId: String(racket) }, { used_rackets: [{ _id: racket }] }), { productIds: [], racketIds: [String(racket)] });
  assert.deepEqual(await targets({ productId: String(product) }, { products: [{ _id: product }], used_rackets: [{ _id: product }] }), { productIds: [String(product)], racketIds: [String(product)] });
  assert.deepEqual(await targets({ relatedProductIds: ["", "p1", "p1"], relatedRacketIds: ["r1", "", "r1"] }, {}), { productIds: ["p1"], racketIds: ["r1"] });

  const appId = oid("3");
  assert.deepEqual(await targets({ serviceApplicationId: String(appId) }, {
    stringing_applications: [{ _id: appId, racketId: racket, stringDetails: { stringTypes: [product] } }],
  }), { productIds: [String(product)], racketIds: [String(racket)] });
  assert.deepEqual(await targets({ applicationId: String(appId) }, {
    stringing_applications: [{ _id: appId, racketId: racket, stringDetails: { stringTypes: [product] } }],
  }), { productIds: [String(product)], racketIds: [String(racket)] });
  ...
  assert.deepEqual(await targets({ applicationId: String(appId) }, {
    ...
    orders: [{ _id: orderId, items: [{ type: "racket", productId: racket }, { type: "product", productId: product }] }],
  }), { productIds: [], racketIds: [String(racket)] });
  assert.deepEqual(await targets({ rentalId: String(rentalId) }, { rental_orders: [{ _id: rentalId, racketId: racket }] }), { productIds: [], racketIds: [String(racket)] });
  assert.deepEqual(await targets({ reviewContext: "product_stringing", orderId: String(orderId) }, { orders: [{ _id: orderId, items: [{ type: "racket", productId: racket }, { type: "product", productId: product }] }] }), { productIds: [], racketIds: [String(racket)] });
  assert.deepEqual(await targets({ reviewContext: "product", orderId: String(orderId) }, { orders: [{ _id: orderId, items: [{ type: "racket", productId: racket }] }] }), { productIds: [], racketIds: [] });
  assert.deepEqual(await targets({ racketId: String(racket), relatedRacketIds: [String(racket)], serviceApplicationId: String(appId), rentalId: String(rentalId) }, {
...
test("캐시 update payload와 safe wrapper를 실제 운영 함수로 실행한다", async () => {
  ...
  const dependencies = { getPublicReviewSummary: async (_db, target) => target.type === "product" ? { average: 4.236, count: 2 } : { average: 0, count: 0 } };
  assert.deepEqual(await cache.refreshReviewSummaryCachesForTargets(db, { productIds: [String(product)], racketIds: [String(racket)] }, dependencies), { productsUpdated: 1, racketsUpdated: 1 });
  ...
  const racketSet = db.calls.updateOne.find((call) => call.collection === "used_rackets").update.$set;
  ...
  try {
    await cache.refreshReviewSummaryCachesForReviewSafely(db, { productId: String(product) }, "test", { getPublicReviewSummary: async () => { throw new Error("boom"); } });
  } finally {
