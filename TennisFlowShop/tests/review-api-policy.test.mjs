...
test("후기 POST와 cursor 정책 계약: photos 타입과 cursor 필수 필드를 엄격 검증한다", () => {
  ...
  assert.ok(postRoute.includes("isValidReviewCursor(parsed, sort"));
  assert.ok(guard.includes('!value || typeof value !== "object" || Array.isArray(value)'));
  assert.ok(guard.includes('isValidId(String(cursor.id ?? ""))'));
  assert.ok(guard.includes('sort === "latest"'));
  assert.ok(guard.includes("cursor.createdAt"));
  assert.ok(guard.includes('sort === "helpful" ? cursor.helpfulCount : cursor.rating'));
});
...
test("helpful API는 실제 공개 후기만 허용하고 관리자 숨김을 차단한다", () => {
  ...
  assert.ok(
    helpfulRoute.includes('exists.status !== "visible" || exists.moderationStatus === "hidden"'),
  );
...
test("후기 POST body 계약: 일반 JSON 객체만 허용하고 검증 이후 필드에 접근한다", () => {
  ...
  assert.ok(postRoute.includes("isPlainReviewRequestBody(body)"));
  assert.ok(guard.includes("!Array.isArray(value)"));
...
test("GET 후기 상품 필터는 권한 $or와 target $or를 $and로 함께 보존한다", () => {
  const route = read("app/api/reviews/route.ts");
  const helper = read("lib/reviews/review-query-match.ts");

  assert.ok(helper.includes("export function appendMatchCondition"));
  assert.ok(
    helper.includes("const existingAnd = Array.isArray(match.$and) ? [...match.$and] : []"),
  );
  assert.ok(helper.includes("existingAnd.push({ $or: match.$or })"));
  assert.ok(helper.includes("delete match.$or"));
  assert.ok(helper.includes("existingAnd.push(condition)"));
  assert.ok(helper.includes("match.$and = existingAnd"));
  assert.ok(route.includes("import { appendMatchCondition }"));
  assert.ok(route.includes('withHidden === "mask"'));
  assert.ok(route.includes('{ moderationStatus: { $ne: "hidden" } }'));
  assert.ok(route.includes("{ userId: currentUserId }"));
  assert.ok(
    route.includes("if (productTargetMatch) appendMatchCondition(match, productTargetMatch);"),
  );
  assert.ok(!route.includes("Object.assign(match, productTargetMatch)"));
});

test("GET 후기 상품 필터 권한 조합은 일반 사용자·비로그인·관리자 정책을 분리한다", () => {
  const route = read("app/api/reviews/route.ts");
  ...
  assert.ok(route.includes("const match: any = isAdmin\n    ? { isDeleted: { $ne: true } }"));
  assert.ok(route.includes('if (type === "product") match.productId = { $exists: true };'));
  assert.ok(route.includes('if (type === "rental") {'));
  assert.ok(route.includes("appendMatchCondition(match, rentalTypeMatch);"));
  assert.ok(route.includes('if (productFilterCandidates && type !== "product")'));
});

test("관리자 후기 목록 상태 변경은 저장 실패와 재검증 실패를 분리하고 부분 성공을 롤백하지 않는다", () => {
  const adminClient = read("app/admin/reviews/_components/AdminReviewListClient.tsx");

  assert.ok(adminClient.includes('"[admin-reviews] failed to revalidate after moderation update"'));
  assert.ok(
    adminClient.includes('"[admin-reviews] failed to revalidate after bulk moderation update"'),
  );
  assert.ok(adminClient.includes("Promise.allSettled("));
  assert.ok(adminClient.includes("const failedIds: string[] = []"));
  assert.ok(adminClient.includes("setSelected(failedIds);"));
  assert.ok(adminClient.includes("선택 항목 상태 변경에 실패했습니다."));
  assert.ok(adminClient.includes("개 처리에 실패했습니다."));
  assert.ok(!adminClient.includes("await Promise.all(\n          part.map"));
  assert.ok(!adminClient.includes("일부 항목 상태 변경에 실패했습니다."));
});
