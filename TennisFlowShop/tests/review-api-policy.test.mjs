...
test("GET 후기 상품 필터는 권한 $or와 target $or를 $and로 함께 보존한다", () => {
  ...
  assert.ok(helper.includes("const existingAnd = Array.isArray(match.$and) ? [...match.$and] : []"));
  ...
  assert.ok(route.includes("if (productTargetMatch) appendMatchCondition(match, productTargetMatch);"));
...
test("GET 후기 상품 필터 권한 조합은 일반 사용자·비로그인·관리자 정책을 분리한다", () => {
  ...
  assert.ok(route.includes('currentUserId\n            ? {\n                $or: [\n                  { moderationStatus: { $ne: "hidden" } },\n                  { userId: currentUserId },\n                ],\n              }\n            : { moderationStatus: { $ne: "hidden" } }'));
...
test("관리자 후기 목록 상태 변경은 저장 실패와 재검증 실패를 분리하고 부분 성공을 롤백하지 않는다", () => {
  ...
  assert.ok(adminClient.includes('"[admin-reviews] failed to revalidate after bulk moderation update"'));
