...
import {
  ...
  buildBoardPublicUrl,
} from "../lib/board-public-url-policy.js";

test("buildBoardPublicUrl: postNo 기반 공개 URL을 정상 생성한다", () => {
  const result = buildBoardPublicUrl({
    type: "notice",
    postNo: 42,
    status: "public",
  });
...
test("buildBoardPublicUrl: hidden/private 상태는 공개 링크를 차단한다", () => {
  const hiddenResult = buildBoardPublicUrl({
    type: "qna",
    postNo: 9,
    status: "hidden",
  });
  const privateResult = buildBoardPublicUrl({
    type: "qna",
    postNo: 9,
    status: "private",
  });
...
test("buildAdminBoardDetailUrl: id가 유효할 때만 관리자 상세 fallback URL을 생성한다", () => {
  assert.equal(
    buildAdminBoardDetailUrl({ id: "abc123" }),
    "/admin/boards/abc123",
  );
...
test("buildAdminBoardDetailUrl: 공백/슬래시/한글 식별자를 URL-safe 형태로 인코딩한다", () => {
  ...
  assert.equal(
    buildAdminBoardDetailUrl({ id: "group/42" }),
    "/admin/boards/group%2F42",
  );
  assert.equal(
    buildAdminBoardDetailUrl({ id: "공지사항/테스트 글" }),
    "/admin/boards/%EA%B3%B5%EC%A7%80%EC%82%AC%ED%95%AD%2F%ED%85%8C%EC%8A%A4%ED%8A%B8%20%EA%B8%80",
  );
});
