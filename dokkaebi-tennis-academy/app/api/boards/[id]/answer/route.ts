...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  await db.collection('board_posts').updateOne(
    ...
    {
      $set: {
        answer: { content, authorId: String(admin.sub), authorName: (admin as any).name, createdAt: now },
        updatedAt: now,
      },
    }
  );
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const db = await getDb();
  const { id } = await params;
  await db.collection('board_posts').updateOne({ _id: new ObjectId(id) }, { $set: { 'answer.content': content, 'answer.updatedAt': new Date() } });
  return NextResponse.json({ ok: true });
}
