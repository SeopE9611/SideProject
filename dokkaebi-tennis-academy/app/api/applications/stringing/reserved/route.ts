...
export async function GET(req: Request) {
  ...
  try {
    ...
    // 'stringingApplications' 컬렉션 접근 (스트링 장착 신청서가 저장된 곳)
    const applications = db.collection('stringingApplications');

    // 해당 날짜(date)에 접수된 신청서 중 preferredTime(희망 시간대) 필드만 조회
    const results = await applications
      .find({ preferredDate: date }) // preferredDate가 같은 문서 필터
      .project({ preferredTime: 1, _id: 0 }) // preferredTime만 가져오고 _id는 제외
      .toArray(); // 커서를 배열로 변환
