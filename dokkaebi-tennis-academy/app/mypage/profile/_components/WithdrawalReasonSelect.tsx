...
export default function WithdrawalReasonSelect({ onSubmit }: WithdrawalReasonSelectProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* 제출 버튼 */}
      <button onClick={handleSubmit} disabled={submitting} className={`px-4 py-2 text-sm font-medium text-destructive-foreground bg-destructive hover:bg-destructive/90 rounded-md ${submitting ? 'opacity-60 cursor-not-allowed' : ''}`}>
        탈퇴하기
      </button>
    </div>
  );
