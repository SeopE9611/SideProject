...
export default function StringingApplicationDetailClient({ id, baseUrl, backUrl = '/admin/orders', isAdmin = true, userEditableStatuses = ['검토 중', '접수완료'] }: Props) {
 ...
 <div>
 <h1 className="text-3xl font-bold tracking-tight text-foreground dark:text-foreground">{isAdmin ? '스트링 신청 관리' : '스트링 신청 상세'}</h1>
 <p className="mt-1 text-muted-foreground dark:text-muted-foreground">신청 ID: {data.id}</p>
 </div>
 </div>
 <TooltipProvider>
 ...
 variant="outline"
 size="sm"
 className="bg-card/70 backdrop-blur-sm border-green-200 hover:bg-green-50
 ...
 : isEditMode
 ? ''
 : 'bg-card/70 backdrop-blur-sm border-green-200 hover:bg-green-50 \
 dark:bg-slate-800/60 dark:border-slate-700 dark:hover:bg-slate-700/60'
 }
 onClick={() => {
 ...
 <span className="text-sm font-medium text-gray-700 ">신청일시</span>
 </div>
 <p className="text-lg font-semibold text-foreground dark:text-foreground">{new Date(data.requestedAt).toLocaleDateString()}</p>
 </div>

 <div className="bg-card/70 rounded-xl p-4 backdrop-blur-sm">
 <div className="flex items-center space-x-2 mb-2">
 <CreditCard className="h-4 w-4 text-muted-foreground" />
 <span className="text-sm font-medium text-gray-700 ">총 비용</span>
 </div>
 <p className="text-lg font-semibold text-foreground dark:text-foreground">{data.totalPrice.toLocaleString()}원</p>
 </div>

 <div className="bg-card/70 rounded-xl p-4 backdrop-blur-sm">
 <div className="flex items-center space-x-2 mb-2">
 <Target className="h-4 w-4 text-muted-foreground" />
 <span className="text-sm font-medium text-gray-700 ">라켓 종류</span>
 </div>
 <p className="text-lg font-semibold text-foreground dark:text-foreground">{racketTypeSummary}</p>
 </div>

 <div className="bg-card/70 rounded-xl p-4 backdrop-blur-sm">
 <div className="flex items-center space-x-2 mb-2">
 <Calendar className="h-4 w-4 text-muted-foreground" />
 <span className="text-sm font-medium text-gray-700 ">희망 일시</span>
 </div>
 <p className="text-lg font-semibold text-foreground dark:text-foreground">{visitTimeLabel}</p>
 </div>
 </div>
 {data.orderId && (
 ...
 <p className="font-semibold text-foreground dark:text-foreground">{data.customer.name ?? '정보 없음'}</p>
 </div>
 </div>
 ...
 <p className="font-semibold text-foreground dark:text-foreground">{data.customer.email ?? '정보 없음'}</p>
 </div>
 </div>
 ...
 <p className="font-semibold text-foreground dark:text-foreground">{data.customer?.phone ?? '정보 없음'}</p>
 </div>
 </div>
 ...
 <p className="font-semibold text-foreground dark:text-foreground">{data.customer?.address ?? '정보 없음'}</p>
 {data.customer?.addressDetail && <p className="text-sm text-muted-foreground dark:text-muted-foreground mt-1">{data.customer.addressDetail}</p>}
 {data.customer?.postalCode && <p className="text-sm text-muted-foreground dark:text-muted-foreground">우편번호: {data.customer.postalCode}</p>}
 </div>
 </div>
 </div>
 )}
 ...
 <CardFooter className="pt-2 flex justify-center bg-gray-50/50 ">
 ...
 <div className="mt-3 rounded-lg border border-dashed border-purple-100 bg-purple-50/60 px-3 py-2 text-xs text-gray-700 dark:border-purple-700/60 dark:bg-purple-950/30 dark:text-foreground">
 ...
 <CardHeader className="bg-gradient-to-r from-gray-50 to-gray-100 dark:from-slate-800 dark:to-slate-700 border-b border-border dark:border-slate-700 flex flex-col items-center py-4">
 ...
 <span className="inline-flex items-center rounded-full bg-card/90 dark:bg-slate-800 px-3 py-1 text-xs sm:text-sm font-medium text-slate-900 dark:text-slate-50">스트링 {stringTypeCount}종</span>
 <span className="inline-flex items-center rounded-full bg-card/90 dark:bg-slate-800 px-3 py-1 text-xs sm:text-sm font-medium text-slate-900 dark:text-slate-50">라켓 {racketCount}자루</span>
 </div>
 ...
 <div className="text-right text-foreground dark:text-foreground text-sm">{visitTimeLabel}</div>
 </section>
 ...
 <div key={line.id ?? index} className="rounded-xl px-4 py-3 ring-1 ring-slate-200/70 bg-card/70 dark:ring-slate-700 dark:bg-slate-900/40">
 {/* 라켓 이름 + 순번 */}
 ...
 <div className="overflow-hidden rounded-xl ring-1 ring-slate-200/70 bg-card/80 dark:ring-slate-700 dark:bg-slate-900/60">
 {/* 헤더 행 */}
 ...
 <div className="pr-2">
 <p className="font-medium text-foreground dark:text-foreground truncate">{item.name}</p>
 </div>
 <div className="text-center text-xs text-muted-foreground">x {item.quantity}개</div>
 <div className="text-right font-semibold text-foreground dark:text-foreground">{item.price.toLocaleString()}원</div>
 </div>
 ))}
 </div>
 </section>
 ...
 <div className="text-right text-foreground dark:text-foreground text-sm max-w-xs">{racketTypeSummary}</div>
 </section>
 </div>
 </CardContent>
 ...
 {/* 수정 버튼 */}
 {isEditMode && (
 <CardFooter className="flex justify-center pt-2 bg-gray-50/50 ">
 ...
 <p className="text-gray-700 whitespace-pre-line">{data.stringDetails.requirements}</p>
 </div>
 ) : (
 <p className="text-muted-foreground dark:text-muted-foreground italic">요청사항이 없습니다.</p>
 )}
 </CardContent>
 ...
 {isAdmin && (
 <Card className="border-0 shadow-xl bg-card/80 dark:bg-slate-900/80 backdrop-blur mb-8">
 <CardHeader className="bg-gradient-to-r from-gray-50 to-gray-100 dark:from-slate-800 dark:to-slate-700 border-b border-border dark:border-slate-700 flex flex-col items-center py-4">
