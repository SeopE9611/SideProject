...
export default function FreeBoardClient() {
  ...
  const handleCategoryChange = (next: 'all' | 'racket' | 'string' | 'shoes' | 'bag' | 'apparel' | 'grip' | 'accessory' | 'ball' | 'other') => {
    ...
    router.push(`/board/gear?${params.toString()}`);
    setPage(1);
  };
            ...
            {authorId && (
              <div className="flex items-center gap-2 text-sm">
                ...
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => router.push('/board/gear')} // 쿼리 제거(해제)
                >
                  해제
                </Button>
              </div>
              ...
              <>
                ...
                <div className="hidden text-sm md:block">
                  ...
                  <div className="divide-y divide-gray-100 dark:divide-gray-800">
                    {items.map((post) => (
                      <Link key={post.id} href={`/board/gear/${post.postNo ?? post.id}`} className="grid grid-cols-[60px_80px_minmax(0,1fr)_120px_140px_70px_70px_70px] items-center px-4 py-3 text-sm hover:bg-blue-50/40 dark:hover:bg-gray-800/60">
                        {/* 번호 */}
                            ...
                            <DropdownMenuContent align="start" className="w-44">
                              <DropdownMenuItem
                                onClick={(e) => {
                                  ...
                                  router.push(`/board/gear?authorId=${post.userId}&authorName=${encodeURIComponent(authorName)}`);
                                }}
                              >
                                이 작성자의 글 보기
                              </DropdownMenuItem>
                              ...
                              <DropdownMenuItem
                                onClick={(e) => {
                                  ...
                                  e.stopPropagation();
                                  if (!post.userId) return;
                                  router.push(`/board/gear/${post.postNo ?? post.id}?openProfile=1`);
                                }}
                              >
                                작성자 테니스 프로필
                              </DropdownMenuItem>
                ...
                <div className="space-y-3 md:hidden">
                  {items.map((post) => (
                    <Link
                      key={post.id}
                      href={`/board/gear/${post.postNo ?? post.id}`}
