...
export default function TensionGuidePage() {
  ...
  const getTensionFeedback = (value: number) => {
    if (value < 22) return { level: 'low', text: '매우 낮음 - 파워 중심', color: 'text-emerald-600' };
    if (value < 25) return { level: 'medium-low', text: '낮음 - 편안함 & 파워', color: 'text-teal-600' };
    if (value < 27) return { level: 'medium', text: '중간 - 균형 잡힌 세팅', color: 'text-blue-600' };
    if (value < 29) return { level: 'medium-high', text: '높음 - 컨트롤 중심', color: 'text-violet-600' };
    return { level: 'high', text: '매우 높음 - 최대 컨트롤', color: 'text-orange-600' };
  };
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="relative overflow-hidden bg-slate-50 dark:bg-slate-900/50">
        <div className="relative mx-auto w-full px-3 bp-sm:px-4 bp-md:px-6 bp-lg:max-w-[1200px] bp-lg:px-6 pt-8 bp-sm:pt-10 bp-md:pt-12 pb-10 bp-md:pb-16">
          <div className="text-center">
            <div className="inline-flex items-center gap-2 bg-blue-100 dark:bg-blue-900/50 px-4 py-2 rounded-full mb-4 bp-md:mb-6">
              <Gauge className="h-4 w-4 bp-sm:h-5 bp-sm:w-5 text-blue-600 dark:text-blue-400" />
              <span className="text-xs bp-sm:text-sm font-semibold text-blue-700 dark:text-blue-300">전문가 텐션 가이드</span>
            </div>
            <h1 className="text-3xl bp-sm:text-4xl bp-md:text-5xl bp-lg:text-6xl font-bold text-foreground mb-4 bp-md:mb-6 text-balance">
              나에게 맞는
              <br />
              <span className="text-blue-600 dark:text-blue-400">최적의 텐션</span>을 찾아보세요
            </h1>
            ...
            <div className="flex flex-wrap justify-center gap-4 bp-sm:gap-6 bp-md:gap-12 text-center">
              <div className="bg-white dark:bg-slate-800/80 rounded-xl px-4 py-3 bp-sm:px-6 bp-sm:py-4 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 cursor-default">
      ...
      <div className="mx-auto w-full px-3 bp-sm:px-4 bp-md:px-6 bp-lg:max-w-[1200px] bp-lg:px-6 py-6 bp-md:py-10 bp-lg:pb-16">
        ...
        <Tabs value={activeSection} onValueChange={setActiveSection} className="mb-8 bp-md:mb-12">
          <TabsList className="grid w-full max-w-2xl mx-auto grid-cols-4 h-auto p-1 bg-slate-100 dark:bg-slate-800">
          ...
          <TabsContent value="calculator" className="mt-6 bp-md:mt-8">
            <div className="grid bp-lg:grid-cols-2 gap-6 bp-md:gap-8">
              ...
              <div className="space-y-6">
                <Card className="border-0 shadow-md bg-white dark:bg-slate-800/90">
                  <CardHeader className="pb-4 bp-md:pb-6">
                    <CardTitle className="flex items-center gap-2 text-base bp-md:text-lg text-foreground">
                      <Settings2 className="h-4 w-4 bp-md:h-5 bp-md:w-5 text-blue-600 dark:text-blue-400" />
                      나의 플레이 정보
                    </CardTitle>
                  </CardHeader>
                        ...
                        {stringTypes.map((st) => (
                          <button
                            ...
                            className={`p-3 bp-sm:p-4 rounded-xl transition-all duration-200 text-left ${
                              stringType === st.id ? 'bg-blue-100 dark:bg-blue-900/50 ring-2 ring-blue-500 shadow-sm' : 'bg-slate-50 dark:bg-slate-700/50 hover:bg-slate-100 dark:hover:bg-slate-700 hover:shadow-sm'
                            }`}
                          >
                            <div className="flex items-center gap-2 mb-1">
                              <st.icon className={`h-4 w-4 ${stringType === st.id ? 'text-blue-600 dark:text-blue-400' : 'text-muted-foreground'}`} />
                              <span className={`font-medium text-sm ${stringType === st.id ? 'text-blue-700 dark:text-blue-300' : 'text-foreground'}`}>{st.name}</span>
                            </div>
                        ...
                        {playStyleOptions.map((option) => (
                          <button
                            ...
                            className={`p-2 bp-sm:p-3 rounded-xl transition-all duration-200 ${
                              playStyle === option.id ? 'bg-blue-100 dark:bg-blue-900/50 ring-2 ring-blue-500 shadow-sm' : 'bg-slate-50 dark:bg-slate-700/50 hover:bg-slate-100 dark:hover:bg-slate-700 hover:shadow-sm'
                            }`}
                          >
                            <div className={`font-medium text-xs bp-sm:text-sm ${playStyle === option.id ? 'text-blue-700 dark:text-blue-300' : 'text-foreground'}`}>{option.label}</div>
                      ...
                      <div className="grid grid-cols-3 gap-2 bp-sm:gap-3">
                        {swingSpeedOptions.map((option) => (
                          <button
                            ...
                            className={`p-2 bp-sm:p-3 rounded-xl transition-all duration-200 ${
                              swingSpeed === option.id ? 'bg-blue-100 dark:bg-blue-900/50 ring-2 ring-blue-500 shadow-sm' : 'bg-slate-50 dark:bg-slate-700/50 hover:bg-slate-100 dark:hover:bg-slate-700 hover:shadow-sm'
                            }`}
                          >
                            <div className={`font-medium text-xs bp-sm:text-sm ${swingSpeed === option.id ? 'text-blue-700 dark:text-blue-300' : 'text-foreground'}`}>{option.label}</div>
                            <div className="text-[10px] bp-sm:text-xs text-muted-foreground mt-1 hidden bp-sm:block">{option.desc}</div>
                          </button>
                        ))}
                      </div>
              ...
              {/* 결과 섹션 */}
              <div className="space-y-6">
                <Card className="border-0 shadow-lg bg-gradient-to-br from-blue-50 to-blue-100/80 dark:from-blue-950/60 dark:to-slate-800/90 overflow-hidden">
                  <CardHeader className="pb-4">
                    <CardTitle className="flex items-center gap-2 text-base bp-md:text-lg text-blue-900 dark:text-blue-100">
                  ...
                  <CardContent>
                    <div className="text-center mb-6 bp-md:mb-8">
                      <div className="text-5xl bp-sm:text-6xl bp-md:text-7xl font-bold text-blue-600 dark:text-blue-400 mb-2 animate-in fade-in duration-500">{calculatedTension}kg</div>
                    ...
                    <div className="mb-6 bp-md:mb-8">
                      ...
                      <div className="relative h-3 bp-sm:h-4 bg-gradient-to-r from-emerald-400 via-blue-400 to-orange-400 rounded-full shadow-inner">
                        <div
                          className="absolute top-1/2 -translate-y-1/2 w-5 h-5 bp-sm:w-6 bp-sm:h-6 bg-white dark:bg-slate-900 ring-4 ring-blue-500 rounded-full shadow-lg transition-all duration-500 ease-out"
                          style={{ left: `calc(${((calculatedTension - 20) / 12) * 100}% - 12px)` }}
                        />
                      </div>
                      <div className="flex justify-between text-xs mt-2">
                        <span className="text-emerald-600 dark:text-emerald-400">파워</span>
                        <span className="text-orange-600 dark:text-orange-400">컨트롤</span>
                      </div>
                    </div>

                    {/* 추천 범위 */}
                    <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm rounded-xl p-4 mb-6">
                  ...
                  <CardContent>
                    <div className="grid grid-cols-2 gap-2 bp-sm:gap-3">
                      {environmentFactors.map((factor, index) => (
                        <div key={index} className="flex items-start gap-2 p-2 bp-sm:p-3 bg-slate-50 dark:bg-slate-700/50 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors duration-200">
                          ...
                          <div>
                            ...
                            <div className="text-[10px] bp-sm:text-xs text-blue-600 dark:text-blue-400 font-semibold">{factor.adjustment}</div>
                          </div>
                        </div>
                      ))}
                    </div>
            ...
            <div className="grid grid-cols-1 bp-md:grid-cols-2 gap-4 bp-md:gap-6">
              {playerTypes.map((player, index) => {
                ...
                return (
                  <Card
                    key={index}
                    className={`cursor-pointer transition-all duration-300 overflow-hidden border bg-card ${isSelected ? 'ring-2 ring-blue-500 shadow-lg' : 'hover:shadow-md'}`}
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex items-center gap-3 bp-sm:gap-4">
                          <div className={`w-10 h-10 bp-sm:w-12 bp-sm:h-12 bp-md:w-14 bp-md:h-14 bg-gradient-to-br ${player.color} rounded-xl bp-md:rounded-2xl flex items-center justify-center shadow-md`}>
                            <IconComponent className="h-5 w-5 bp-sm:h-6 bp-sm:w-6 bp-md:h-7 bp-md:w-7 text-white" />
                          </div>
                          <div>
                    ...
                    <CardContent>
                      ...
                      {isSelected && (
                        <div className="mt-3 bp-md:mt-4 pt-3 bp-md:pt-4 border-t border-border animate-in fade-in slide-in-from-top-2">
                          <div className="flex items-start gap-2 bg-amber-50 dark:bg-amber-900/20 p-3 rounded-lg">
                            <Lightbulb className="h-4 w-4 bp-md:h-5 bp-md:w-5 text-amber-500 flex-shrink-0 mt-0.5" />
                ...
                <Card key={index} className="overflow-hidden border bg-card hover:shadow-md transition-all duration-300">
                    ...
                    <div className="flex items-center justify-between flex-wrap gap-2">
                      <CardTitle className="flex items-center gap-2 bp-sm:gap-3 text-base bp-sm:text-lg bp-md:text-xl text-card-foreground">
                        <div className={`w-8 h-8 bp-sm:w-9 bp-sm:h-9 bp-md:w-10 bp-md:h-10 bg-gradient-to-br ${string.color} rounded-lg bp-md:rounded-xl flex items-center justify-center`}>
                          <string.icon className="h-4 w-4 bp-sm:h-5 bp-sm:w-5 text-white" />
                        </div>
                        {string.name}
                      </CardTitle>
                  ...
                  <CardContent className="space-y-3 bp-md:space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-3 bp-md:gap-4">
                      <div>
                        <h4 className="text-xs bp-md:text-sm font-semibold text-emerald-600 dark:text-emerald-400 mb-2 flex items-center gap-1">
                      ...
                      <div>
                        <h4 className="text-xs bp-md:text-sm font-semibold text-amber-600 dark:text-amber-400 mb-2 flex items-center gap-1">
                    ...
                    <div className="bg-blue-50 dark:bg-blue-900/30 p-3 bp-md:p-4 rounded-xl">
                      <div className="flex items-start gap-2">
                        <Info className="h-3 w-3 bp-md:h-4 bp-md:w-4 text-blue-600 dark:text-blue-400 mt-0.5 flex-shrink-0" />
                        <div>
                          <p className="text-xs bp-md:text-sm font-medium text-blue-700 dark:text-blue-300 mb-0.5 bp-md:mb-1">텐션 조정 팁</p>
                          <p className="text-[10px] bp-sm:text-xs text-blue-600 dark:text-blue-400">{string.adjustment}</p>
                        </div>
                      </div>
                    </div>
          ...
          <TabsContent value="tips" className="mt-6 bp-md:mt-8">
            <div className="grid gap-4 bp-md:gap-6">
              ...
              <Card className="overflow-hidden border bg-card">
                ...
                <CardHeader className="pb-3 bp-md:pb-4">
                  <CardTitle className="flex items-center gap-2 text-base bp-md:text-lg text-card-foreground">
                    <BarChart3 className="h-4 w-4 bp-md:h-5 bp-md:w-5 text-blue-600 dark:text-blue-400" />
                    텐션이 플레이에 미치는 영향
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="grid bp-md:grid-cols-2 gap-6 bp-md:gap-8">
                    ...
                    <div className="space-y-3 bp-md:space-y-4">
                      <div className="flex items-center gap-2 bp-sm:gap-3">
                        <div className="w-10 h-10 bp-md:w-12 bp-md:h-12 bg-emerald-100 dark:bg-emerald-900/40 rounded-xl flex items-center justify-center">
                          <TrendingUp className="h-5 w-5 bp-md:h-6 bp-md:w-6 text-emerald-600 dark:text-emerald-400 rotate-180" />
                        </div>
                        <div>
                      ...
                      <ul className="space-y-1.5 bp-md:space-y-2">
                        <li className="flex items-start gap-2 text-xs bp-md:text-sm text-muted-foreground">
                          <CheckCircle2 className="h-3 w-3 bp-md:h-4 bp-md:w-4 text-emerald-500 dark:text-emerald-400 mt-0.5 flex-shrink-0" />
                        ...
                        <li className="flex items-start gap-2 text-xs bp-md:text-sm text-muted-foreground">
                          <AlertTriangle className="h-3 w-3 bp-md:h-4 bp-md:w-4 text-amber-500 mt-0.5 flex-shrink-0" />
                          정밀한 컨트롤이 어려울 수 있습니다
                        </li>
                      </ul>
                    </div>
                    ...
                    <div className="space-y-3 bp-md:space-y-4">
                      <div className="flex items-center gap-2 bp-sm:gap-3">
                        <div className="w-10 h-10 bp-md:w-12 bp-md:h-12 bg-orange-100 dark:bg-orange-900/40 rounded-xl flex items-center justify-center">
                          <TrendingUp className="h-5 w-5 bp-md:h-6 bp-md:w-6 text-orange-600 dark:text-orange-400" />
                        </div>
                        <div>
              ...
              <div className="grid bp-sm:grid-cols-2 bp-lg:grid-cols-3 gap-4 bp-md:gap-6">
                <Card className="bg-blue-50 dark:bg-blue-900/20 border-blue-200 dark:border-blue-800">
                  <CardHeader className="pb-2 bp-md:pb-3">
                    <CardTitle className="text-sm bp-md:text-base flex items-center gap-2 text-blue-900 dark:text-blue-100">
                      <Gauge className="h-4 w-4 bp-md:h-5 bp-md:w-5" />
                      텐션 손실 이해하기
                    </CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-2 bp-md:space-y-3">
                    <p className="text-xs bp-md:text-sm text-blue-800 dark:text-blue-200">스트링은 장착 후 지속적으로 텐션이 감소합니다.</p>
                    <ul className="space-y-1.5 bp-md:space-y-2 text-xs bp-md:text-sm text-blue-700 dark:text-blue-300">
                      <li className="flex items-center gap-2">
                        <div className="w-1.5 h-1.5 bp-md:w-2 bp-md:h-2 bg-blue-400 rounded-full flex-shrink-0" />첫 24시간: 10-15% 손실
                      </li>
                      <li className="flex items-center gap-2">
                        <div className="w-1.5 h-1.5 bp-md:w-2 bp-md:h-2 bg-blue-400 rounded-full flex-shrink-0" />첫 주: 추가 5-10% 손실
                      </li>
                      <li className="flex items-center gap-2">
                        <div className="w-1.5 h-1.5 bp-md:w-2 bp-md:h-2 bg-blue-400 rounded-full flex-shrink-0" />
                        이후: 점진적 안정화
                      </li>
                    </ul>
                  </CardContent>
                ...
                <Card className="bg-violet-50 dark:bg-violet-900/20 border-violet-200 dark:border-violet-800">
                  <CardHeader className="pb-2 bp-md:pb-3">
                    <CardTitle className="text-sm bp-md:text-base flex items-center gap-2 text-violet-900 dark:text-violet-100">
                      <Shield className="h-4 w-4 bp-md:h-5 bp-md:w-5" />
                      스트링 보관 팁
                    </CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-2 bp-md:space-y-3">
                    <p className="text-xs bp-md:text-sm text-violet-800 dark:text-violet-200">올바른 보관은 스트링 수명과 텐션 유지에 중요합니다.</p>
                    <ul className="space-y-1.5 bp-md:space-y-2 text-xs bp-md:text-sm text-violet-700 dark:text-violet-300">
                      <li className="flex items-center gap-2">
                        <div className="w-1.5 h-1.5 bp-md:w-2 bp-md:h-2 bg-violet-400 rounded-full flex-shrink-0" />
                        직사광선 피하기
                      </li>
                      <li className="flex items-center gap-2">
                        <div className="w-1.5 h-1.5 bp-md:w-2 bp-md:h-2 bg-violet-400 rounded-full flex-shrink-0" />
                ...
                <Card className="bg-amber-50 dark:bg-amber-900/20 border-amber-200 dark:border-amber-800 bp-sm:col-span-2 bp-lg:col-span-1">
                  <CardHeader className="pb-2 bp-md:pb-3">
                    <CardTitle className="text-sm bp-md:text-base flex items-center gap-2 text-amber-900 dark:text-amber-100">
                      <Target className="h-4 w-4 bp-md:h-5 bp-md:w-5" />
                      교체 시기 판단
                    </CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-2 bp-md:space-y-3">
                    <p className="text-xs bp-md:text-sm text-amber-800 dark:text-amber-200">다음 신호가 나타나면 교체를 고려하세요.</p>
                    <ul className="space-y-1.5 bp-md:space-y-2 text-xs bp-md:text-sm text-amber-700 dark:text-amber-300">
                      <li className="flex items-center gap-2">
                        <div className="w-1.5 h-1.5 bp-md:w-2 bp-md:h-2 bg-amber-400 rounded-full flex-shrink-0" />
                        탄력 감소 느낌
                      </li>
                      <li className="flex items-center gap-2">
                        <div className="w-1.5 h-1.5 bp-md:w-2 bp-md:h-2 bg-amber-400 rounded-full flex-shrink-0" />
              ...
              <Card className="border bg-card">
                <CardHeader className="pb-3 bp-md:pb-4">
                  <CardTitle className="flex items-center gap-2 text-base bp-md:text-lg text-card-foreground">
                    <Info className="h-4 w-4 bp-md:h-5 bp-md:w-5 text-blue-600 dark:text-blue-400" />
                    자주 묻는 질문
                  </CardTitle>
                </CardHeader>
                <CardContent className="space-y-3 bp-md:space-y-4">
        ...
        {/* CTA 섹션 */}
        <Card className="bg-blue-600 dark:bg-blue-700 border-0 overflow-hidden">
          <CardContent className="p-6 bp-sm:p-8 bp-md:p-10 bp-lg:p-12">
            <div className="flex flex-col bp-md:flex-row items-center justify-between gap-4 bp-md:gap-6">
              <div className="text-center bp-md:text-left">
                <h3 className="text-xl bp-sm:text-2xl bp-md:text-3xl font-bold mb-2 text-white">최적의 텐션으로 스트링 서비스를 받아보세요</h3>
                <p className="text-blue-100 text-sm bp-md:text-base bp-lg:text-lg">스트링어가 정밀하게 작업해 드립니다</p>
              </div>
              <Button asChild size="lg" className="bg-white text-blue-600 hover:bg-blue-50 px-6 bp-md:px-8 whitespace-nowrap">
                <Link href="/services/apply">
                  스트링 신청하기
        ...
        <Card className="bg-blue-600 dark:bg-blue-700 border-0 overflow-hidden">
          <CardContent className="p-6 bp-sm:p-8 bp-md:p-10 bp-lg:p-12">
            <div className="flex flex-col bp-md:flex-row items-center justify-between gap-4 bp-md:gap-6">
              <div className="text-center bp-md:text-left">
                <h3 className="text-xl bp-sm:text-2xl bp-md:text-3xl font-bold mb-2 text-white">나에게 맞는 라켓을 찾아보세요</h3>
                <p className="text-blue-100 text-sm bp-md:text-base bp-lg:text-lg">라켓 파인더를 활용해 나의 라켓을 선택해보세요</p>
              </div>
              <Button asChild size="lg" className="bg-white text-blue-600 hover:bg-blue-50 px-6 bp-md:px-8 whitespace-nowrap">
