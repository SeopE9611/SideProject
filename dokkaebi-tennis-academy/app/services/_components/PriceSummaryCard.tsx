...
export default function PriceSummaryCard({ preferredDate, preferredTime, collectionMethod = 'self_ship', stringTypes, usingPackage, base, pickupFee, total }: PriceSummaryProps) {
  ...
  return (
    <Card className="overflow-hidden border border-slate-200/70 shadow-sm">
      {/* Header */}
      <div className="bg-gradient-to-r from-slate-900 via-slate-800 to-slate-700 text-white px-4 py-3">
        <div className="flex items-center gap-2">
          <ReceiptText className="h-4 w-4" />
          <p className="text-sm font-semibold">요금 요약</p>
        </div>
        <p className="text-[11px] text-slate-300 mt-1">입력에 따라 실시간 반영됩니다</p>
      </div>
        ...
        <div className="space-y-3">
          ...
          {usingPackage && (
            <div className="flex items-center justify-between">
              ...
              <p className="text-sm text-emerald-600">교체비 0원</p>
            </div>
          )}
          ...
          <div className="flex items-center justify-between">
            ...
            <p className="text-base font-bold tabular-nums rounded-lg px-2 py-1 bg-slate-100 ring-1 ring-inset ring-slate-200" aria-live="polite">
              {won(total)}
            </p>
          </div>
