        ...
        {products.map((p: any) => {
          ...
          return (
            ...
            >
              ...
              {typeof p.mountingFee === 'number' && <div className="text-xs text-slate-500 mt-1">장착비: {p.mountingFee.toLocaleString()}원</div>}
              <div className="mt-2 text-blue-600 text-sm">{isAdding ? '이동 중…' : '이 스트링 선택'}</div>
            </button>
          );
        })}
      </div>
