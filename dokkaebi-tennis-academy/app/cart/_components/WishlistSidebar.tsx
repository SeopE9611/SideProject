...
export default function WishlistSidebar({ className, variant = 'sidebar' }: Props) {
  ...
  return (
    <Card className={clsx('mt-6', className)}>
      <CardHeader className={clsx('rounded-t-lg', variant === 'inline' && 'bg-gradient-to-r from-blue-500/10 via-purple-500/10 to-teal-500/10')}>
      ...
      <CardContent className={variant === 'inline' ? 'p-0' : ''}>
        <div className={clsx(variant === 'inline' ? 'divide-y' : 'space-y-3')}>
          {list.map((it) => (
            ...
            >
              <Image src={it.image} alt={it.name} width={56} height={56} className="h-14 w-14 rounded-md border object-cover flex-shrink-0" />
              {/* 이름/가격 영역 - 긴 이름은 말줄임 */}
              <div className="flex-1 min-w-0">
                <Link href={`/products/${it.id}`} className="block truncate text-[15px] font-medium hover:underline">
                  {it.name}
                </Link>
