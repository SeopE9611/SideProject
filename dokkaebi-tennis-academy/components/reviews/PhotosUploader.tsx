        ...
        <div className="grid grid-cols-3 gap-2">
          ...
          {value.map((src, i) => (
            <div key={src + i} className="relative group rounded-md overflow-hidden border">
              ...
              <button type="button" onClick={() => removeAt(i)} className="absolute top-1 right-1 inline-flex p-1 rounded-full bg-black/50 text-white opacity-0 group-hover:opacity-100 transition-opacity" aria-label="삭제" title="삭제">
                <X className="h-3 w-3" />
              </button>
            </div>
          ))}
