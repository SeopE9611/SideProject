  ...
  return (
    <section className="mt-8 sm:mt-10 md:mt-12">
      <div className="mb-3 sm:mb-4 flex items-center justify-between">
        <h2 className="text-lg sm:text-xl font-semibold text-slate-900 dark:text-white">공지사항</h2>
        <Link className="text-xs sm:text-sm text-slate-600 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white transition-colors" href="/board/notice">
          더보기
        </Link>
      </div>
        ...
        {isLoading ? (
          ...
          items.map((p, idx) => (
            <li key={p.id ?? `${p.createdAt}-${idx}`}>
              <Link className="group flex items-start justify-between gap-2 sm:gap-3 rounded-lg px-3 sm:px-4 py-2.5 sm:py-3 transition-colors hover:bg-slate-50 dark:hover:bg-slate-900" href={`/board/notice/${p.id}`}>
          ...
          <li key="empty" className="py-8 sm:py-10 text-center text-xs sm:text-sm text-slate-500 dark:text-slate-400">
            등록된 공지사항이 없습니다
          </li>
        )}
      </ul>
