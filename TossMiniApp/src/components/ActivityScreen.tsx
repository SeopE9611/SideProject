...
export default function ActivityScreen({ onHome }: { onHome: () => void }) {
  ...
  return <main className="min-h-dvh bg-white pb-[calc(32px+env(safe-area-inset-bottom))] text-[#191f28]">
    <section className="pt-[calc(16px+env(safe-area-inset-top))]"><Top title={<Top.TitleParagraph size={22}>내 이용내역</Top.TitleParagraph>} subtitleBottom={<Top.SubtitleParagraph size={17}>MiniApp에서 접수한 교체서비스</Top.SubtitleParagraph>} /></section>
    <section className="px-6">
      ...
      {auth.status === "authenticated" && state === "success" && <ul className="m-0 list-none space-y-3 p-0">{items.map((item) => <li key={item.id} className="rounded-[20px] border border-[#e5e8eb] p-5"><strong className="block text-[17px]">{item.productName}</strong><p className="mt-2 mb-0 text-sm text-[#6b7684]">{item.createdAt ? new Date(item.createdAt).toLocaleDateString("ko-KR") : "신청일 확인 중"} · {item.collectionMethod === "visit" ? "방문 접수" : "직접 발송"}</p>{item.collectionMethod === "visit" && item.preferredDate && <p className="mt-1 mb-0 text-sm">방문 {item.preferredDate} {item.preferredTime ?? ""}</p>}<div className="mt-4 flex justify-between text-sm"><span>{item.status} · {item.paymentStatus}</span><b>{formatPrice(item.amount)}</b></div></li>)}</ul>}
    </section>
  </main>;
}
