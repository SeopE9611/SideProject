...
export default function RacketPurchaseStepOne({
  racket,
  availability,
  quantity,
  stringProductId,
  ...
  return (
    <main className="min-h-dvh bg-white pb-[calc(32px+env(safe-area-inset-bottom))] text-[#191f28]">
      <section className="pt-[calc(16px+env(safe-area-inset-top))]">
        <Top title={<Top.TitleParagraph size={22}>라켓 구매</Top.TitleParagraph>} subtitleBottom={<Top.SubtitleParagraph size={17}>1 / 6 · 라켓 수량·스트링 선택</Top.SubtitleParagraph>} />
      </section>
      <section className="px-6 max-[359px]:px-5">
        <button type="button" className="mb-4 border-0 bg-transparent p-0 text-sm font-bold text-[#688d00]" onClick={onBack}>라켓 상세로</button>
        ...
        <section className="mt-8" aria-labelledby="string-list-title">
          ...
          <p className="mt-1 text-sm leading-6 text-[#6b7684]">라켓 수량과 같은 수량의 스트링 및 장착서비스가 함께 주문됩니다.</p>
