...
export default function RacketRentalStepOne({ racket, availability, days, errorMessage = "", onChange, onBack, onContinue }: { racket: RacketDetail; availability: RacketAvailability; days: RentalDays | null; errorMessage?: string; onChange: (days: RentalDays) => void; onBack: () => void; onContinue: () => void }) {
  const reason = getRacketRentalAvailability(racket, availability, days);
  if (reason !== "available") return <main className="min-h-dvh bg-white px-6 pt-[calc(24px+env(safe-area-inset-top))]"><Top title={<Top.TitleParagraph size={22}>라켓 대여</Top.TitleParagraph>} subtitleBottom={<Top.SubtitleParagraph size={17}>1 / 7 · 대여 기간</Top.SubtitleParagraph>} /><div role="alert" className="mt-6 rounded-2xl bg-[#fff4f2] p-5"><strong>대여를 진행할 수 없어요.</strong><p className="text-sm">{RACKET_RENTAL_AVAILABILITY_MESSAGES[reason]}</p></div><button type="button" onClick={onBack} className="mt-6 min-h-[52px] w-full rounded-2xl border bg-white font-bold">라켓 상세로</button></main>;
  ...
  return <main className="min-h-dvh bg-white px-6 pb-8 text-[#191f28]"><section className="pt-[calc(16px+env(safe-area-inset-top))]"><Top title={<Top.TitleParagraph size={22}>라켓 대여</Top.TitleParagraph>} subtitleBottom={<Top.SubtitleParagraph size={17}>1 / 7 · 대여 기간</Top.SubtitleParagraph>} /></section>
    ...
    <div className="grid grid-cols-[.72fr_1.28fr] gap-2"><button onClick={onBack} className="min-h-[52px] rounded-2xl border bg-white">상세로</button><button disabled={!days || getRacketRentalAvailability(racket, availability, days) !== "available"} onClick={onContinue} className="min-h-[52px] rounded-2xl border-0 bg-[#191f28] font-bold text-white disabled:bg-[#e5e8eb]">다음: 스트링 선택</button></div>
  </main>;
}
